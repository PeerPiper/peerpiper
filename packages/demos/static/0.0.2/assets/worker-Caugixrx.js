...
let Descriptor$1 = (_a = class {
  ...
  getType() {
    if (__privateGet(this, _stream)) return "fifo";
    if (__privateGet(this, _entry).dir) return "directory";
    if (__privateGet(this, _entry).source) return "regular-file";
    return "unknown";
  }
  setSize(size) {
...
function getErrorPayload(e4) {
  if (e4 && hasOwnProperty.call(e4, "payload")) return e4.payload;
...
const $init = (() => {
  let gen = function* init() {
    const module0 = fetchCompile(new URL("http://localhost:4175/0.0.2/assets/js-component-bindgen-component.core-DAdJdxwU.wasm", import.meta.url));
    const module1 = fetchCompile(new URL("http://localhost:4175/0.0.2/assets/js-component-bindgen-component.core2-C0sLQRk6.wasm", import.meta.url));
    ...
    yield instantiateCore(yield module3, {
      "": {
...
!function(e4) {
  ...
  const o2 = "undefined" != typeof TextDecoder ? new TextDecoder() : "undefined" != typeof Buffer ? { decode: (e5) => Buffer.from(e5.buffer, e5.byteOffset, e5.byteLength).toString() } : { decode(e5) {
    let t3 = "";
    for (let n3 = 0; n3 < e5.length; n3++) t3 += String.fromCharCode(e5[n3]);
    return t3;
  } };
  function a2(e5) {
...
let i = class e2 {
  ...
  eachNext(e4) {
    let t2 = this;
    for (; t2; ) e4(t2), t2 = t2.next;
  }
  eachPrevious(e4) {
    let t2 = this;
    for (; t2; ) e4(t2), t2 = t2.previous;
  }
  edit(e4, t2, n2) {
...
function l(e4, t2) {
  ...
  for (n2.pop(); n2[0] === s2[0]; ) n2.shift(), s2.shift();
  if (n2.length) {
    let e5 = n2.length;
    for (; e5--; ) n2[e5] = "..";
  }
...
function d(e4) {
  const t2 = e4.split("\n"), n2 = [];
  for (let e5 = 0, s2 = 0; e5 < t2.length; e5++) n2.push(s2), s2 += t2[e5].length + 1;
  return function(e5) {
...
class p {
  ...
  advance(e4) {
    ...
    const t2 = e4.split("\n");
    if (t2.length > 1) {
      for (let e5 = 0; e5 < t2.length - 1; e5++) this.generatedCodeLine++, this.raw[this.generatedCodeLine] = this.rawSegments = [];
...
class g {
  ...
  append(e4) {
    if ("string" != typeof e4) throw new TypeError("outro content must be a string");
    return this.outro += e4, this;
  }
  appendLeft(e4, t2) {
    if ("string" != typeof t2) throw new TypeError("inserted content must be a string");
    this._split(e4);
  ...
  _ensureindentStr() {
    void 0 === this.indentStr && (this.indentStr = function(e4) {
      ...
      if (0 === n2.length && 0 === s2.length) return null;
      if (n2.length >= s2.length) return "	";
  ...
  indent(e4, t2) {
    const n2 = /^[^\r\n]/gm;
    if (u(e4) && (t2 = e4, e4 = void 0), void 0 === e4 && (this._ensureindentStr(), e4 = this.indentStr || "	"), "" === e4) return this;
    const s2 = {};
    if ((t2 = t2 || {}).exclude) {
      ("number" == typeof t2.exclude[0] ? [t2.exclude] : t2.exclude).forEach((e5) => {
        for (let t3 = e5[0]; t3 < e5[1]; t3 += 1) s2[t3] = true;
      });
    }
    ...
    for (; a2; ) {
      const t3 = a2.end;
      if (a2.edited) s2[o2] || (a2.content = a2.content.replace(n2, r2), a2.content.length && (i2 = "\n" === a2.content[a2.content.length - 1]));
      else for (o2 = a2.start; o2 < t3; ) {
        if (!s2[o2]) {
  ...
  move(e4, t2, n2) {
    if (n2 >= e4 && n2 <= t2) throw new Error("Cannot move a selection inside itself");
    ...
    if (!a2 && i2 === this.lastChunk) return this;
  ...
  update(e4, t2, n2, s2) {
    if ("string" != typeof n2) throw new TypeError("replacement content must be a string");
    for (; e4 < 0; ) e4 += this.original.length;
    for (; t2 < 0; ) t2 += this.original.length;
    if (t2 > this.original.length) throw new Error("end is out of bounds");
    if (e4 === t2) throw new Error("Cannot overwrite a zero-length range – use appendLeft or prependRight instead");
    ...
    if (a2) {
      let e5 = a2;
      for (; e5 !== l2; ) {
        if (e5.next !== this.byStart[e5.end]) throw new Error("Cannot overwrite across a split point");
  ...
  remove(e4, t2) {
    for (; e4 < 0; ) e4 += this.original.length;
    for (; t2 < 0; ) t2 += this.original.length;
    if (e4 === t2) return this;
    if (e4 < 0 || t2 > this.original.length) throw new Error("Character is out of bounds");
    if (e4 > t2) throw new Error("end must be greater than start");
  ...
  lastLine() {
    ...
    if (-1 !== e4) return this.outro.substr(e4 + 1);
    let t2 = this.outro, n2 = this.lastChunk;
    do {
      if (n2.outro.length > 0) {
        if (e4 = n2.outro.lastIndexOf(f), -1 !== e4) return n2.outro.substr(e4 + 1) + t2;
        t2 = n2.outro + t2;
      }
      if (n2.content.length > 0) {
        if (e4 = n2.content.lastIndexOf(f), -1 !== e4) return n2.content.substr(e4 + 1) + t2;
        t2 = n2.content + t2;
      }
      if (n2.intro.length > 0) {
        if (e4 = n2.intro.lastIndexOf(f), -1 !== e4) return n2.intro.substr(e4 + 1) + t2;
        t2 = n2.intro + t2;
      }
  ...
  slice(e4 = 0, t2 = this.original.length) {
    ...
    for (; s2 && (s2.start > e4 || s2.end <= e4); ) {
      if (s2.start < t2 && s2.end >= t2) return n2;
      s2 = s2.next;
    }
    if (s2 && s2.edited && s2.start !== e4) throw new Error(`Cannot use replaced character ${e4} as slice start anchor.`);
    const i2 = s2;
    for (; s2; ) {
      !s2.intro || i2 === s2 && s2.start !== e4 || (n2 += s2.intro);
      const r2 = s2.start < t2 && s2.end >= t2;
      if (r2 && s2.edited && s2.end !== t2) throw new Error(`Cannot use replaced character ${t2} as slice end anchor.`);
      const o2 = i2 === s2 ? e4 - s2.start : 0, a2 = r2 ? s2.content.length + t2 - s2.end : s2.content.length;
      if (n2 += s2.content.slice(o2, a2), !s2.outro || r2 && s2.end !== t2 || (n2 += s2.outro), r2) break;
      s2 = s2.next;
    }
    return n2;
  }
  ...
  _split(e4) {
    ...
    const n2 = e4 > t2.end;
    for (; t2; ) {
      if (t2.contains(e4)) return this._splitChunk(t2, e4);
  ...
  toString() {
    ...
    for (; t2; ) e4 += t2.toString(), t2 = t2.next;
  ...
  trimEndAborted(e4) {
    ...
    do {
      ...
      if (n2.end !== e5 && (this.lastChunk === n2 && (this.lastChunk = n2.next), this.byEnd[n2.end] = n2, this.byStart[n2.next.start] = n2.next, this.byEnd[n2.next.end] = n2.next), s2) return true;
      n2 = n2.previous;
    } while (n2);
    return false;
  }
  ...
  trimStartAborted(e4) {
    ...
    do {
      ...
      if (n2.end !== e5 && (n2 === this.lastChunk && (this.lastChunk = n2.next), this.byEnd[n2.end] = n2, this.byStart[n2.next.start] = n2.next, this.byEnd[n2.next.end] = n2.next), s2) return true;
      n2 = n2.next;
    } while (n2);
    return false;
  }
  ...
  replaceAll(e4, t2) {
    if ("string" == typeof e4) return this._replaceAllString(e4, t2);
...
let b = class e3 {
  ...
  addSource(e4) {
    if (e4 instanceof g) return this.addSource({ content: e4, filename: e4.filename, separator: this.separator });
    if (!u(e4) || !e4.content) throw new Error("bundle.addSource() takes an object with a `content` property, which should be an instance of MagicString, and an optional `filename`");
    if (["filename", "ignoreList", "indentExclusionRanges", "separator"].forEach((t2) => {
      y.call(e4, t2) || (e4[t2] = e4.content[t2]);
    }), void 0 === e4.separator && (e4.separator = this.separator), e4.filename) if (y.call(this.uniqueSourceIndexByFilename, e4.filename)) {
      const t2 = this.uniqueSources[this.uniqueSourceIndexByFilename[e4.filename]];
      if (e4.content.original !== t2.content) throw new Error(`Illegal source: same filename (${e4.filename}), different contents`);
    } else this.uniqueSourceIndexByFilename[e4.filename] = this.uniqueSources.length, this.uniqueSources.push({ filename: e4.filename, content: e4.content.original });
  ...
  indent(e4) {
    if (arguments.length || (e4 = this.getIndentString()), "" === e4) return this;
    let t2 = !this.intro || "\n" === this.intro.slice(-1);
  ...
  trimStart(e4) {
    ...
    if (this.intro = this.intro.replace(t2, ""), !this.intro) {
      let t3, n2 = 0;
      do {
        if (t3 = this.sources[n2++], !t3) break;
...
function C(e4, t2) {
  ...
  for ("." === n2[0] && n2.shift(), "." === s2[0] && s2.shift(); n2[0] && s2[0] && n2[0] === s2[0]; ) n2.shift(), s2.shift();
  for (; ".." === s2[0] && n2.length > 0; ) s2.shift(), n2.pop();
  for (; n2.pop(); ) s2.unshift("..");
  return s2.join("/");
}
function O(...e4) {
  const t2 = e4.shift();
  if (!t2) return "/";
  let n2 = t2.split(A);
  for (const t3 of e4) if (w(t3)) n2 = t3.split(A);
  else {
    const e5 = t3.split(A);
    for (; "." === e5[0] || ".." === e5[0]; ) {
      ".." === e5.shift() && n2.pop();
...
function V(e4, t2, n2, s2) {
  for (; t2.startsWith("../"); ) t2 = t2.slice(3), e4 = "_/" + e4;
  let i2 = P(C(k(e4), t2));
  if (n2 && i2.endsWith(".js") && (i2 = i2.slice(0, -3)), s2) {
    if ("" === i2) return "../" + I(t2);
    if (z.test(i2)) return [...i2.split("/"), "..", I(t2)].join("/");
  }
...
class F {
  ...
  getImportAttributes(e4) {
    return this.importAttributes || (this.importAttributes = function(e5, { getObject: t2 }) {
      ...
      if (n2.length > 0) return t2(n2, { lineBreakIndent: null });
      return null;
...
class ee {
  ...
  getEntities(e4) {
    ...
    for (const n2 of e4) t2 = t2[n2] = t2[n2] || Object.create(null, { [Q]: { value: /* @__PURE__ */ new Set() } });
    return t2[Q];
  }
}
const te = new ee();
class ne {
  ...
  trackEntityAtPathAndGetIfTracked(e4, t2, n2) {
    ...
    for (const t3 of e4) s2 = s2[t3] = s2[t3] || Object.create(null, { [Q]: { value: /* @__PURE__ */ new Map() } });
...
class ae {
  ...
  includeCallArguments(e4, t2) {
    for (const n2 of t2) n2.include(e4, false);
  }
  shouldBeIncluded(e4) {
    return true;
  }
...
const le = new class extends ae {
  ...
  for (const t2 of e4.args) t2 == null ? void 0 : t2.deoptimizePath(J);
...
class xe extends ae {
  ...
  getName(e4, t2) {
    if (t2 == null ? void 0 : t2(this)) return this.name;
    if (this.renderedLikeHoisted) return this.renderedLikeHoisted.getName(e4, t2);
...
function Fe(e4, t2, n2) {
  return function(e5, t3 = {}) {
    ...
    return function(t4, i3) {
      if ("string" == typeof t4 && (t4 = e5.indexOf(t4, i3 ?? 0)), -1 === t4) return;
      let a2 = r2[o2];
      const l2 = t4 >= a2.end ? 1 : -1;
      for (; a2; ) {
        if (Ve(a2, t4)) return { line: n3 + a2.line, column: s2 + t4 - a2.start, character: t4 };
        o2 += l2, a2 = r2[o2];
      }
    };
...
function ht(e4) {
  var _a3;
  if (!e4.plugin && !e4.loc || e4[dt]) return;
  e4[dt] = true;
  let t2 = "";
...
class En {
  ...
  warnUnusedImports() {
    ...
    for (const n3 of e4) for (const e5 of this.declarations.get(n3).module.importers) t2.add(e5);
    const n2 = [...t2];
    var s2, i2, r2;
...
const xn = { ArrayPattern(e4, t2) {
  for (const n2 of t2.elements) n2 && xn[n2.type](e4, n2);
  ...
  for (const n2 of t2.properties) "RestElement" === n2.type ? xn.RestElement(e4, n2) : xn[n2.value.type](e4, n2.value);
}, RestElement(e4, t2) {
...
async function Vn(e4) {
  ...
  void 0 === e4 && (e4 = new URL("http://localhost:4175/0.0.2/assets/bindings_wasm_bg-Ba-JsYst.wasm", import.meta.url));
  const t2 = zn();
  ...
  const { instance: n2, module: s2 } = await async function(e5, t3) {
    if ("function" == typeof Response && e5 instanceof Response) {
      if ("function" == typeof WebAssembly.instantiateStreaming) try {
        ...
        if ("application/wasm" == e5.headers.get("Content-Type")) throw t4;
...
const Gn = "_rollupAnnotations", Wn = "_rollupRemoved", qn = (e4, t2) => {
  if (0 === e4) return we;
  const n2 = t2[e4++], s2 = [];
  for (let i2 = 0; i2 < n2; i2++) s2.push(Hn(t2[e4++], t2));
  return s2;
}, Hn = (e4, t2) => {
  const n2 = t2[e4++];
  return { end: t2[e4++], start: n2, type: Un[t2[e4]] };
};
const Kn = [function(e4, t2) {
  return { type: "PanicError", start: t2[e4], end: t2[e4 + 1], message: t2.convertString(t2[e4 + 2]) };
}, function(e4, t2) {
  return { type: "ParseError", start: t2[e4], end: t2[e4 + 1], message: t2.convertString(t2[e4 + 2]) };
}, function(e4, t2) {
  return { type: "ArrayExpression", start: t2[e4], end: t2[e4 + 1], elements: Jn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  return { type: "ArrayPattern", start: t2[e4], end: t2[e4 + 1], elements: Jn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = qn(t2[e4 + 3], t2);
  return { type: "ArrowFunctionExpression", start: t2[e4], end: t2[e4 + 1], async: !(1 & ~n2), expression: !(2 & ~n2), generator: !(4 & ~n2), ...s2.length > 0 ? { [Gn]: s2 } : {}, params: Jn(t2[e4 + 4], t2), body: Yn(t2[e4 + 5], t2), id: null };
}, function(e4, t2) {
  return { type: "AssignmentExpression", start: t2[e4], end: t2[e4 + 1], operator: Un[t2[e4 + 2]], left: Yn(t2[e4 + 3], t2), right: Yn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  return { type: "AssignmentPattern", start: t2[e4], end: t2[e4 + 1], left: Yn(t2[e4 + 2], t2), right: Yn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  return { type: "AwaitExpression", start: t2[e4], end: t2[e4 + 1], argument: Yn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  return { type: "BinaryExpression", start: t2[e4], end: t2[e4 + 1], operator: Un[t2[e4 + 2]], left: Yn(t2[e4 + 3], t2), right: Yn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  return { type: "BlockStatement", start: t2[e4], end: t2[e4 + 1], body: Jn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2];
  return { type: "BreakStatement", start: t2[e4], end: t2[e4 + 1], label: 0 === n2 ? null : Yn(n2, t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = qn(t2[e4 + 3], t2);
  return { type: "CallExpression", start: t2[e4], end: t2[e4 + 1], optional: !(1 & ~n2), ...s2.length > 0 ? { [Gn]: s2 } : {}, callee: Yn(t2[e4 + 4], t2), arguments: Jn(t2[e4 + 5], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2];
  return { type: "CatchClause", start: t2[e4], end: t2[e4 + 1], param: 0 === n2 ? null : Yn(n2, t2), body: Yn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  return { type: "ChainExpression", start: t2[e4], end: t2[e4 + 1], expression: Yn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  return { type: "ClassBody", start: t2[e4], end: t2[e4 + 1], body: Jn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = t2[e4 + 3];
  return { type: "ClassDeclaration", start: t2[e4], end: t2[e4 + 1], id: 0 === n2 ? null : Yn(n2, t2), superClass: 0 === s2 ? null : Yn(s2, t2), body: Yn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = t2[e4 + 3];
  return { type: "ClassExpression", start: t2[e4], end: t2[e4 + 1], id: 0 === n2 ? null : Yn(n2, t2), superClass: 0 === s2 ? null : Yn(s2, t2), body: Yn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  return { type: "ConditionalExpression", start: t2[e4], end: t2[e4 + 1], test: Yn(t2[e4 + 2], t2), consequent: Yn(t2[e4 + 3], t2), alternate: Yn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2];
  return { type: "ContinueStatement", start: t2[e4], end: t2[e4 + 1], label: 0 === n2 ? null : Yn(n2, t2) };
}, function(e4, t2) {
  return { type: "DebuggerStatement", start: t2[e4], end: t2[e4 + 1] };
}, function(e4, t2) {
  return { type: "ExpressionStatement", start: t2[e4], end: t2[e4 + 1], directive: t2.convertString(t2[e4 + 2]), expression: Yn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  return { type: "DoWhileStatement", start: t2[e4], end: t2[e4 + 1], body: Yn(t2[e4 + 2], t2), test: Yn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  return { type: "EmptyStatement", start: t2[e4], end: t2[e4 + 1] };
}, function(e4, t2) {
  const n2 = t2[e4 + 2];
  return { type: "ExportAllDeclaration", start: t2[e4], end: t2[e4 + 1], exported: 0 === n2 ? null : Yn(n2, t2), source: Yn(t2[e4 + 3], t2), attributes: Jn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  return { type: "ExportDefaultDeclaration", start: t2[e4], end: t2[e4 + 1], declaration: Yn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 3], s2 = t2[e4 + 5];
  return { type: "ExportNamedDeclaration", start: t2[e4], end: t2[e4 + 1], specifiers: Jn(t2[e4 + 2], t2), source: 0 === n2 ? null : Yn(n2, t2), attributes: Jn(t2[e4 + 4], t2), declaration: 0 === s2 ? null : Yn(s2, t2) };
}, function(e4, t2) {
  const n2 = Yn(t2[e4 + 2], t2), s2 = t2[e4 + 3];
  return { type: "ExportSpecifier", start: t2[e4], end: t2[e4 + 1], local: n2, exported: 0 === s2 ? { ...n2 } : Yn(s2, t2) };
}, function(e4, t2) {
  return { type: "ExpressionStatement", start: t2[e4], end: t2[e4 + 1], expression: Yn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  return { type: "ForInStatement", start: t2[e4], end: t2[e4 + 1], left: Yn(t2[e4 + 2], t2), right: Yn(t2[e4 + 3], t2), body: Yn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2];
  return { type: "ForOfStatement", start: t2[e4], end: t2[e4 + 1], await: !(1 & ~n2), left: Yn(t2[e4 + 3], t2), right: Yn(t2[e4 + 4], t2), body: Yn(t2[e4 + 5], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = t2[e4 + 3], i2 = t2[e4 + 4];
  return { type: "ForStatement", start: t2[e4], end: t2[e4 + 1], init: 0 === n2 ? null : Yn(n2, t2), test: 0 === s2 ? null : Yn(s2, t2), update: 0 === i2 ? null : Yn(i2, t2), body: Yn(t2[e4 + 5], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = qn(t2[e4 + 3], t2), i2 = t2[e4 + 4];
  return { type: "FunctionDeclaration", start: t2[e4], end: t2[e4 + 1], async: !(1 & ~n2), generator: !(2 & ~n2), ...s2.length > 0 ? { [Gn]: s2 } : {}, id: 0 === i2 ? null : Yn(i2, t2), params: Jn(t2[e4 + 5], t2), body: Yn(t2[e4 + 6], t2), expression: false };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = qn(t2[e4 + 3], t2), i2 = t2[e4 + 4];
  return { type: "FunctionExpression", start: t2[e4], end: t2[e4 + 1], async: !(1 & ~n2), generator: !(2 & ~n2), ...s2.length > 0 ? { [Gn]: s2 } : {}, id: 0 === i2 ? null : Yn(i2, t2), params: Jn(t2[e4 + 5], t2), body: Yn(t2[e4 + 6], t2), expression: false };
}, function(e4, t2) {
  return { type: "Identifier", start: t2[e4], end: t2[e4 + 1], name: t2.convertString(t2[e4 + 2]) };
}, function(e4, t2) {
  const n2 = t2[e4 + 4];
  return { type: "IfStatement", start: t2[e4], end: t2[e4 + 1], test: Yn(t2[e4 + 2], t2), consequent: Yn(t2[e4 + 3], t2), alternate: 0 === n2 ? null : Yn(n2, t2) };
}, function(e4, t2) {
  return { type: "ImportAttribute", start: t2[e4], end: t2[e4 + 1], key: Yn(t2[e4 + 2], t2), value: Yn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  return { type: "ImportDeclaration", start: t2[e4], end: t2[e4 + 1], specifiers: Jn(t2[e4 + 2], t2), source: Yn(t2[e4 + 3], t2), attributes: Jn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  return { type: "ImportDefaultSpecifier", start: t2[e4], end: t2[e4 + 1], local: Yn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 3];
  return { type: "ImportExpression", start: t2[e4], end: t2[e4 + 1], source: Yn(t2[e4 + 2], t2), options: 0 === n2 ? null : Yn(n2, t2) };
}, function(e4, t2) {
  return { type: "ImportNamespaceSpecifier", start: t2[e4], end: t2[e4 + 1], local: Yn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = Yn(t2[e4 + 3], t2);
  return { type: "ImportSpecifier", start: t2[e4], end: t2[e4 + 1], imported: 0 === n2 ? { ...s2 } : Yn(n2, t2), local: s2 };
}, function(e4, t2) {
  return { type: "LabeledStatement", start: t2[e4], end: t2[e4 + 1], label: Yn(t2[e4 + 2], t2), body: Yn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  const n2 = t2.convertString(t2[e4 + 2]);
  return { type: "Literal", start: t2[e4], end: t2[e4 + 1], bigint: n2, raw: t2.convertString(t2[e4 + 3]), value: BigInt(n2) };
}, function(e4, t2) {
  const n2 = !(1 & ~t2[e4 + 2]);
  return { type: "Literal", start: t2[e4], end: t2[e4 + 1], value: n2, raw: n2 ? "true" : "false" };
}, function(e4, t2) {
  return { type: "Literal", start: t2[e4], end: t2[e4 + 1], raw: "null", value: null };
}, function(e4, t2) {
  const n2 = t2[e4 + 2];
  return { type: "Literal", start: t2[e4], end: t2[e4 + 1], raw: 0 === n2 ? void 0 : t2.convertString(n2), value: new DataView(t2.buffer).getFloat64(e4 + 3 << 2, true) };
}, function(e4, t2) {
  const n2 = t2.convertString(t2[e4 + 2]), s2 = t2.convertString(t2[e4 + 3]);
  return { type: "Literal", start: t2[e4], end: t2[e4 + 1], raw: `/${s2}/${n2}`, regex: { flags: n2, pattern: s2 }, value: new RegExp(s2, n2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 3];
  return { type: "Literal", start: t2[e4], end: t2[e4 + 1], value: t2.convertString(t2[e4 + 2]), raw: 0 === n2 ? void 0 : t2.convertString(n2) };
}, function(e4, t2) {
  return { type: "LogicalExpression", start: t2[e4], end: t2[e4 + 1], operator: Un[t2[e4 + 2]], left: Yn(t2[e4 + 3], t2), right: Yn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2];
  return { type: "MemberExpression", start: t2[e4], end: t2[e4 + 1], computed: !(1 & ~n2), optional: !(2 & ~n2), object: Yn(t2[e4 + 3], t2), property: Yn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  return { type: "MetaProperty", start: t2[e4], end: t2[e4 + 1], meta: Yn(t2[e4 + 2], t2), property: Yn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2];
  return { type: "MethodDefinition", start: t2[e4], end: t2[e4 + 1], static: !(1 & ~n2), computed: !(2 & ~n2), key: Yn(t2[e4 + 3], t2), value: Yn(t2[e4 + 4], t2), kind: Un[t2[e4 + 5]] };
}, function(e4, t2) {
  const n2 = qn(t2[e4 + 2], t2);
  return { type: "NewExpression", start: t2[e4], end: t2[e4 + 1], ...n2.length > 0 ? { [Gn]: n2 } : {}, callee: Yn(t2[e4 + 3], t2), arguments: Jn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  return { type: "ObjectExpression", start: t2[e4], end: t2[e4 + 1], properties: Jn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  return { type: "ObjectPattern", start: t2[e4], end: t2[e4 + 1], properties: Jn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  return { type: "PrivateIdentifier", start: t2[e4], end: t2[e4 + 1], name: t2.convertString(t2[e4 + 2]) };
}, function(e4, t2) {
  const n2 = qn(t2[e4 + 3], t2);
  return { type: "Program", start: t2[e4], end: t2[e4 + 1], body: Jn(t2[e4 + 2], t2), ...n2.length > 0 ? { [Wn]: n2 } : {}, sourceType: "module" };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = t2[e4 + 3], i2 = Yn(t2[e4 + 4], t2);
  return { type: "Property", start: t2[e4], end: t2[e4 + 1], method: !(1 & ~n2), shorthand: !(2 & ~n2), computed: !(4 & ~n2), key: 0 === s2 ? { ...i2 } : Yn(s2, t2), value: i2, kind: Un[t2[e4 + 5]] };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = t2[e4 + 4];
  return { type: "PropertyDefinition", start: t2[e4], end: t2[e4 + 1], static: !(1 & ~n2), computed: !(2 & ~n2), key: Yn(t2[e4 + 3], t2), value: 0 === s2 ? null : Yn(s2, t2) };
}, function(e4, t2) {
  return { type: "RestElement", start: t2[e4], end: t2[e4 + 1], argument: Yn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2];
  return { type: "ReturnStatement", start: t2[e4], end: t2[e4 + 1], argument: 0 === n2 ? null : Yn(n2, t2) };
}, function(e4, t2) {
  return { type: "SequenceExpression", start: t2[e4], end: t2[e4 + 1], expressions: Jn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  return { type: "SpreadElement", start: t2[e4], end: t2[e4 + 1], argument: Yn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  return { type: "StaticBlock", start: t2[e4], end: t2[e4 + 1], body: Jn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  return { type: "Super", start: t2[e4], end: t2[e4 + 1] };
}, function(e4, t2) {
  const n2 = t2[e4 + 2];
  return { type: "SwitchCase", start: t2[e4], end: t2[e4 + 1], test: 0 === n2 ? null : Yn(n2, t2), consequent: Jn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  return { type: "SwitchStatement", start: t2[e4], end: t2[e4 + 1], discriminant: Yn(t2[e4 + 2], t2), cases: Jn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  return { type: "TaggedTemplateExpression", start: t2[e4], end: t2[e4 + 1], tag: Yn(t2[e4 + 2], t2), quasi: Yn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = t2[e4 + 3], i2 = 0 === s2 ? void 0 : t2.convertString(s2), r2 = t2.convertString(t2[e4 + 4]);
  return { type: "TemplateElement", start: t2[e4], end: t2[e4 + 1], tail: !(1 & ~n2), value: { cooked: i2, raw: r2 } };
}, function(e4, t2) {
  return { type: "TemplateLiteral", start: t2[e4], end: t2[e4 + 1], quasis: Jn(t2[e4 + 2], t2), expressions: Jn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  return { type: "ThisExpression", start: t2[e4], end: t2[e4 + 1] };
}, function(e4, t2) {
  return { type: "ThrowStatement", start: t2[e4], end: t2[e4 + 1], argument: Yn(t2[e4 + 2], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 3], s2 = t2[e4 + 4];
  return { type: "TryStatement", start: t2[e4], end: t2[e4 + 1], block: Yn(t2[e4 + 2], t2), handler: 0 === n2 ? null : Yn(n2, t2), finalizer: 0 === s2 ? null : Yn(s2, t2) };
}, function(e4, t2) {
  return { type: "UnaryExpression", start: t2[e4], end: t2[e4 + 1], operator: Un[t2[e4 + 2]], argument: Yn(t2[e4 + 3], t2), prefix: true };
}, function(e4, t2) {
  const n2 = t2[e4 + 2];
  return { type: "UpdateExpression", start: t2[e4], end: t2[e4 + 1], prefix: !(1 & ~n2), operator: Un[t2[e4 + 3]], argument: Yn(t2[e4 + 4], t2) };
}, function(e4, t2) {
  return { type: "VariableDeclaration", start: t2[e4], end: t2[e4 + 1], kind: Un[t2[e4 + 2]], declarations: Jn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 3];
  return { type: "VariableDeclarator", start: t2[e4], end: t2[e4 + 1], id: Yn(t2[e4 + 2], t2), init: 0 === n2 ? null : Yn(n2, t2) };
}, function(e4, t2) {
  return { type: "WhileStatement", start: t2[e4], end: t2[e4 + 1], test: Yn(t2[e4 + 2], t2), body: Yn(t2[e4 + 3], t2) };
}, function(e4, t2) {
  const n2 = t2[e4 + 2], s2 = t2[e4 + 3];
  return { type: "YieldExpression", start: t2[e4], end: t2[e4 + 1], delegate: !(1 & ~n2), argument: 0 === s2 ? null : Yn(s2, t2) };
}];
function Yn(e4, t2) {
  const n2 = t2[e4], s2 = Kn[n2];
  if (!s2) throw console.trace(), new Error(`Unknown node type: ${n2}`);
  return s2(e4 + 1, t2);
}
function Jn(e4, t2) {
  if (0 === e4) return we;
  ...
  for (let i2 = 0; i2 < n2; i2++) {
    const n3 = t2[e4++];
    s2.push(n3 ? Yn(n3, t2) : null);
  }
  return s2;
}
function Xn(e4) {
  const t2 = new Uint32Array(e4.buffer);
  let n2;
  if ("undefined" != typeof Buffer && e4 instanceof Buffer) n2 = (n3) => {
    const s2 = t2[n3++], i2 = n3 << 2;
    return e4.toString("utf8", i2, i2 + s2);
  };
  else {
    const s2 = new TextDecoder();
    n2 = (n3) => {
      const i2 = t2[n3++], r2 = n3 << 2;
      return s2.decode(e4.subarray(r2, r2 + i2));
    };
  }
  return Object.assign(t2, { convertString: n2 });
}
function Zn(e4, t2 = null) {
  return Object.create(t2, e4);
}
const Qn = new class extends ae {
  getLiteralValueAtPath() {
  }
}(), es = { value: { hasEffectsWhenCalled: null, returns: le } }, ts = new class extends ae {
  getReturnExpressionWhenCalledAtPath(e4) {
    return 1 === e4.length ? fs(cs, e4[0]) : ce;
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    return 0 === t2.type ? e4.length > 1 : 2 !== t2.type || 1 !== e4.length || ps(cs, e4[0], t2, n2);
  }
}(), ns = { value: { hasEffectsWhenCalled: null, returns: ts } }, ss = new class extends ae {
  getReturnExpressionWhenCalledAtPath(e4) {
    return 1 === e4.length ? fs(us, e4[0]) : ce;
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    return 0 === t2.type ? e4.length > 1 : 2 !== t2.type || 1 !== e4.length || ps(us, e4[0], t2, n2);
  }
}(), is = { value: { hasEffectsWhenCalled: null, returns: ss } }, rs = new class extends ae {
  getReturnExpressionWhenCalledAtPath(e4) {
    return 1 === e4.length ? fs(hs, e4[0]) : ce;
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    return 0 === t2.type ? e4.length > 1 : 2 !== t2.type || 1 !== e4.length || ps(hs, e4[0], t2, n2);
  }
}(), os = { value: { hasEffectsWhenCalled: null, returns: rs } }, as = { value: { hasEffectsWhenCalled({ args: e4 }, t2) {
  const n2 = e4[2];
  return e4.length < 3 || "symbol" == typeof n2.getLiteralValueAtPath(Y, te, { deoptimizeCache() {
  } }) && n2.hasEffectsOnInteractionAtPath(Y, pe, t2);
}, returns: rs } }, ls = Zn({ hasOwnProperty: ns, isPrototypeOf: ns, propertyIsEnumerable: ns, toLocaleString: os, toString: os, valueOf: es }), cs = Zn({ valueOf: ns }, ls), us = Zn({ toExponential: os, toFixed: os, toLocaleString: os, toPrecision: os, valueOf: is }, ls), ds = Zn({ exec: es, test: ns }, ls), hs = Zn({ anchor: os, at: es, big: os, blink: os, bold: os, charAt: os, charCodeAt: is, codePointAt: es, concat: os, endsWith: ns, fixed: os, fontcolor: os, fontsize: os, includes: ns, indexOf: is, italics: os, lastIndexOf: is, link: os, localeCompare: is, match: es, matchAll: es, normalize: os, padEnd: os, padStart: os, repeat: os, replace: as, replaceAll: as, search: is, slice: os, small: os, split: es, startsWith: ns, strike: os, sub: os, substr: os, substring: os, sup: os, toLocaleLowerCase: os, toLocaleUpperCase: os, toLowerCase: os, toString: os, toUpperCase: os, trim: os, trimEnd: os, trimLeft: os, trimRight: os, trimStart: os, valueOf: os }, ls);
function ps(e4, t2, n2, s2) {
  var _a3, _b;
...
const ms = { ArrayExpression: ["elements"], ArrayPattern: ["elements"], ArrowFunctionExpression: ["params", "body"], AssignmentExpression: ["left", "right"], AssignmentPattern: ["left", "right"], AwaitExpression: ["argument"], BinaryExpression: ["left", "right"], BlockStatement: ["body"], BreakStatement: ["label"], CallExpression: ["callee", "arguments"], CatchClause: ["param", "body"], ChainExpression: ["expression"], ClassBody: ["body"], ClassDeclaration: ["id", "superClass", "body"], ClassExpression: ["id", "superClass", "body"], ConditionalExpression: ["test", "consequent", "alternate"], ContinueStatement: ["label"], DebuggerStatement: [], DoWhileStatement: ["body", "test"], EmptyStatement: [], ExportAllDeclaration: ["exported", "source", "attributes"], ExportDefaultDeclaration: ["declaration"], ExportNamedDeclaration: ["specifiers", "source", "attributes", "declaration"], ExportSpecifier: ["local", "exported"], ExpressionStatement: ["expression"], ForInStatement: ["left", "right", "body"], ForOfStatement: ["left", "right", "body"], ForStatement: ["init", "test", "update", "body"], FunctionDeclaration: ["id", "params", "body"], FunctionExpression: ["id", "params", "body"], Identifier: [], IfStatement: ["test", "consequent", "alternate"], ImportAttribute: ["key", "value"], ImportDeclaration: ["specifiers", "source", "attributes"], ImportDefaultSpecifier: ["local"], ImportExpression: ["source", "options"], ImportNamespaceSpecifier: ["local"], ImportSpecifier: ["imported", "local"], LabeledStatement: ["label", "body"], Literal: [], LogicalExpression: ["left", "right"], MemberExpression: ["object", "property"], MetaProperty: ["meta", "property"], MethodDefinition: ["key", "value"], NewExpression: ["callee", "arguments"], ObjectExpression: ["properties"], ObjectPattern: ["properties"], PanicError: [], ParseError: [], PrivateIdentifier: [], Program: ["body"], Property: ["key", "value"], PropertyDefinition: ["key", "value"], RestElement: ["argument"], ReturnStatement: ["argument"], SequenceExpression: ["expressions"], SpreadElement: ["argument"], StaticBlock: ["body"], Super: [], SwitchCase: ["test", "consequent"], SwitchStatement: ["discriminant", "cases"], TaggedTemplateExpression: ["tag", "quasi"], TemplateElement: [], TemplateLiteral: ["quasis", "expressions"], ThisExpression: [], ThrowStatement: ["argument"], TryStatement: ["block", "handler", "finalizer"], UnaryExpression: ["argument"], UpdateExpression: ["argument"], VariableDeclaration: ["declarations"], VariableDeclarator: ["id", "init"], WhileStatement: ["test", "body"], YieldExpression: ["argument"] }, gs = "variables";
class ys extends ae {
  get deoptimized() {
  ...
  bind() {
    for (const e4 of ms[this.type]) {
      const t2 = this[e4];
      if (Array.isArray(t2)) for (const e5 of t2) e5 == null ? void 0 : e5.bind();
      else t2 && t2.bind();
    }
  }
  createScope(e4) {
  ...
  hasEffects(e4) {
    ...
    for (const t2 of ms[this.type]) {
      const n2 = this[t2];
      if (null !== n2) {
        if (Array.isArray(n2)) {
          for (const t3 of n2) if (t3 == null ? void 0 : t3.hasEffects(e4)) return true;
        } else if (n2.hasEffects(e4)) return true;
      }
    }
    return false;
  }
  ...
  include(e4, t2, n2) {
    ...
    for (const n3 of ms[this.type]) {
      const s2 = this[n3];
      if (null !== s2) if (Array.isArray(s2)) for (const n4 of s2) n4 == null ? void 0 : n4.include(e4, t2);
      else s2.include(e4, t2);
    }
  }
  ...
  parseNode(e4) {
    var _a3;
    for (const [t2, n2] of Object.entries(e4)) if (!this.hasOwnProperty(t2)) if (95 === t2.charCodeAt(0)) t2 === Gn ? this.annotations = n2 : t2 === Wn && (this.invalidAnnotations = n2);
    else if ("object" != typeof n2 || null === n2) this[t2] = n2;
    else if (Array.isArray(n2)) {
      this[t2] = [];
      for (const e5 of n2) this[t2].push(null === e5 ? null : new (this.scope.context.getNodeConstructor(e5.type))(this, this.scope).parseNode(e5));
    } else this[t2] = new (this.scope.context.getNodeConstructor(n2.type))(this, this.scope).parseNode(n2);
    return ms[_a3 = e4.type] || (ms[_a3] = function(e5) {
      return Object.keys(e5).filter((t2) => "object" == typeof e5[t2] && 95 !== t2.charCodeAt(0));
    }(e4)), this.initialise(), this;
  }
  removeAnnotations(e4) {
    if (this.annotations) for (const t2 of this.annotations) e4.remove(t2.start, t2.end);
  }
  render(e4, t2) {
    for (const n2 of ms[this.type]) {
      const s2 = this[n2];
      if (null !== s2) if (Array.isArray(s2)) for (const n3 of s2) n3 == null ? void 0 : n3.render(e4, t2);
      else s2.render(e4, t2);
    }
  }
  setAssignedValue(e4) {
  ...
  applyDeoptimizations() {
    ...
    for (const e4 of ms[this.type]) {
      const t2 = this[e4];
      if (null !== t2) if (Array.isArray(t2)) for (const e5 of t2) e5 == null ? void 0 : e5.deoptimizePath(J);
      else t2.deoptimizePath(J);
    }
...
class Es extends ae {
  ...
  deoptimizeArgumentsOnInteractionAtPath({ args: e4, type: t2 }, n2) {
    var _a3;
    if (2 === t2 && 0 === n2.length && (this.description.mutatesSelfAsArray && ((_a3 = e4[0]) == null ? void 0 : _a3.deoptimizePath(Z)), this.description.mutatesArgs)) for (let t3 = 1; t3 < e4.length; t3++) e4[t3].deoptimizePath(J);
  }
  ...
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    var _a3, _b;
    const { type: s2 } = t2;
    if (e4.length > (0 === s2 ? 1 : 0)) return true;
    if (2 === s2) {
      const { args: e5 } = t2;
      if (true === this.description.mutatesSelfAsArray && ((_a3 = e5[0]) == null ? void 0 : _a3.hasEffectsOnInteractionAtPath(Z, he, n2))) return true;
      if (this.description.callsArgs) {
        for (const t3 of this.description.callsArgs) if ((_b = e5[t3 + 1]) == null ? void 0 : _b.hasEffectsOnInteractionAtPath(Y, pe, n2)) return true;
      }
    }
    return false;
  }
}
const xs = [new Es({ callsArgs: null, mutatesArgs: false, mutatesSelfAsArray: false, returns: null, returnsPrimitive: ts })], $s = [new Es({ callsArgs: null, mutatesArgs: false, mutatesSelfAsArray: false, returns: null, returnsPrimitive: rs })], As = [new Es({ callsArgs: null, mutatesArgs: false, mutatesSelfAsArray: false, returns: null, returnsPrimitive: ss })], Ss = [new Es({ callsArgs: null, mutatesArgs: false, mutatesSelfAsArray: false, returns: null, returnsPrimitive: le })], ws = /^\d+$/;
class vs extends ae {
  get hasLostTrack() {
  ...
  deoptimizeAllProperties(e4) {
    ...
    if (e4 ? this.hasUnknownDeoptimizedProperty = true : this.hasLostTrack = true, !t2) {
      for (const e5 of [...Object.values(this.propertiesAndGettersByKey), ...Object.values(this.settersByKey)]) for (const t3 of e5) t3.deoptimizePath(J);
  ...
  deoptimizeArgumentsOnInteractionAtPath(e4, t2, n2) {
    ...
    if (this.hasLostTrack || (2 === o2 || t2.length > 1) && (this.hasUnknownDeoptimizedProperty || "string" == typeof s2 && this.deoptimizedPaths[s2])) return void ue(e4);
    ...
    if ("string" == typeof s2) {
      if (a2[s2]) {
        const t3 = l2[s2];
        if (t3) for (const s3 of t3) s3.deoptimizeArgumentsOnInteractionAtPath(e4, i2, n2);
        if (!this.immutable) for (const e5 of r2) e5 && this.additionalExpressionsToBeDeoptimized.add(e5);
        return;
      }
      for (const t3 of c2) t3.deoptimizeArgumentsOnInteractionAtPath(e4, i2, n2);
      if (ws.test(s2)) for (const t3 of this.unknownIntegerProps) t3.deoptimizeArgumentsOnInteractionAtPath(e4, i2, n2);
    } else {
      for (const t3 of [...Object.values(l2), c2]) for (const s3 of t3) s3.deoptimizeArgumentsOnInteractionAtPath(e4, i2, n2);
      for (const t3 of this.unknownIntegerProps) t3.deoptimizeArgumentsOnInteractionAtPath(e4, i2, n2);
    }
    if (!this.immutable) for (const e5 of r2) e5 && this.additionalExpressionsToBeDeoptimized.add(e5);
  ...
  deoptimizeIntegerProperties() {
    if (!(this.hasLostTrack || this.hasUnknownDeoptimizedProperty || this.hasUnknownDeoptimizedInteger)) {
      ...
      for (const [e4, t2] of Object.entries(this.propertiesAndGettersByKey)) if (ws.test(e4)) for (const e5 of t2) e5.deoptimizePath(J);
  ...
  deoptimizePath(e4) {
    ...
    const t2 = e4[0];
    if (1 === e4.length) {
      if (t2 === H) return this.deoptimizeIntegerProperties();
      if ("string" != typeof t2) return this.deoptimizeAllProperties(t2 === q);
      if (!this.deoptimizedPaths[t2]) {
        this.deoptimizedPaths[t2] = true;
        const e5 = this.expressionsToBeDeoptimizedByKey[t2];
        if (e5) for (const t3 of e5) t3.deoptimizeCache();
      }
    }
    const n2 = 1 === e4.length ? J : e4.slice(1);
    for (const e5 of "string" == typeof t2 ? [...this.propertiesAndGettersByKey[t2] || this.unmatchablePropertiesAndGetters, ...this.settersByKey[t2] || this.unmatchableSetters] : this.allProperties) e5.deoptimizePath(n2);
    (_a3 = this.prototypeExpression) == null ? void 0 : _a3.deoptimizePath(1 === e4.length ? [...e4, W] : e4);
  }
  getLiteralValueAtPath(e4, t2, n2) {
    if (0 === e4.length) return oe;
  ...
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    ...
    if ("string" == typeof s2) {
      if (r2[s2]) {
        const e5 = o2[s2];
        if (e5) {
          for (const s3 of e5) if (s3.hasEffectsOnInteractionAtPath(i2, t2, n2)) return true;
        }
        return false;
      }
      for (const e5 of a2) if (e5.hasEffectsOnInteractionAtPath(i2, t2, n2)) return true;
    } else for (const e5 of [...Object.values(o2), a2]) for (const s3 of e5) if (s3.hasEffectsOnInteractionAtPath(i2, t2, n2)) return true;
  ...
  buildPropertyMaps(e4) {
    ...
    for (let d2 = e4.length - 1; d2 >= 0; d2--) {
      ...
      if (t2.push(f2), "string" == typeof h2) "set" === p2 ? s2[h2] || (s2[h2] = [f2, ...u2], i2[h2] = [f2, ...c2]) : "get" === p2 ? n2[h2] || (n2[h2] = [f2, ...a2], r2[h2] = [f2, ...l2]) : (s2[h2] || (s2[h2] = [f2, ...u2]), n2[h2] || (n2[h2] = [f2, ...a2]));
      else {
        if (h2 === H) {
          o2.push(f2);
          continue;
  ...
  deoptimizeCachedEntities() {
    for (const e4 of Object.values(this.expressionsToBeDeoptimizedByKey)) for (const t2 of e4) t2.deoptimizeCache();
    for (const e4 of this.additionalExpressionsToBeDeoptimized) e4.deoptimizePath(J);
  }
  deoptimizeCachedIntegerEntities() {
    for (const [e4, t2] of Object.entries(this.expressionsToBeDeoptimizedByKey)) if (ws.test(e4)) for (const e5 of t2) e5.deoptimizeCache();
    for (const e4 of this.additionalExpressionsToBeDeoptimized) e4.deoptimizePath(Z);
  }
  getMemberExpression(e4) {
    if (this.hasLostTrack || this.hasUnknownDeoptimizedProperty || "string" != typeof e4 || this.hasUnknownDeoptimizedInteger && ws.test(e4) || this.deoptimizedPaths[e4]) return le;
    const t2 = this.propertiesAndGettersByKey[e4];
    return 1 === (t2 == null ? void 0 : t2.length) ? t2[0] : t2 || this.unmatchablePropertiesAndGetters.length > 0 || this.unknownIntegerProps.length > 0 && ws.test(e4) ? le : null;
  }
  getMemberExpressionAndTrackDeopt(e4, t2) {
    if ("string" != typeof e4) return le;
...
const Ps = (e4) => "string" == typeof e4 && /^\d+$/.test(e4), Is = new class extends ae {
  deoptimizeArgumentsOnInteractionAtPath(e4, t2) {
    2 !== e4.type || 1 !== t2.length || Ps(t2[0]) || ue(e4);
  }
  getLiteralValueAtPath(e4) {
    return 1 === e4.length && Ps(e4[0]) ? void 0 : re;
  }
  hasEffectsOnInteractionAtPath(e4, { type: t2 }) {
    return e4.length > 1 || 2 === t2;
  }
}(), ks = new vs({ __proto__: null, hasOwnProperty: xs, isPrototypeOf: xs, propertyIsEnumerable: xs, toLocaleString: $s, toString: $s, valueOf: Ss }, Is, true), Ns = [{ key: H, kind: "init", property: le }, { key: "length", kind: "init", property: ss }], Cs = [new Es({ callsArgs: [0], mutatesArgs: false, mutatesSelfAsArray: "deopt-only", returns: null, returnsPrimitive: ts })], Os = [new Es({ callsArgs: [0], mutatesArgs: false, mutatesSelfAsArray: "deopt-only", returns: null, returnsPrimitive: ss })], Ds = [new Es({ callsArgs: null, mutatesArgs: false, mutatesSelfAsArray: true, returns: () => new vs(Ns, Fs), returnsPrimitive: null })], Ms = [new Es({ callsArgs: null, mutatesArgs: false, mutatesSelfAsArray: "deopt-only", returns: () => new vs(Ns, Fs), returnsPrimitive: null })], Rs = [new Es({ callsArgs: [0], mutatesArgs: false, mutatesSelfAsArray: "deopt-only", returns: () => new vs(Ns, Fs), returnsPrimitive: null })], _s = [new Es({ callsArgs: null, mutatesArgs: true, mutatesSelfAsArray: true, returns: null, returnsPrimitive: ss })], Ls = [new Es({ callsArgs: null, mutatesArgs: false, mutatesSelfAsArray: true, returns: null, returnsPrimitive: le })], Bs = [new Es({ callsArgs: null, mutatesArgs: false, mutatesSelfAsArray: "deopt-only", returns: null, returnsPrimitive: le })], Ts = [new Es({ callsArgs: [0], mutatesArgs: false, mutatesSelfAsArray: "deopt-only", returns: null, returnsPrimitive: le })], zs = [new Es({ callsArgs: null, mutatesArgs: false, mutatesSelfAsArray: true, returns: "self", returnsPrimitive: null })], Vs = [new Es({ callsArgs: [0], mutatesArgs: false, mutatesSelfAsArray: true, returns: "self", returnsPrimitive: null })], Fs = new vs({ __proto__: null, at: Bs, concat: Ms, copyWithin: zs, entries: Ms, every: Cs, fill: zs, filter: Rs, find: Ts, findIndex: Os, findLast: Ts, findLastIndex: Os, flat: Ms, flatMap: Rs, forEach: Ts, includes: xs, indexOf: As, join: $s, keys: Ss, lastIndexOf: As, map: Rs, pop: Ls, push: _s, reduce: Ts, reduceRight: Ts, reverse: zs, shift: Ls, slice: Ms, some: Cs, sort: Vs, splice: Ds, toLocaleString: $s, toString: $s, unshift: _s, values: Bs }, ks, true);
class js extends ys {
  constructor() {
  ...
  applyDeoptimizations() {
    ...
    for (let t2 = 0; t2 < this.elements.length; t2++) {
      ...
      n2 && (e4 || n2 instanceof bs) && (e4 = true, n2.deoptimizePath(J));
    }
    this.scope.context.requestTreeshakingPass();
  }
  getObjectEntity() {
    if (null !== this.objectEntity) return this.objectEntity;
    const e4 = [{ key: "length", kind: "init", property: ss }];
    let t2 = false;
    for (let n2 = 0; n2 < this.elements.length; n2++) {
      const s2 = this.elements[n2];
      t2 || s2 instanceof bs ? s2 && (t2 = true, e4.unshift({ key: H, kind: "init", property: s2 })) : s2 ? e4.push({ key: String(n2), kind: "init", property: s2 }) : e4.push({ key: String(n2), kind: "init", property: Qn });
    }
    return this.objectEntity = new vs(e4, Fs);
  }
}
class Us extends ys {
  addExportedVariables(e4, t2) {
    for (const n2 of this.elements) n2 == null ? void 0 : n2.addExportedVariables(e4, t2);
  }
  declare(e4) {
    const t2 = [];
    for (const n2 of this.elements) null !== n2 && t2.push(...n2.declare(e4, le));
    return t2;
  }
  deoptimizePath() {
    for (const e4 of this.elements) e4 == null ? void 0 : e4.deoptimizePath(Y);
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    for (const e5 of this.elements) if (e5 == null ? void 0 : e5.hasEffectsOnInteractionAtPath(Y, t2, n2)) return true;
    return false;
  }
  markDeclarationReached() {
    for (const e4 of this.elements) e4 == null ? void 0 : e4.markDeclarationReached();
  }
}
function Gs(e4, t2) {
  if ("MemberExpression" === e4.type) return !e4.computed && Gs(e4.object, e4);
...
const Ws = Symbol("PureFunction"), qs = () => {
}, Hs = Symbol("Value Properties"), Ks = () => oe, Ys = () => false, Js = () => true, Xs = { deoptimizeArgumentsOnCall: qs, getLiteralValue: Ks, hasEffectsWhenCalled: Ys }, Zs = { deoptimizeArgumentsOnCall: qs, getLiteralValue: Ks, hasEffectsWhenCalled: Js }, Qs = { deoptimizeArgumentsOnCall: qs, getLiteralValue: Ks, hasEffectsWhenCalled: ({ args: e4 }) => e4.length > 1 && !(e4[1] instanceof js) }, ei = { deoptimizeArgumentsOnCall: qs, getLiteralValue: Ks, hasEffectsWhenCalled({ args: e4 }, t2) {
  const [n2, s2] = e4;
  return !(s2 instanceof ae) || s2.hasEffectsOnInteractionAtPath(J, de, t2);
} }, ti = { __proto__: null, [Hs]: Zs }, ni = { __proto__: null, [Hs]: Xs }, si = { __proto__: null, [Hs]: ei }, ii = { __proto__: null, [Hs]: { deoptimizeArgumentsOnCall({ args: [, e4] }) {
  e4 == null ? void 0 : e4.deoptimizePath(J);
}, getLiteralValue: Ks, hasEffectsWhenCalled: ({ args: e4 }, t2) => e4.length <= 1 || e4[1].hasEffectsOnInteractionAtPath(X, he, t2) } }, ri = { __proto__: null, [Hs]: Zs, prototype: ti }, oi = { __proto__: null, [Hs]: Xs, prototype: ti }, ai = { __proto__: null, [Hs]: Qs, prototype: ti }, li = { __proto__: null, [Hs]: Xs, from: ti, of: ni, prototype: ti }, ci = { __proto__: null, [Hs]: Xs, supportedLocalesOf: oi }, ui = { global: ti, globalThis: ti, self: ti, window: ti, __proto__: null, [Hs]: Zs, Array: { __proto__: null, [Hs]: Zs, from: ti, isArray: ni, of: ni, prototype: ti }, ArrayBuffer: { __proto__: null, [Hs]: Xs, isView: ni, prototype: ti }, Atomics: ti, BigInt: ri, BigInt64Array: ri, BigUint64Array: ri, Boolean: oi, constructor: ri, DataView: oi, Date: { __proto__: null, [Hs]: Xs, now: ni, parse: ni, prototype: ti, UTC: ni }, decodeURI: ni, decodeURIComponent: ni, encodeURI: ni, encodeURIComponent: ni, Error: oi, escape: ni, eval: ti, EvalError: oi, Float32Array: li, Float64Array: li, Function: ri, hasOwnProperty: ti, Infinity: ti, Int16Array: li, Int32Array: li, Int8Array: li, isFinite: ni, isNaN: ni, isPrototypeOf: ti, JSON: ti, Map: ai, Math: { __proto__: null, [Hs]: Zs, abs: ni, acos: ni, acosh: ni, asin: ni, asinh: ni, atan: ni, atan2: ni, atanh: ni, cbrt: ni, ceil: ni, clz32: ni, cos: ni, cosh: ni, exp: ni, expm1: ni, floor: ni, fround: ni, hypot: ni, imul: ni, log: ni, log10: ni, log1p: ni, log2: ni, max: ni, min: ni, pow: ni, random: ni, round: ni, sign: ni, sin: ni, sinh: ni, sqrt: ni, tan: ni, tanh: ni, trunc: ni }, NaN: ti, Number: { __proto__: null, [Hs]: Xs, isFinite: ni, isInteger: ni, isNaN: ni, isSafeInteger: ni, parseFloat: ni, parseInt: ni, prototype: ti }, Object: { __proto__: null, [Hs]: Xs, create: ni, defineProperty: ii, defineProperties: ii, freeze: ii, getOwnPropertyDescriptor: ni, getOwnPropertyDescriptors: ni, getOwnPropertyNames: ni, getOwnPropertySymbols: ni, getPrototypeOf: ni, hasOwn: ni, is: ni, isExtensible: ni, isFrozen: ni, isSealed: ni, keys: ni, fromEntries: ti, entries: si, values: si, prototype: ti }, parseFloat: ni, parseInt: ni, Promise: { __proto__: null, [Hs]: Zs, all: ti, allSettled: ti, any: ti, prototype: ti, race: ti, reject: ti, resolve: ti }, propertyIsEnumerable: ti, Proxy: ti, RangeError: oi, ReferenceError: oi, Reflect: ti, RegExp: oi, Set: ai, SharedArrayBuffer: ri, String: { __proto__: null, [Hs]: Xs, fromCharCode: ni, fromCodePoint: ni, prototype: ti, raw: ni }, Symbol: { __proto__: null, [Hs]: Xs, for: ni, keyFor: ni, prototype: ti, toStringTag: { __proto__: null, [Hs]: { deoptimizeArgumentsOnCall: qs, getLiteralValue: () => K, hasEffectsWhenCalled: Js } } }, SyntaxError: oi, toLocaleString: ti, toString: ti, TypeError: oi, Uint16Array: li, Uint32Array: li, Uint8Array: li, Uint8ClampedArray: li, unescape: ni, URIError: oi, valueOf: ti, WeakMap: ai, WeakSet: ai, clearInterval: ri, clearTimeout: ri, console: { __proto__: null, [Hs]: Zs, assert: ri, clear: ri, count: ri, countReset: ri, debug: ri, dir: ri, dirxml: ri, error: ri, exception: ri, group: ri, groupCollapsed: ri, groupEnd: ri, info: ri, log: ri, table: ri, time: ri, timeEnd: ri, timeLog: ri, trace: ri, warn: ri }, Intl: { __proto__: null, [Hs]: Zs, Collator: ci, DateTimeFormat: ci, DisplayNames: ci, ListFormat: ci, Locale: ci, NumberFormat: ci, PluralRules: ci, RelativeTimeFormat: ci, Segmenter: ci }, setInterval: ri, setTimeout: ri, TextDecoder: ri, TextEncoder: ri, URL: { __proto__: null, [Hs]: Zs, prototype: ti, canParse: ni }, URLSearchParams: ri, AbortController: ri, AbortSignal: ri, addEventListener: ti, alert: ti, AnalyserNode: ri, Animation: ri, AnimationEvent: ri, applicationCache: ti, ApplicationCache: ri, ApplicationCacheErrorEvent: ri, atob: ti, Attr: ri, Audio: ri, AudioBuffer: ri, AudioBufferSourceNode: ri, AudioContext: ri, AudioDestinationNode: ri, AudioListener: ri, AudioNode: ri, AudioParam: ri, AudioProcessingEvent: ri, AudioScheduledSourceNode: ri, AudioWorkletNode: ri, BarProp: ri, BaseAudioContext: ri, BatteryManager: ri, BeforeUnloadEvent: ri, BiquadFilterNode: ri, Blob: ri, BlobEvent: ri, blur: ti, BroadcastChannel: ri, btoa: ti, ByteLengthQueuingStrategy: ri, Cache: ri, caches: ti, CacheStorage: ri, cancelAnimationFrame: ti, cancelIdleCallback: ti, CanvasCaptureMediaStreamTrack: ri, CanvasGradient: ri, CanvasPattern: ri, CanvasRenderingContext2D: ri, ChannelMergerNode: ri, ChannelSplitterNode: ri, CharacterData: ri, clientInformation: ti, ClipboardEvent: ri, close: ti, closed: ti, CloseEvent: ri, Comment: ri, CompositionEvent: ri, confirm: ti, ConstantSourceNode: ri, ConvolverNode: ri, CountQueuingStrategy: ri, createImageBitmap: ti, Credential: ri, CredentialsContainer: ri, crypto: ti, Crypto: ri, CryptoKey: ri, CSS: ri, CSSConditionRule: ri, CSSFontFaceRule: ri, CSSGroupingRule: ri, CSSImportRule: ri, CSSKeyframeRule: ri, CSSKeyframesRule: ri, CSSMediaRule: ri, CSSNamespaceRule: ri, CSSPageRule: ri, CSSRule: ri, CSSRuleList: ri, CSSStyleDeclaration: ri, CSSStyleRule: ri, CSSStyleSheet: ri, CSSSupportsRule: ri, CustomElementRegistry: ri, customElements: ti, CustomEvent: { __proto__: null, [Hs]: { deoptimizeArgumentsOnCall({ args: e4 }) {
  var _a3;
  (_a3 = e4[2]) == null ? void 0 : _a3.deoptimizePath(["detail"]);
}, getLiteralValue: Ks, hasEffectsWhenCalled: Ys }, prototype: ti }, DataTransfer: ri, DataTransferItem: ri, DataTransferItemList: ri, defaultstatus: ti, defaultStatus: ti, DelayNode: ri, DeviceMotionEvent: ri, DeviceOrientationEvent: ri, devicePixelRatio: ti, dispatchEvent: ti, document: ti, Document: ri, DocumentFragment: ri, DocumentType: ri, DOMError: ri, DOMException: ri, DOMImplementation: ri, DOMMatrix: ri, DOMMatrixReadOnly: ri, DOMParser: ri, DOMPoint: ri, DOMPointReadOnly: ri, DOMQuad: ri, DOMRect: ri, DOMRectReadOnly: ri, DOMStringList: ri, DOMStringMap: ri, DOMTokenList: ri, DragEvent: ri, DynamicsCompressorNode: ri, Element: ri, ErrorEvent: ri, Event: ri, EventSource: ri, EventTarget: ri, external: ti, fetch: ti, File: ri, FileList: ri, FileReader: ri, find: ti, focus: ti, FocusEvent: ri, FontFace: ri, FontFaceSetLoadEvent: ri, FormData: ri, frames: ti, GainNode: ri, Gamepad: ri, GamepadButton: ri, GamepadEvent: ri, getComputedStyle: ti, getSelection: ti, HashChangeEvent: ri, Headers: ri, history: ti, History: ri, HTMLAllCollection: ri, HTMLAnchorElement: ri, HTMLAreaElement: ri, HTMLAudioElement: ri, HTMLBaseElement: ri, HTMLBodyElement: ri, HTMLBRElement: ri, HTMLButtonElement: ri, HTMLCanvasElement: ri, HTMLCollection: ri, HTMLContentElement: ri, HTMLDataElement: ri, HTMLDataListElement: ri, HTMLDetailsElement: ri, HTMLDialogElement: ri, HTMLDirectoryElement: ri, HTMLDivElement: ri, HTMLDListElement: ri, HTMLDocument: ri, HTMLElement: ri, HTMLEmbedElement: ri, HTMLFieldSetElement: ri, HTMLFontElement: ri, HTMLFormControlsCollection: ri, HTMLFormElement: ri, HTMLFrameElement: ri, HTMLFrameSetElement: ri, HTMLHeadElement: ri, HTMLHeadingElement: ri, HTMLHRElement: ri, HTMLHtmlElement: ri, HTMLIFrameElement: ri, HTMLImageElement: ri, HTMLInputElement: ri, HTMLLabelElement: ri, HTMLLegendElement: ri, HTMLLIElement: ri, HTMLLinkElement: ri, HTMLMapElement: ri, HTMLMarqueeElement: ri, HTMLMediaElement: ri, HTMLMenuElement: ri, HTMLMetaElement: ri, HTMLMeterElement: ri, HTMLModElement: ri, HTMLObjectElement: ri, HTMLOListElement: ri, HTMLOptGroupElement: ri, HTMLOptionElement: ri, HTMLOptionsCollection: ri, HTMLOutputElement: ri, HTMLParagraphElement: ri, HTMLParamElement: ri, HTMLPictureElement: ri, HTMLPreElement: ri, HTMLProgressElement: ri, HTMLQuoteElement: ri, HTMLScriptElement: ri, HTMLSelectElement: ri, HTMLShadowElement: ri, HTMLSlotElement: ri, HTMLSourceElement: ri, HTMLSpanElement: ri, HTMLStyleElement: ri, HTMLTableCaptionElement: ri, HTMLTableCellElement: ri, HTMLTableColElement: ri, HTMLTableElement: ri, HTMLTableRowElement: ri, HTMLTableSectionElement: ri, HTMLTemplateElement: ri, HTMLTextAreaElement: ri, HTMLTimeElement: ri, HTMLTitleElement: ri, HTMLTrackElement: ri, HTMLUListElement: ri, HTMLUnknownElement: ri, HTMLVideoElement: ri, IDBCursor: ri, IDBCursorWithValue: ri, IDBDatabase: ri, IDBFactory: ri, IDBIndex: ri, IDBKeyRange: ri, IDBObjectStore: ri, IDBOpenDBRequest: ri, IDBRequest: ri, IDBTransaction: ri, IDBVersionChangeEvent: ri, IdleDeadline: ri, IIRFilterNode: ri, Image: ri, ImageBitmap: ri, ImageBitmapRenderingContext: ri, ImageCapture: ri, ImageData: ri, indexedDB: ti, innerHeight: ti, innerWidth: ti, InputEvent: ri, IntersectionObserver: ri, IntersectionObserverEntry: ri, isSecureContext: ti, KeyboardEvent: ri, KeyframeEffect: ri, length: ti, localStorage: ti, location: ti, Location: ri, locationbar: ti, matchMedia: ti, MediaDeviceInfo: ri, MediaDevices: ri, MediaElementAudioSourceNode: ri, MediaEncryptedEvent: ri, MediaError: ri, MediaKeyMessageEvent: ri, MediaKeySession: ri, MediaKeyStatusMap: ri, MediaKeySystemAccess: ri, MediaList: ri, MediaQueryList: ri, MediaQueryListEvent: ri, MediaRecorder: ri, MediaSettingsRange: ri, MediaSource: ri, MediaStream: ri, MediaStreamAudioDestinationNode: ri, MediaStreamAudioSourceNode: ri, MediaStreamEvent: ri, MediaStreamTrack: ri, MediaStreamTrackEvent: ri, menubar: ti, MessageChannel: ri, MessageEvent: ri, MessagePort: ri, MIDIAccess: ri, MIDIConnectionEvent: ri, MIDIInput: ri, MIDIInputMap: ri, MIDIMessageEvent: ri, MIDIOutput: ri, MIDIOutputMap: ri, MIDIPort: ri, MimeType: ri, MimeTypeArray: ri, MouseEvent: ri, moveBy: ti, moveTo: ti, MutationEvent: ri, MutationObserver: ri, MutationRecord: ri, name: ti, NamedNodeMap: ri, NavigationPreloadManager: ri, navigator: ti, Navigator: ri, NetworkInformation: ri, Node: ri, NodeFilter: ti, NodeIterator: ri, NodeList: ri, Notification: ri, OfflineAudioCompletionEvent: ri, OfflineAudioContext: ri, offscreenBuffering: ti, OffscreenCanvas: ri, open: ti, openDatabase: ti, Option: ri, origin: ti, OscillatorNode: ri, outerHeight: ti, outerWidth: ti, PageTransitionEvent: ri, pageXOffset: ti, pageYOffset: ti, PannerNode: ri, parent: ti, Path2D: ri, PaymentAddress: ri, PaymentRequest: ri, PaymentRequestUpdateEvent: ri, PaymentResponse: ri, performance: ti, Performance: ri, PerformanceEntry: ri, PerformanceLongTaskTiming: ri, PerformanceMark: ri, PerformanceMeasure: ri, PerformanceNavigation: ri, PerformanceNavigationTiming: ri, PerformanceObserver: ri, PerformanceObserverEntryList: ri, PerformancePaintTiming: ri, PerformanceResourceTiming: ri, PerformanceTiming: ri, PeriodicWave: ri, Permissions: ri, PermissionStatus: ri, personalbar: ti, PhotoCapabilities: ri, Plugin: ri, PluginArray: ri, PointerEvent: ri, PopStateEvent: ri, postMessage: ti, Presentation: ri, PresentationAvailability: ri, PresentationConnection: ri, PresentationConnectionAvailableEvent: ri, PresentationConnectionCloseEvent: ri, PresentationConnectionList: ri, PresentationReceiver: ri, PresentationRequest: ri, print: ti, ProcessingInstruction: ri, ProgressEvent: ri, PromiseRejectionEvent: ri, prompt: ti, PushManager: ri, PushSubscription: ri, PushSubscriptionOptions: ri, queueMicrotask: ti, RadioNodeList: ri, Range: ri, ReadableStream: ri, RemotePlayback: ri, removeEventListener: ti, Request: ri, requestAnimationFrame: ti, requestIdleCallback: ti, resizeBy: ti, ResizeObserver: ri, ResizeObserverEntry: ri, resizeTo: ti, Response: ri, RTCCertificate: ri, RTCDataChannel: ri, RTCDataChannelEvent: ri, RTCDtlsTransport: ri, RTCIceCandidate: ri, RTCIceTransport: ri, RTCPeerConnection: ri, RTCPeerConnectionIceEvent: ri, RTCRtpReceiver: ri, RTCRtpSender: ri, RTCSctpTransport: ri, RTCSessionDescription: ri, RTCStatsReport: ri, RTCTrackEvent: ri, screen: ti, Screen: ri, screenLeft: ti, ScreenOrientation: ri, screenTop: ti, screenX: ti, screenY: ti, ScriptProcessorNode: ri, scroll: ti, scrollbars: ti, scrollBy: ti, scrollTo: ti, scrollX: ti, scrollY: ti, SecurityPolicyViolationEvent: ri, Selection: ri, ServiceWorker: ri, ServiceWorkerContainer: ri, ServiceWorkerRegistration: ri, sessionStorage: ti, ShadowRoot: ri, SharedWorker: ri, SourceBuffer: ri, SourceBufferList: ri, speechSynthesis: ti, SpeechSynthesisEvent: ri, SpeechSynthesisUtterance: ri, StaticRange: ri, status: ti, statusbar: ti, StereoPannerNode: ri, stop: ti, Storage: ri, StorageEvent: ri, StorageManager: ri, styleMedia: ti, StyleSheet: ri, StyleSheetList: ri, SubtleCrypto: ri, SVGAElement: ri, SVGAngle: ri, SVGAnimatedAngle: ri, SVGAnimatedBoolean: ri, SVGAnimatedEnumeration: ri, SVGAnimatedInteger: ri, SVGAnimatedLength: ri, SVGAnimatedLengthList: ri, SVGAnimatedNumber: ri, SVGAnimatedNumberList: ri, SVGAnimatedPreserveAspectRatio: ri, SVGAnimatedRect: ri, SVGAnimatedString: ri, SVGAnimatedTransformList: ri, SVGAnimateElement: ri, SVGAnimateMotionElement: ri, SVGAnimateTransformElement: ri, SVGAnimationElement: ri, SVGCircleElement: ri, SVGClipPathElement: ri, SVGComponentTransferFunctionElement: ri, SVGDefsElement: ri, SVGDescElement: ri, SVGDiscardElement: ri, SVGElement: ri, SVGEllipseElement: ri, SVGFEBlendElement: ri, SVGFEColorMatrixElement: ri, SVGFEComponentTransferElement: ri, SVGFECompositeElement: ri, SVGFEConvolveMatrixElement: ri, SVGFEDiffuseLightingElement: ri, SVGFEDisplacementMapElement: ri, SVGFEDistantLightElement: ri, SVGFEDropShadowElement: ri, SVGFEFloodElement: ri, SVGFEFuncAElement: ri, SVGFEFuncBElement: ri, SVGFEFuncGElement: ri, SVGFEFuncRElement: ri, SVGFEGaussianBlurElement: ri, SVGFEImageElement: ri, SVGFEMergeElement: ri, SVGFEMergeNodeElement: ri, SVGFEMorphologyElement: ri, SVGFEOffsetElement: ri, SVGFEPointLightElement: ri, SVGFESpecularLightingElement: ri, SVGFESpotLightElement: ri, SVGFETileElement: ri, SVGFETurbulenceElement: ri, SVGFilterElement: ri, SVGForeignObjectElement: ri, SVGGElement: ri, SVGGeometryElement: ri, SVGGradientElement: ri, SVGGraphicsElement: ri, SVGImageElement: ri, SVGLength: ri, SVGLengthList: ri, SVGLinearGradientElement: ri, SVGLineElement: ri, SVGMarkerElement: ri, SVGMaskElement: ri, SVGMatrix: ri, SVGMetadataElement: ri, SVGMPathElement: ri, SVGNumber: ri, SVGNumberList: ri, SVGPathElement: ri, SVGPatternElement: ri, SVGPoint: ri, SVGPointList: ri, SVGPolygonElement: ri, SVGPolylineElement: ri, SVGPreserveAspectRatio: ri, SVGRadialGradientElement: ri, SVGRect: ri, SVGRectElement: ri, SVGScriptElement: ri, SVGSetElement: ri, SVGStopElement: ri, SVGStringList: ri, SVGStyleElement: ri, SVGSVGElement: ri, SVGSwitchElement: ri, SVGSymbolElement: ri, SVGTextContentElement: ri, SVGTextElement: ri, SVGTextPathElement: ri, SVGTextPositioningElement: ri, SVGTitleElement: ri, SVGTransform: ri, SVGTransformList: ri, SVGTSpanElement: ri, SVGUnitTypes: ri, SVGUseElement: ri, SVGViewElement: ri, TaskAttributionTiming: ri, Text: ri, TextEvent: ri, TextMetrics: ri, TextTrack: ri, TextTrackCue: ri, TextTrackCueList: ri, TextTrackList: ri, TimeRanges: ri, toolbar: ti, top: ti, Touch: ri, TouchEvent: ri, TouchList: ri, TrackEvent: ri, TransitionEvent: ri, TreeWalker: ri, UIEvent: ri, ValidityState: ri, visualViewport: ti, VisualViewport: ri, VTTCue: ri, WaveShaperNode: ri, WebAssembly: ti, WebGL2RenderingContext: ri, WebGLActiveInfo: ri, WebGLBuffer: ri, WebGLContextEvent: ri, WebGLFramebuffer: ri, WebGLProgram: ri, WebGLQuery: ri, WebGLRenderbuffer: ri, WebGLRenderingContext: ri, WebGLSampler: ri, WebGLShader: ri, WebGLShaderPrecisionFormat: ri, WebGLSync: ri, WebGLTexture: ri, WebGLTransformFeedback: ri, WebGLUniformLocation: ri, WebGLVertexArrayObject: ri, WebSocket: ri, WheelEvent: ri, Window: ri, Worker: ri, WritableStream: ri, XMLDocument: ri, XMLHttpRequest: ri, XMLHttpRequestEventTarget: ri, XMLHttpRequestUpload: ri, XMLSerializer: ri, XPathEvaluator: ri, XPathExpression: ri, XPathResult: ri, XSLTProcessor: ri };
for (const e4 of ["window", "global", "self", "globalThis"]) ui[e4] = ui;
function di(e4) {
  let t2 = ui;
  for (const n2 of e4) {
    if ("string" != typeof n2) return null;
    if (t2 = t2[n2], !t2) return null;
  }
  return t2[Hs];
}
class hi extends xe {
  ...
  deoptimizeArgumentsOnInteractionAtPath(e4, t2, n2) {
    switch (e4.type) {
      case 0:
      case 1:
        return void (di([this.name, ...t2].slice(0, -1)) || super.deoptimizeArgumentsOnInteractionAtPath(e4, t2, n2));
      case 2: {
        const s2 = di([this.name, ...t2]);
        return void (s2 ? s2.deoptimizeArgumentsOnCall(e4) : super.deoptimizeArgumentsOnInteractionAtPath(e4, t2, n2));
      }
    }
  }
  getLiteralValueAtPath(e4, t2, n2) {
    const s2 = di([this.name, ...e4]);
    return s2 ? s2.getLiteralValue() : re;
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    switch (t2.type) {
      case 0:
        return 0 === e4.length ? "undefined" !== this.name && !di([this.name]) : !di([this.name, ...e4].slice(0, -1));
      case 1:
        return true;
      case 2: {
        const s2 = di([this.name, ...e4]);
...
class pi extends xe {
  ...
  consolidateInitializers() {
    if (this.additionalInitializers) {
      for (const e4 of this.additionalInitializers) e4.deoptimizePath(J);
  ...
  deoptimizePath(e4) {
    if (!this.isReassigned && !this.deoptimizationTracker.trackEntityAtPathAndGetIfTracked(e4, this)) if (0 === e4.length) {
      this.markReassigned();
      const e5 = this.expressionsToBeDeoptimized;
      this.expressionsToBeDeoptimized = we;
      for (const t2 of e5) t2.deoptimizeCache();
      this.init.deoptimizePath(J);
    } else this.init.deoptimizePath(e4);
  }
  getLiteralValueAtPath(e4, t2, n2) {
  ...
  include() {
    if (!this.included) {
      ...
      for (const e4 of this.declarations) {
        ...
        let t2 = e4.parent;
        for (; !t2.included && (t2.included = true, t2.type !== Ee); ) t2 = t2.parent;
      }
    }
  }
  includeCallArguments(e4, t2) {
    if (this.isReassigned || e4.includedCallArguments.has(this.init)) for (const n2 of t2) n2.include(e4, false);
    else e4.includedCallArguments.add(this.init), this.init.includeCallArguments(e4, t2), e4.includedCallArguments.delete(this.init);
  }
  markCalledFromTryStatement() {
...
const fi = /* @__PURE__ */ new Set(["class", "const", "let", "var", "using", "await using"]);
class mi extends ys {
  constructor() {
  ...
  bind() {
    !this.variable && Gs(this, this.parent) && (this.variable = this.scope.findVariable(this.name), this.variable.addReference(this), this.isReferenceVariable = true);
  }
  declare(e4, t2) {
    let n2;
  ...
  hasEffects(e4) {
    return this.deoptimized || this.applyDeoptimizations(), !(!this.isPossibleTDZ() || "var" === this.variable.kind) || this.scope.context.options.treeshake.unknownGlobalSideEffects && this.variable instanceof hi && !this.isPureFunction(Y) && this.variable.hasEffectsOnInteractionAtPath(Y, de, e4);
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    switch (t2.type) {
      case 0:
  ...
  isPossibleTDZ() {
    ...
    if (!(this.variable instanceof pi && this.variable.kind && fi.has(this.variable.kind) && this.variable.module === this.scope.context.module)) return this.isTDZAccess = false;
    let t2;
    return this.variable.declarations && 1 === this.variable.declarations.length && (t2 = this.variable.declarations[0]) && this.start < t2.start && gi(this) === gi(t2) || !this.variable.initReached && this.scope.context.module.isExecuted ? this.isTDZAccess = true : this.isTDZAccess = false;
  }
  markDeclarationReached() {
  ...
  applyDeoptimizations() {
    this.deoptimized = true, this.variable instanceof pi && (this.variable.consolidateInitializers(), this.scope.context.requestTreeshakingPass()), this.isReferenceVariable && (this.variable.addUsedPlace(this), this.scope.context.requestTreeshakingPass());
  }
  ...
  isPureFunction(e4) {
    ...
    for (const n2 of e4) {
      if (!t2) return false;
      if (t2[Ws]) return true;
      t2 = t2[n2];
    }
    return t2 == null ? void 0 : t2[Ws];
  }
}
function gi(e4) {
  for (; e4 && !/^Program|Function/.test(e4.type); ) e4 = e4.parent;
  return e4;
}
const yi = we, bi = /* @__PURE__ */ new Set([W]), Ei = new ee(), xi = /* @__PURE__ */ new Set([le]);
class $i extends pi {
  constructor(e4, t2, n2) {
  ...
  addEntityToBeDeoptimized(e4) {
    if (e4 === le) {
      if (!this.entitiesToBeDeoptimized.has(le)) {
        ...
        for (const { interaction: e5 } of this.deoptimizationInteractions) ue(e5);
        this.deoptimizationInteractions = yi;
      }
    } else if (this.deoptimizedFields.has(W)) e4.deoptimizePath(J);
    else if (!this.entitiesToBeDeoptimized.has(e4)) {
      this.entitiesToBeDeoptimized.add(e4);
      for (const t2 of this.deoptimizedFields) e4.deoptimizePath([t2]);
      for (const { interaction: t2, path: n2 } of this.deoptimizationInteractions) e4.deoptimizeArgumentsOnInteractionAtPath(t2, n2, te);
    }
  }
  markReassigned() {
    if (!this.isReassigned) {
      super.markReassigned();
      for (const e4 of this.expressionsUseTheKnownValue) e4.deoptimizeCache();
  ...
  updateKnownValue(e4) {
    ...
    if (null === this.knownValue) return this.knownValue = e4, void (this.knownValueLiteral = e4.getLiteralValueAtPath(Y, te, this));
    if (this.knownValue === e4 || this.knownValue instanceof mi && e4 instanceof mi && this.knownValue.variable === e4.variable) return;
    const t2 = this.knownValueLiteral;
    if ("symbol" == typeof t2) return void this.markReassigned();
  ...
  getLiteralValueAtPath(e4, t2, n2) {
    if (this.isReassigned) return re;
  ...
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    if (this.isReassigned || 1 === t2.type) return super.hasEffectsOnInteractionAtPath(e4, t2, n2);
    return this.getKnownValue().hasEffectsOnInteractionAtPath(e4, t2, n2);
  }
  deoptimizeArgumentsOnInteractionAtPath(e4, t2) {
    if (t2.length >= 2 || this.entitiesToBeDeoptimized.has(le) || this.deoptimizationInteractions.length >= 20 || 1 === t2.length && (this.deoptimizedFields.has(W) || 2 === e4.type && this.deoptimizedFields.has(t2[0]))) ue(e4);
    else if (!this.deoptimizations.trackEntityAtPathAndGetIfTracked(t2, e4.args)) {
      for (const n2 of this.entitiesToBeDeoptimized) n2.deoptimizeArgumentsOnInteractionAtPath(e4, t2, te);
      this.entitiesToBeDeoptimized.has(le) || this.deoptimizationInteractions.push({ interaction: e4, path: t2 });
    }
  }
  deoptimizePath(e4) {
    if (0 === e4.length) return void this.markReassigned();
    if (this.deoptimizedFields.has(W)) return;
    const t2 = e4[0];
    if (!this.deoptimizedFields.has(t2)) {
      this.deoptimizedFields.add(t2);
      for (const e5 of this.entitiesToBeDeoptimized) e5.deoptimizePath([t2]);
      t2 === W && (this.deoptimizationInteractions = yi, this.deoptimizations = Ei, this.deoptimizedFields = bi, this.entitiesToBeDeoptimized = xi);
    }
  }
  getReturnExpressionWhenCalledAtPath(e4) {
...
const Ai = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_$", Si = 64;
function wi(e4) {
  let t2 = "";
  do {
    const n2 = e4 % Si;
    e4 = e4 / Si | 0, t2 = Ai[n2] + t2;
  } while (0 !== e4);
  return t2;
}
function vi(e4, t2, n2) {
  let s2 = e4, i2 = 1;
  for (; t2.has(s2) || Ie.has(s2) || (n2 == null ? void 0 : n2.has(s2)); ) s2 = `${e4}$${wi(i2++)}`;
  return t2.add(s2), s2;
}
class Pi {
  constructor() {
  ...
  addDeclaration(e4, t2, n2, s2) {
    ...
    if (r2) {
      const o3 = r2.kind;
      if ("var" === s2 && "var" === o3) return r2.addDeclaration(e4, n2), r2;
      t2.error(dn(i2), e4.start);
    }
    const o2 = new pi(e4.name, e4, n2, t2, s2);
...
class Ii extends Pi {
  ...
  addAccessedDynamicImport(e4) {
    (this.accessedDynamicImports || (this.accessedDynamicImports = /* @__PURE__ */ new Set())).add(e4), this.parent instanceof Ii && this.parent.addAccessedDynamicImport(e4);
  }
  addAccessedGlobals(e4, t2) {
    const n2 = t2.get(this) || /* @__PURE__ */ new Set();
    for (const t3 of e4) n2.add(t3);
    t2.set(this, n2), this.parent instanceof Ii && this.parent.addAccessedGlobals(e4, t2);
  }
  ...
  addReturnExpression(e4) {
    this.parent instanceof Ii && this.parent.addReturnExpression(e4);
  }
  addUsedOutsideNames(e4, t2, n2, s2) {
    for (const s3 of this.accessedOutsideVariables.values()) s3.included && (e4.add(s3.getBaseVariableName()), "system" === t2 && n2.has(s3) && e4.add("exports"));
    const i2 = s2.get(this);
    if (i2) for (const t3 of i2) e4.add(t3);
  }
  contains(e4) {
  ...
  deconflict(e4, t2, n2) {
    ...
    if (this.addUsedOutsideNames(s2, e4, t2, n2), this.accessedDynamicImports) for (const e5 of this.accessedDynamicImports) e5.inlineNamespace && s2.add(e5.inlineNamespace.getBaseVariableName());
    for (const [e5, t3] of this.variables) (t3.included || t3.alwaysRendered) && t3.setRenderNames(null, vi(e5, s2, t3.forbiddenNames));
    for (const s3 of this.children) s3.deconflict(e4, t2, n2);
  }
  findLexicalBoundary() {
    ...
    if ("var" === s2) {
      ...
      if (r2) {
        ...
        if ("parameter" === o3 && "CatchClause" === r2.declarations[0].parent.type) {
          const n3 = this.parent.parent.addDeclaration(e4, t2, Qn, s2);
...
class Ni extends Ii {
  ...
  addDeclaration(e4, t2, n2, s2) {
    ...
    if (r2) {
      const o3 = r2.kind;
      if (!("var" !== s2 && "function" !== s2 || "var" !== o3 && "function" !== o3 && "parameter" !== o3)) return r2.addDeclaration(e4, n2), r2;
      t2.error(dn(i2), e4.start);
    }
    const o2 = new pi(e4.name, e4, n2, t2, s2);
    return this.variables.set(i2, o2), o2;
...
class Ci extends Ii {
  constructor(e4, t2) {
    super(e4, e4.context), this.parameters = [], this.hasRest = false, this.bodyScope = t2 ? new ki(this) : new Ni(this);
  }
  addParameterDeclaration(e4) {
    const { name: t2, start: n2 } = e4;
    if (this.variables.get(t2)) return this.context.error(function(e5) {
      return { code: "DUPLICATE_ARGUMENT_NAME", message: `Duplicate argument name "${e5}"` };
    }(t2), n2);
    const s2 = new $i(t2, e4, this.context);
    return this.variables.set(t2, s2), this.bodyScope.addHoistedVariable(t2, s2), s2;
  }
  addParameterVariables(e4, t2) {
    this.parameters = e4;
    for (const t3 of e4) for (const e5 of t3) e5.alwaysRendered = true;
    this.hasRest = t2;
  }
  includeCallArguments(e4, t2) {
    let n2 = false, s2 = false;
    const i2 = this.hasRest && this.parameters[this.parameters.length - 1];
    for (const n3 of t2) if (n3 instanceof bs) {
      for (const n4 of t2) n4.include(e4, false);
      break;
    }
    for (let r2 = t2.length - 1; r2 >= 0; r2--) {
      const o2 = this.parameters[r2] || i2, a2 = t2[r2];
      if (o2) if (n2 = false, 0 === o2.length) s2 = true;
      else for (const e5 of o2) e5.included && (s2 = true), e5.calledFromTryStatement && (n2 = true);
...
class Oi extends Ci {
  ...
  updateReturnExpression() {
    if (1 === this.returnExpressions.length) this.returnExpression = this.returnExpressions[0];
      ...
      for (const e4 of this.returnExpressions) e4.deoptimizePath(J);
    }
  }
}
function Di(e4, t2, n2, s2) {
  t2.remove(n2, s2), e4.removeAnnotations(t2);
}
const Mi = { isNoStatement: true };
function Ri(e4, t2, n2 = 0) {
  let s2, i2;
  for (s2 = e4.indexOf(t2, n2); ; ) {
    if (-1 === (n2 = e4.indexOf("/", n2)) || n2 >= s2) return s2;
...
function Li(e4, t2) {
  _i.lastIndex = t2;
  return _i.exec(e4).index;
}
function Bi(e4) {
  let t2, n2, s2 = 0;
  for (t2 = e4.indexOf("\n", s2); ; ) {
    if (s2 = e4.indexOf("/", s2), -1 === s2 || s2 > t2) return [t2, t2 + 1];
    if (n2 = e4.charCodeAt(s2 + 1), 47 === n2) return [s2, t2 + 1];
    s2 = e4.indexOf("*/", s2 + 2) + 2, s2 > t2 && (t2 = e4.indexOf("\n", s2));
  }
}
function Ti(e4, t2, n2, s2, i2) {
  let r2, o2, a2, l2, c2 = e4[0], u2 = !c2.included || c2.needsBoundaries;
  u2 && (l2 = n2 + Bi(t2.original.slice(n2, c2.start))[1]);
  for (let n3 = 1; n3 <= e4.length; n3++) r2 = c2, o2 = l2, a2 = u2, c2 = e4[n3], u2 = void 0 !== c2 && (!c2.included || c2.needsBoundaries), a2 || u2 ? (l2 = r2.end + Bi(t2.original.slice(r2.end, void 0 === c2 ? s2 : c2.start))[1], r2.included ? a2 ? r2.render(t2, i2, { end: l2, start: o2 }) : r2.render(t2, i2) : Di(r2, t2, o2, l2)) : r2.render(t2, i2);
}
function zi(e4, t2, n2, s2) {
  const i2 = [];
  let r2, o2, a2, l2, c2 = n2 - 1;
  for (const s3 of e4) {
    for (void 0 !== r2 && (c2 = r2.end + Ri(t2.original.slice(r2.end, s3.start), ",")), o2 = a2 = c2 + 1 + Bi(t2.original.slice(c2 + 1, s3.start))[1]; l2 = t2.original.charCodeAt(o2), 32 === l2 || 9 === l2 || 10 === l2 || 13 === l2; ) o2++;
...
function Vi(e4, t2, n2) {
  for (; ; ) {
    const [s2, i2] = Bi(e4.original.slice(t2, n2));
    if (-1 === s2) break;
...
class Fi extends Ii {
  ...
  addDeclaration(e4, t2, n2, s2) {
    ...
    if ("var" === s2) {
      const i2 = e4.name, r2 = ((_a3 = this.hoistedVariables) == null ? void 0 : _a3.get(i2)) || this.variables.get(i2);
      if (r2) return "var" === r2.kind || "var" === s2 && "parameter" === r2.kind ? (r2.addDeclaration(e4, n2), r2) : t2.error(dn(i2), e4.start);
...
class Ui extends ys {
  ...
  createScope(e4) {
    this.scope = this.parent.preventChildBlockScope ? e4 : new Fi(e4);
  }
  hasEffects(e4) {
    if (this.deoptimizeBody) return true;
    for (const t2 of this.body) {
      if (e4.brokenFlow) break;
      if (t2.hasEffects(e4)) return true;
    }
    return false;
  }
  include(e4, t2) {
    if (!this.deoptimizeBody || !this.directlyIncluded) {
      ...
      for (const n2 of this.body) (t2 || n2.shouldBeIncluded(e4)) && n2.include(e4, t2);
    }
  }
  initialise() {
    super.initialise();
    const e4 = this.body[0];
    this.deoptimizeBody = e4 instanceof ji && "use asm" === e4.directive;
  }
  render(e4, t2) {
    this.body.length > 0 ? Ti(this.body, e4, this.start + 1, this.end - 1, t2) : super.render(e4, t2);
  }
}
class Gi extends ys {
  constructor() {
...
class Wi extends ys {
  ...
  updateParameterVariableValues(e4) {
    for (let t2 = 0; t2 < this.params.length; t2++) {
      ...
      if (!(n2 instanceof mi)) continue;
      const s2 = n2.variable, i2 = e4[t2 + 1] ?? Qn;
      s2.updateKnownValue(i2);
    }
  }
  deoptimizeParameterVariableValues() {
    for (const e4 of this.params) if (e4 instanceof mi) {
  ...
  deoptimizeArgumentsOnInteractionAtPath(e4, t2, n2) {
    if (2 === e4.type) {
      ...
      for (let e5 = 0; e5 < n3.length - 1; e5++) {
        ...
        r2 instanceof bs && this.deoptimizeParameterVariableValues(), s2 || i2 instanceof Gi ? (s2 = true, r2.deoptimizePath(J)) : i2 instanceof mi ? (t3[e5][0].addEntityToBeDeoptimized(r2), this.addArgumentToBeDeoptimized(r2)) : i2 ? r2.deoptimizePath(J) : this.addArgumentToBeDeoptimized(r2);
      }
      this.updateParameterVariableValues(n3);
    } else this.getObjectEntity().deoptimizeArgumentsOnInteractionAtPath(e4, t2, n2);
  }
  deoptimizePath(e4) {
    if (this.getObjectEntity().deoptimizePath(e4), 1 === e4.length && e4[0] === W) {
      this.scope.getReturnExpression().deoptimizePath(J);
      for (const e5 of this.scope.parameters) for (const t2 of e5) t2.deoptimizePath(J), t2.markReassigned();
    }
  }
  getLiteralValueAtPath(e4, t2, n2) {
    return this.getObjectEntity().getLiteralValueAtPath(e4, t2, n2);
  ...
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    if (e4.length > 0 || 2 !== t2.type) return this.getObjectEntity().hasEffectsOnInteractionAtPath(e4, t2, n2);
    if (this.annotationNoSideEffects) return false;
    if (this.async) {
      const { propertyReadSideEffects: e5 } = this.scope.context.options.treeshake, t3 = this.scope.getReturnExpression();
      if (t3.hasEffectsOnInteractionAtPath(["then"], pe, n2) || e5 && ("always" === e5 || t3.hasEffectsOnInteractionAtPath(["then"], de, n2))) return true;
    }
    for (const e5 of this.params) if (e5.hasEffects(n2)) return true;
    return false;
  }
  ...
  initialise() {
    super.initialise(), this.body instanceof Ui ? this.body.addImplicitReturnExpressionToScope() : this.scope.addReturnExpression(this.body), this.annotations && this.scope.context.options.treeshake.annotations && (this.annotationNoSideEffects = this.annotations.some((e4) => "noSideEffects" === e4.type));
  }
  parseNode(e4) {
    ...
    return s2.addParameterVariables(o2.map((e5) => e5.declare("parameter", le)), o2[o2.length - 1] instanceof Gi), this.body = new (r2.getNodeConstructor(t2.type))(this, i2).parseNode(t2), super.parseNode(e4);
  }
...
Wi.prototype.preventChildBlockScope = true;
class qi extends Wi {
  constructor() {
    super(...arguments), this.objectEntity = null;
  }
  ...
  createScope(e4) {
    this.scope = new Oi(e4, false);
  }
  hasEffects() {
    return this.deoptimized || this.applyDeoptimizations(), false;
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    if (super.hasEffectsOnInteractionAtPath(e4, t2, n2)) return true;
    if (this.annotationNoSideEffects) return false;
    if (2 === t2.type) {
      const { ignore: e5, brokenFlow: t3 } = n2;
      if (n2.ignore = { breaks: false, continues: false, labels: /* @__PURE__ */ new Set(), returnYield: true, this: false }, this.body.hasEffects(n2)) return true;
  ...
  include(e4, t2) {
    super.include(e4, t2);
    for (const n2 of this.params) n2 instanceof mi || n2.include(e4, t2);
  }
  getObjectEntity() {
    return null !== this.objectEntity ? this.objectEntity : this.objectEntity = new vs([], ks);
  }
}
function Hi(e4, { exportNamesByVariable: t2, snippets: { _: n2, getObject: s2, getPropertyAccess: i2 } }, r2 = "") {
  ...
  {
    const n3 = [];
    for (const s3 of e4) for (const e5 of t2.get(s3)) n3.push([e5, s3.getName(i2) + r2]);
    return `exports(${s2(n3, { lineBreakIndent: null })})`;
  }
}
function Ki(e4, t2, n2, s2, { exportNamesByVariable: i2, snippets: { _: r2 } }) {
  s2.prependRight(t2, `exports(${JSON.stringify(i2.get(e4)[0])},${r2}`), s2.appendLeft(n2, ")");
}
function Yi(e4, t2, n2, s2, i2, r2) {
  const { _: o2, getPropertyAccess: a2 } = r2.snippets;
  i2.appendLeft(n2, `,${o2}${Hi([e4], r2)},${o2}${e4.getName(a2)}`), s2 && (i2.prependRight(t2, "("), i2.appendLeft(n2, ")"));
}
class Ji extends ys {
  addExportedVariables(e4, t2) {
    for (const n2 of this.properties) "Property" === n2.type ? n2.value.addExportedVariables(e4, t2) : n2.argument.addExportedVariables(e4, t2);
  }
  declare(e4, t2) {
    const n2 = [];
    for (const s2 of this.properties) n2.push(...s2.declare(e4, t2));
    return n2;
  }
  deoptimizePath(e4) {
    if (0 === e4.length) for (const t2 of this.properties) t2.deoptimizePath(e4);
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    for (const e5 of this.properties) if (e5.hasEffectsOnInteractionAtPath(Y, t2, n2)) return true;
    return false;
  }
  markDeclarationReached() {
    for (const e4 of this.properties) e4.markDeclarationReached();
  }
}
class Xi extends ys {
  hasEffects(e4) {
  ...
  initialise() {
    if (super.initialise(), this.left instanceof mi) {
  ...
  render(e4, t2, { preventASI: n2, renderedParentType: s2, renderedSurroundingElement: i2 } = Ae) {
    ...
    if (r2.included) r2.render(e4, t2), o2.render(e4, t2);
    else {
      const l3 = Li(e4.original, Ri(e4.original, "=", r2.end) + 1);
      e4.remove(a2, l3), n2 && Vi(e4, l3, o2.start), o2.render(e4, t2, { renderedParentType: s2 || c2.type, renderedSurroundingElement: i2 || c2.type });
    }
    if ("system" === t2.format) if (r2 instanceof mi) {
      const n3 = r2.variable, s3 = t2.exportNamesByVariable.get(n3);
      if (s3) return void (1 === s3.length ? Ki(n3, a2, l2, e4, t2) : Yi(n3, a2, l2, c2.type !== ye, e4, t2));
    } else {
      const n3 = [];
      if (r2.addExportedVariables(n3, t2.exportNamesByVariable), n3.length > 0) return void function(e5, t3, n4, s3, i3, r3) {
        const { _: o3, getDirectReturnIifeLeft: a3 } = r3.snippets;
        i3.prependRight(t3, a3(["v"], `${Hi(e5, r3)},${o3}v`, { needsArrowReturnParens: true, needsWrappedFunction: s3 })), i3.appendLeft(n4, ")");
      }(n3, a2, l2, i2 === ye, e4, t2);
    }
    r2.included && r2 instanceof Ji && (i2 === ye || i2 === fe) && (e4.appendRight(a2, "("), e4.prependLeft(l2, ")"));
  }
  applyDeoptimizations() {
...
class Qi extends pi {
  ...
  include() {
    super.include();
    for (const e4 of this.deoptimizedArguments) e4.deoptimizePath(J);
...
class tr extends Oi {
  constructor(e4) {
    const { context: t2 } = e4;
    super(e4, false), this.variables.set("arguments", this.argumentsVariable = new Qi(t2)), this.variables.set("this", this.thisVariable = new er(t2));
  }
  findLexicalBoundary() {
    return this;
  }
  includeCallArguments(e4, t2) {
    if (super.includeCallArguments(e4, t2), this.argumentsVariable.included) for (const n2 of t2) n2.included || n2.include(e4, false);
  }
}
class nr extends Wi {
  constructor() {
  ...
  createScope(e4) {
    this.scope = new tr(e4), this.constructedEntity = new vs(/* @__PURE__ */ Object.create(null), ks), this.scope.thisVariable.addEntityToBeDeoptimized(this.constructedEntity);
  }
  deoptimizeArgumentsOnInteractionAtPath(e4, t2, n2) {
  ...
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    ...
    if (2 === t2.type) {
      ...
      if (n2.ignore = { breaks: false, continues: false, labels: /* @__PURE__ */ new Set(), returnYield: true, this: t2.withNew }, this.body.hasEffects(n2)) return true;
  ...
  include(e4, t2) {
    ...
    for (const s2 of this.params) s2 instanceof mi && !n2 || s2.include(e4, t2);
  }
  initialise() {
    var _a3;
  ...
  getObjectEntity() {
    return null !== this.objectEntity ? this.objectEntity : this.objectEntity = new vs([{ key: "prototype", kind: "init", property: new vs([], ks) }], ks);
  }
}
class sr extends ys {
  hasEffects() {
  ...
  include(e4, t2) {
    if (this.deoptimized || this.applyDeoptimizations(), !this.included) {
      this.included = true;
      e: if (!this.scope.context.usesTopLevelAwait) {
        let e5 = this.parent;
        do {
          if (e5 instanceof nr || e5 instanceof qi) break e;
        } while (e5 = e5.parent);
...
const ir = { "!=": (e4, t2) => e4 != t2, "!==": (e4, t2) => e4 !== t2, "%": (e4, t2) => e4 % t2, "&": (e4, t2) => e4 & t2, "*": (e4, t2) => e4 * t2, "**": (e4, t2) => e4 ** t2, "+": (e4, t2) => e4 + t2, "-": (e4, t2) => e4 - t2, "/": (e4, t2) => e4 / t2, "<": (e4, t2) => e4 < t2, "<<": (e4, t2) => e4 << t2, "<=": (e4, t2) => e4 <= t2, "==": (e4, t2) => e4 == t2, "===": (e4, t2) => e4 === t2, ">": (e4, t2) => e4 > t2, ">=": (e4, t2) => e4 >= t2, ">>": (e4, t2) => e4 >> t2, ">>>": (e4, t2) => e4 >>> t2, "^": (e4, t2) => e4 ^ t2, "|": (e4, t2) => e4 | t2 };
class rr extends ys {
  deoptimizeCache() {
  }
  getLiteralValueAtPath(e4, t2, n2) {
    if (e4.length > 0) return re;
    const s2 = this.left.getLiteralValueAtPath(Y, t2, n2);
    if ("symbol" == typeof s2) return re;
    const i2 = this.right.getLiteralValueAtPath(Y, t2, n2);
    if ("symbol" == typeof i2) return re;
    const r2 = ir[this.operator];
    return r2 ? r2(s2, i2) : re;
  }
  hasEffects(e4) {
    return "+" === this.operator && this.parent instanceof ji && "" === this.left.getLiteralValueAtPath(Y, te, this) || super.hasEffects(e4);
  }
  hasEffectsOnInteractionAtPath(e4, { type: t2 }) {
    return 0 !== t2 || e4.length > 1;
  }
...
class or extends ys {
  hasEffects(e4) {
    if (this.label) {
      if (!e4.ignore.labels.has(this.label.name)) return true;
      e4.includedLabels.add(this.label.name);
    } else {
      if (!e4.ignore.breaks) return true;
      e4.hasBreak = true;
    }
...
function ar(e4, t2, n2) {
  if (n2.arguments.length > 0) if (n2.arguments[n2.arguments.length - 1].included) for (const s2 of n2.arguments) s2.render(e4, t2);
  else {
    let s2 = n2.arguments.length - 2;
    for (; s2 >= 0 && !n2.arguments[s2].included; ) s2--;
    if (s2 >= 0) {
      for (let i2 = 0; i2 <= s2; i2++) n2.arguments[i2].render(e4, t2);
      e4.remove(Ri(e4.original, ",", n2.arguments[s2].end), n2.end - 1);
    } else e4.remove(Ri(e4.original, "(", n2.callee.end) + 1, n2.end - 1);
  }
}
class lr extends ys {
  ...
  getReturnExpressionWhenCalledAtPath(e4) {
    return 1 !== e4.length ? ce : fs(this.members, e4[0]);
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    switch (t2.type) {
      case 0:
      ...
      case 1:
        return true;
      case 2:
        return !!(this.included && this.value instanceof RegExp && (this.value.global || this.value.sticky)) || (1 !== e4.length || ps(this.members, e4[0], t2, n2));
    }
  }
  initialise() {
    super.initialise(), this.members = function(e4) {
      if (e4 instanceof RegExp) return ds;
      switch (typeof e4) {
        case "boolean":
          return cs;
        case "number":
...
function cr(e4) {
  return e4.computed ? function(e5) {
    if (e5 instanceof lr) return String(e5.value);
    return null;
...
function ur(e4) {
  ...
  if ("string" == typeof t2) {
    if (n2 instanceof mi) return [{ key: n2.name, pos: n2.start }, { key: t2, pos: e4.property.start }];
    if (n2 instanceof dr) {
      const s2 = ur(n2);
...
class dr extends ys {
  ...
  bind() {
    this.bound = true;
    const e4 = ur(this), t2 = e4 && this.scope.findVariable(e4[0].key);
    if (t2 == null ? void 0 : t2.isNamespace) {
      const n2 = hr(t2, e4.slice(1), this.scope.context);
      n2 ? "undefined" === n2 ? this.isUndefined = true : (this.variable = n2, this.scope.addNamespaceMemberAccess(function(e5) {
        let t3 = e5[0].key;
        for (let n3 = 1; n3 < e5.length; n3++) t3 += "." + e5[n3].key;
        return t3;
      }(e4), n2)) : super.bind();
    } else super.bind();
  }
  ...
  deoptimizeCache() {
    ...
    for (const t3 of e4) t3.deoptimizeCache();
  }
  deoptimizePath(e4) {
    if (0 === e4.length && this.disallowNamespaceReassignment(), this.variable) this.variable.deoptimizePath(e4);
  ...
  getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2) {
    return this.variable ? this.variable.getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2) : this.isUndefined ? [Qn, false] : this.propertyKey !== W && e4.length < 7 ? (this.expressionsToBeDeoptimized.push(s2), this.object.getReturnExpressionWhenCalledAtPath([this.getPropertyKey(), ...e4], t2, n2, s2)) : ce;
  }
  hasEffects(e4) {
  ...
  initialise() {
    super.initialise(), this.propertyKey = cr(this), this.accessInteraction = { args: [this.object], type: 0 };
  }
  ...
  render(e4, t2, { renderedParentType: n2, isCalleeOfRenderedParent: s2, renderedSurroundingElement: i2 } = Ae) {
    ...
    } else n2 && s2 && e4.appendRight(this.start, "0, "), this.object.render(e4, t2, { renderedSurroundingElement: i2 }), this.property.render(e4, t2);
  }
  setAssignedValue(e4) {
...
function hr(e4, t2, n2) {
  if (0 === t2.length) return e4;
  if (!e4.isNamespace || e4 instanceof $e) return null;
...
class pr extends ys {
  ...
  deoptimizeArgumentsOnInteractionAtPath(e4, t2, n2) {
    ...
    if (0 !== o2.length) if (i2 === le) for (const e5 of o2) e5.deoptimizePath(J);
    else n2.withTrackedEntityAtPath(t2, i2, () => {
      for (const e5 of o2) this.expressionsToBeDeoptimized.add(e5);
  ...
  deoptimizeCache() {
    ...
    if (((_a3 = this.returnExpression) == null ? void 0 : _a3[0]) !== le) {
      ...
      for (const t3 of e4) t3.deoptimizeCache();
      for (const e5 of t2) e5.deoptimizePath(J);
    }
  }
  deoptimizePath(e4) {
    if (0 === e4.length || this.scope.context.deoptimizationTracker.trackEntityAtPathAndGetIfTracked(e4, this)) return;
  ...
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    ...
    if (2 === s2) {
      ...
      if ((i3 ? n2.instantiated : n2.called).trackEntityAtPathAndGetIfTracked(e4, s3, this)) return false;
    } else if ((1 === s2 ? n2.assigned : n2.accessed).trackEntityAtPathAndGetIfTracked(e4, this)) return false;
...
class fr extends pr {
  ...
  bind() {
    if (super.bind(), this.callee instanceof mi) {
    ...
    var e4;
    this.interaction = { args: [this.callee instanceof dr && !this.callee.variable ? this.callee.object : null, ...this.arguments], type: 2, withNew: false };
  }
  hasEffects(e4) {
    this.deoptimized || this.applyDeoptimizations();
    for (const t2 of this.arguments) if (t2.hasEffects(e4)) return true;
    return !this.annotationPure && (this.callee.hasEffects(e4) || this.callee.hasEffectsOnInteractionAtPath(Y, this.interaction, e4));
  }
  include(e4, t2) {
    this.deoptimized || this.applyDeoptimizations(), t2 ? (super.include(e4, t2), t2 === gs && this.callee instanceof mi && this.callee.variable && this.callee.variable.markCalledFromTryStatement()) : (this.included = true, this.callee.include(e4, false)), this.callee.includeCallArguments(e4, this.arguments);
  }
  initialise() {
  ...
  render(e4, t2, { renderedSurroundingElement: n2 } = Ae) {
    this.callee.render(e4, t2, { isCalleeOfRenderedParent: true, renderedSurroundingElement: n2 }), ar(e4, t2, this);
  }
  applyDeoptimizations() {
...
class mr extends ys {
  createScope(e4) {
    this.scope = new Ci(e4, true);
  }
  parseNode(e4) {
    const { body: t2, param: n2, type: s2 } = e4;
    return this.type = s2, n2 && (this.param = new (this.scope.context.getNodeConstructor(n2.type))(this, this.scope).parseNode(n2), this.param.declare("parameter", le)), this.body = new Ui(this, this.scope.bodyScope).parseNode(t2), super.parseNode(e4);
  }
}
mr.prototype.preventChildBlockScope = true;
class gr extends ys {
  deoptimizeCache() {
  }
  getLiteralValueAtPath(e4, t2, n2) {
    if (!this.expression.isSkippedAsOptional(n2)) return this.expression.getLiteralValueAtPath(e4, t2, n2);
  }
  hasEffects(e4) {
...
class yr extends Ii {
  constructor(e4, t2) {
    const { context: n2 } = e4;
    super(e4, n2), this.variables.set("this", this.thisVariable = new pi("this", null, t2, n2, "other")), this.instanceScope = new Ii(this, n2), this.instanceScope.variables.set("this", new er(n2));
  }
  findLexicalBoundary() {
    return this;
  }
...
class br extends ys {
  createScope(e4) {
    this.scope = new yr(e4, this.parent);
  }
  include(e4, t2) {
    this.included = true, this.scope.context.includeVariableInModule(this.scope.thisVariable);
    for (const n2 of this.body) n2.include(e4, t2);
  }
  parseNode(e4) {
    const t2 = this.body = [];
    for (const n2 of e4.body) t2.push(new (this.scope.context.getNodeConstructor(n2.type))(this, n2.static ? this.scope : this.scope.instanceScope).parseNode(n2));
...
class $r extends ys {
  createScope(e4) {
    this.scope = new Fi(e4);
  }
  hasEffects(e4) {
    for (const t2 of this.body) if (t2.hasEffects(e4)) return true;
    return false;
  }
  include(e4, t2) {
    this.included = true;
  ...
  render(e4, t2) {
    if (this.body.length > 0) {
      const n2 = Ri(e4.original.slice(this.start, this.end), "{") + 1;
      Ti(this.body, e4, this.start + n2, this.end - 1, t2);
    } else super.render(e4, t2);
  }
}
function Ar(e4) {
  return "StaticBlock" === e4.type;
}
class Sr extends ae {
  constructor(e4, t2) {
...
class wr extends ys {
  ...
  createScope(e4) {
    this.scope = new Ii(e4, e4.context);
  }
  deoptimizeArgumentsOnInteractionAtPath(e4, t2, n2) {
    this.getObjectEntity().deoptimizeArgumentsOnInteractionAtPath(e4, t2, n2);
  }
  ...
  initialise() {
    ...
    for (const e4 of this.body.body) if (e4 instanceof xr && "constructor" === e4.kind) return void (this.classConstructor = e4);
    this.classConstructor = null;
  }
  applyDeoptimizations() {
    this.deoptimized = true;
    for (const e4 of this.body.body) Ar(e4) || e4.static || e4 instanceof xr && "constructor" === e4.kind || e4.deoptimizePath(J);
    this.scope.context.requestTreeshakingPass();
  }
  getObjectEntity() {
    if (null !== this.objectEntity) return this.objectEntity;
    ...
    for (const n2 of this.body.body) {
      ...
      if (n2.computed) {
        ...
        o2 = String(e5);
      } else o2 = n2.key instanceof mi ? n2.key.name : String(n2.key.value);
      s2.push({ key: o2, kind: r2, property: n2 });
    }
    return e4.unshift({ key: "prototype", kind: "init", property: new vs(t2, this.superClass ? new Sr(this.superClass, "prototype") : ks) }), this.objectEntity = new vs(e4, this.superClass || ks);
  }
}
class vr extends wr {
  initialise() {
  ...
  parseNode(e4) {
    return null !== e4.id && (this.id = new mi(this, this.scope.parent).parseNode(e4.id)), super.parseNode(e4);
  }
  render(e4, t2) {
    var _a3;
    ...
    if (this.id) {
      ...
      "system" === s2 && n2.has(o2) && e4.appendLeft(this.end, `${i2}${Hi([o2], t2)};`);
      const l2 = o2.getName(r2);
      if (l2 !== a2) return (_a3 = this.superClass) == null ? void 0 : _a3.render(e4, t2), this.body.render(e4, { ...t2, useOriginalName: (e5) => e5 === o2 }), e4.prependRight(this.start, `let ${l2}${i2}=${i2}`), void e4.prependLeft(this.end, ";");
    }
    super.render(e4, t2);
  }
  applyDeoptimizations() {
    ...
    if (e4) {
      ...
      for (const e5 of t2.accessedOutsideVariables.values()) e5 !== s2 && e5.forbidName(n2);
    }
  }
}
class Pr extends wr {
...
class Ir extends ae {
  ...
  deoptimizePath(e4) {
    for (const t2 of this.expressions) t2.deoptimizePath(e4);
  }
  getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2) {
    return [new Ir(this.expressions.map((i2) => i2.getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2)[0])), false];
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    for (const s2 of this.expressions) if (s2.hasEffectsOnInteractionAtPath(e4, t2, n2)) return true;
    return false;
  }
}
class kr extends ys {
  ...
  deoptimizeCache() {
    if (null !== this.usedBranch) {
      ...
      this.expressionsToBeDeoptimized = we;
      for (const e5 of t2) e5.deoptimizeCache();
    }
  }
  deoptimizePath(e4) {
  ...
  getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2) {
    ...
    return i2 ? (this.expressionsToBeDeoptimized.push(s2), i2.getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2)) : [new Ir([this.consequent.getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2)[0], this.alternate.getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2)[0]]), false];
  }
  hasEffects(e4) {
    if (this.test.hasEffects(e4)) return true;
    const t2 = this.getUsedBranch();
  ...
  render(e4, t2, { isCalleeOfRenderedParent: n2, preventASI: s2, renderedParentType: i2, renderedSurroundingElement: r2 } = Ae) {
    ...
    if (this.test.included) this.test.render(e4, t2, { renderedSurroundingElement: r2 }), this.consequent.render(e4, t2), this.alternate.render(e4, t2);
    else {
      const a2 = Ri(e4.original, ":", this.consequent.end), l2 = Li(e4.original, (this.consequent.included ? Ri(e4.original, "?", this.test.end) : a2) + 1);
      s2 && Vi(e4, l2, o2.start), e4.remove(this.start, l2), this.consequent.included && e4.remove(a2, this.end), this.test.removeAnnotations(e4), o2.render(e4, t2, { isCalleeOfRenderedParent: n2, preventASI: true, renderedParentType: i2 || this.parent.type, renderedSurroundingElement: r2 || this.parent.type });
    }
  }
  getUsedBranch() {
...
class Nr extends ys {
  hasEffects(e4) {
    if (this.label) {
      ...
      if (!e4.ignore.continues) return true;
      e4.hasContinue = true;
    }
    return e4.brokenFlow = true, false;
  }
...
class Mr extends ys {
  hasEffects(e4) {
    return !!this.test.hasEffects(e4) || Or(e4, this.body);
  }
  include(e4, t2) {
    this.included = true, this.test.include(e4, t2), Dr(e4, this.body, t2);
  }
}
class Rr extends ys {
  hasEffects() {
...
_r.prototype.needsBoundaries = true;
class Lr extends nr {
  initialise() {
    super.initialise(), null !== this.id && (this.id.variable.isId = true);
  }
...
class Br extends ys {
  ...
  render(e4, t2, n2) {
    const { start: s2, end: i2 } = n2, r2 = function(e5, t3) {
      return Li(e5, Ri(e5, "default", t3) + 7);
    }(e4.original, this.start);
    if (this.declaration instanceof Lr) this.renderNamedDeclaration(e4, r2, null === this.declaration.id ? function(e5, t3) {
      const n3 = Ri(e5, "function", t3) + 8;
      e5 = e5.slice(n3, Ri(e5, "(", n3));
      const s3 = Ri(e5, "*");
      return -1 === s3 ? n3 : n3 + s3 + 1;
    }(e4.original, r2) : null, t2);
    else if (this.declaration instanceof vr) this.renderNamedDeclaration(e4, r2, null === this.declaration.id ? Ri(e4.original, "class", s2) + 5 : null, t2);
    else {
      if (this.variable.getOriginalVariable() !== this.variable) return void Di(this, e4, s2, i2);
      if (!this.variable.included) return e4.remove(this.start, r2), this.declaration.render(e4, t2, { renderedSurroundingElement: ye }), void (";" !== e4.original[this.end - 1] && e4.appendLeft(this.end, ";"));
  ...
  renderNamedDeclaration(e4, t2, n2, s2) {
    ...
    e4.remove(this.start, t2), null !== n2 && e4.appendLeft(n2, ` ${a2}`), "system" === r2 && this.declaration instanceof vr && i2.has(this.variable) && e4.appendLeft(this.end, ` ${Hi([this.variable], s2)};`);
  }
...
class Vr extends ys {
  ...
  hasEffects(e4) {
    ...
    return n2 || this.applyDeoptimizations(), !(!s2.hasEffectsAsAssignmentTarget(e4, false) && !i2.hasEffects(e4)) || Or(e4, t2);
  }
  include(e4, t2) {
    const { body: n2, deoptimized: s2, left: i2, right: r2 } = this;
    s2 || this.applyDeoptimizations(), this.included = true, i2.includeAsAssignmentTarget(e4, t2 || true, false), r2.include(e4, t2), Dr(e4, n2, t2);
  }
  initialise() {
    super.initialise(), this.left.setAssignedValue(le);
  }
  render(e4, t2) {
    this.left.render(e4, t2, Mi), this.right.render(e4, t2, Mi), 110 === e4.original.charCodeAt(this.right.start - 1) && e4.prependLeft(this.right.start, " "), this.body.render(e4, t2);
  }
  applyDeoptimizations() {
...
class Fr extends ys {
  ...
  render(e4, t2) {
    this.left.render(e4, t2, Mi), this.right.render(e4, t2, Mi), 102 === e4.original.charCodeAt(this.right.start - 1) && e4.prependLeft(this.right.start, " "), this.body.render(e4, t2);
  }
  applyDeoptimizations() {
    this.deoptimized = true, this.left.deoptimizePath(Y), this.right.deoptimizePath(J), this.scope.context.requestTreeshakingPass();
  }
...
class jr extends ys {
  ...
  hasEffects(e4) {
    var _a3, _b, _c2;
    return !!(((_a3 = this.init) == null ? void 0 : _a3.hasEffects(e4)) || ((_b = this.test) == null ? void 0 : _b.hasEffects(e4)) || ((_c2 = this.update) == null ? void 0 : _c2.hasEffects(e4))) || Or(e4, this.body);
  }
  include(e4, t2) {
    var _a3, _b, _c2;
    this.included = true, (_a3 = this.init) == null ? void 0 : _a3.include(e4, t2, { asSingleStatement: true }), (_b = this.test) == null ? void 0 : _b.include(e4, t2), (_c2 = this.update) == null ? void 0 : _c2.include(e4, t2), Dr(e4, this.body, t2);
  }
  render(e4, t2) {
    var _a3, _b, _c2;
    (_a3 = this.init) == null ? void 0 : _a3.render(e4, t2, Mi), (_b = this.test) == null ? void 0 : _b.render(e4, t2, Mi), (_c2 = this.update) == null ? void 0 : _c2.render(e4, t2, Mi), this.body.render(e4, t2);
  }
}
class Ur extends nr {
  createScope(e4) {
    super.createScope(this.idScope = new Ii(e4, e4.context));
  }
  parseNode(e4) {
    return null !== e4.id && (this.id = new mi(this, this.idScope).parseNode(e4.id)), super.parseNode(e4);
  }
  onlyFunctionCallUsed() {
...
const Wr = Symbol("unset");
class qr extends ys {
  constructor() {
    super(...arguments), this.testValue = Wr;
  }
  deoptimizeCache() {
    this.testValue = re;
  }
  hasEffects(e4) {
    ...
    if ("symbol" == typeof t2) {
      ...
      if (this.consequent.hasEffects(e4)) return true;
  ...
  include(e4, t2) {
    if (this.included = true, t2) this.includeRecursively(t2, e4);
    else {
  ...
  parseNode(e4) {
    return this.consequent = new (this.scope.context.getNodeConstructor(e4.consequent.type))(this, this.consequentScope = new Gr(this.scope)).parseNode(e4.consequent), e4.alternate && (this.alternate = new (this.scope.context.getNodeConstructor(e4.alternate.type))(this, this.alternateScope = new Gr(this.scope)).parseNode(e4.alternate)), super.parseNode(e4);
  }
  render(e4, t2) {
  ...
  getTestValue() {
    return this.testValue === Wr ? this.testValue = this.test.getLiteralValueAtPath(Y, te, this) : this.testValue;
  }
  ...
  shouldKeepAlternateBranch() {
    let e4 = this.parent;
    do {
      if (e4 instanceof qr && e4.alternate) return true;
      if (e4 instanceof Ui) return false;
      e4 = e4.parent;
    } while (e4);
    return false;
  }
...
Kr.prototype.needsBoundaries = true;
class Yr extends ys {
  applyDeoptimizations() {
  }
}
const Jr = "_interopDefault", Xr = "_interopDefaultCompat", Zr = "_interopNamespace", Qr = "_interopNamespaceCompat", eo = "_interopNamespaceDefault", to = "_interopNamespaceDefaultOnly", no = "_mergeNamespaces", so = "_documentCurrentScript", io = { auto: Jr, compat: Xr, default: null, defaultOnly: null, esModule: null }, ro = (e4, t2) => "esModule" === e4 || t2 && ("auto" === e4 || "compat" === e4), oo = { auto: Zr, compat: Qr, default: eo, defaultOnly: to, esModule: null }, ao = (e4, t2) => "esModule" !== e4 && ro(e4, t2), lo = (e4, t2, n2, s2, i2, r2, o2) => {
  const a2 = new Set(e4);
  for (const e5 of So) t2.has(e5) && a2.add(e5);
  return So.map((e5) => a2.has(e5) ? co[e5](n2, s2, i2, r2, o2, a2) : "").join("");
}, co = { [so]: (e4, { _: t2, n: n2 }) => `var${t2}${so}${t2}=${t2}typeof${t2}document${t2}!==${t2}'undefined'${t2}?${t2}document.currentScript${t2}:${t2}null;${n2}`, [Xr](e4, t2, n2) {
  const { _: s2, getDirectReturnFunction: i2, n: r2 } = t2, [o2, a2] = i2(["e"], { functionReturn: true, lineBreakIndent: null, name: Xr });
  return `${o2}${po(t2)}${s2}?${s2}${n2 ? uo(t2) : ho(t2)}${a2}${r2}${r2}`;
}, [Jr](e4, t2, n2) {
  const { _: s2, getDirectReturnFunction: i2, n: r2 } = t2, [o2, a2] = i2(["e"], { functionReturn: true, lineBreakIndent: null, name: Jr });
  return `${o2}e${s2}&&${s2}e.__esModule${s2}?${s2}${n2 ? uo(t2) : ho(t2)}${a2}${r2}${r2}`;
}, [Qr](e4, t2, n2, s2, i2, r2) {
  const { _: o2, getDirectReturnFunction: a2, n: l2 } = t2;
  if (r2.has(eo)) {
    const [e5, n3] = a2(["e"], { functionReturn: true, lineBreakIndent: null, name: Qr });
    return `${e5}${po(t2)}${o2}?${o2}e${o2}:${o2}${eo}(e)${n3}${l2}${l2}`;
  }
  return `function ${Qr}(e)${o2}{${l2}${e4}if${o2}(${po(t2)})${o2}return e;${l2}` + fo(e4, e4, t2, n2, s2, i2) + `}${l2}${l2}`;
}, [to](e4, t2, n2, s2, i2) {
  const { getDirectReturnFunction: r2, getObject: o2, n: a2, _: l2 } = t2, [c2, u2] = r2(["e"], { functionReturn: true, lineBreakIndent: null, name: to });
  return `${c2}${$o(s2, Ao(i2, o2([[null, `__proto__:${l2}null`], ["default", "e"]], { lineBreakIndent: null }), t2))}${u2}${a2}${a2}`;
}, [eo](e4, t2, n2, s2, i2) {
  const { _: r2, n: o2 } = t2;
  return `function ${eo}(e)${r2}{${o2}` + fo(e4, e4, t2, n2, s2, i2) + `}${o2}${o2}`;
}, [Zr](e4, t2, n2, s2, i2, r2) {
  const { _: o2, getDirectReturnFunction: a2, n: l2 } = t2;
  if (r2.has(eo)) {
    const [e5, t3] = a2(["e"], { functionReturn: true, lineBreakIndent: null, name: Zr });
    return `${e5}e${o2}&&${o2}e.__esModule${o2}?${o2}e${o2}:${o2}${eo}(e)${t3}${l2}${l2}`;
  }
  return `function ${Zr}(e)${o2}{${l2}${e4}if${o2}(e${o2}&&${o2}e.__esModule)${o2}return e;${l2}` + fo(e4, e4, t2, n2, s2, i2) + `}${l2}${l2}`;
}, [no](e4, t2, n2, s2, i2) {
  const { _: r2, cnst: o2, n: a2 } = t2, l2 = "var" === o2 && n2;
  return `function ${no}(n, m)${r2}{${a2}${e4}${go(`{${a2}${e4}${e4}${e4}if${r2}(k${r2}!==${r2}'default'${r2}&&${r2}!(k in n))${r2}{${a2}` + (n2 ? l2 ? bo : Eo : xo)(e4, e4 + e4 + e4 + e4, t2) + `${e4}${e4}${e4}}${a2}${e4}${e4}}`, l2, e4, t2)}${a2}${e4}return ${$o(s2, Ao(i2, "n", t2))};${a2}}${a2}${a2}`;
} }, uo = ({ _: e4, getObject: t2 }) => `e${e4}:${e4}${t2([["default", "e"]], { lineBreakIndent: null })}`, ho = ({ _: e4, getPropertyAccess: t2 }) => `e${t2("default")}${e4}:${e4}e`, po = ({ _: e4 }) => `e${e4}&&${e4}typeof e${e4}===${e4}'object'${e4}&&${e4}'default'${e4}in e`, fo = (e4, t2, n2, s2, i2, r2) => {
  const { _: o2, cnst: a2, getObject: l2, getPropertyAccess: c2, n: u2, s: d2 } = n2, h2 = `{${u2}` + (s2 ? yo : xo)(e4, t2 + e4 + e4, n2) + `${t2}${e4}}`;
  return `${t2}${a2} n${o2}=${o2}Object.create(null${r2 ? `,${o2}{${o2}[Symbol.toStringTag]:${o2}${wo(l2)}${o2}}` : ""});${u2}${t2}if${o2}(e)${o2}{${u2}${t2}${e4}${mo(h2, !s2, n2)}${u2}${t2}}${u2}${t2}n${c2("default")}${o2}=${o2}e;${u2}${t2}return ${$o(i2, "n")}${d2}${u2}`;
}, mo = (e4, t2, { _: n2, cnst: s2, getFunctionIntro: i2, s: r2 }) => "var" !== s2 || t2 ? `for${n2}(${s2} k in e)${n2}${e4}` : `Object.keys(e).forEach(${i2(["k"], { isAsync: false, name: null })}${e4})${r2}`, go = (e4, t2, n2, { _: s2, cnst: i2, getDirectReturnFunction: r2, getFunctionIntro: o2, n: a2 }) => {
  if (t2) {
  ...
  const { _: s2, n: i2 } = n2;
  return `${t2}if${s2}(k${s2}!==${s2}'default')${s2}{${i2}` + bo(e4, t2 + e4, n2) + `${t2}}${i2}`;
...
}, Eo = (e4, t2, { _: n2, cnst: s2, getDirectReturnFunction: i2, n: r2 }) => {
  const [o2, a2] = i2([], { functionReturn: true, lineBreakIndent: null, name: null });
...
}, xo = (e4, t2, { _: n2, n: s2 }) => `${t2}n[k]${n2}=${n2}e[k];${s2}`, $o = (e4, t2) => e4 ? `Object.freeze(${t2})` : t2, Ao = (e4, t2, { _: n2, getObject: s2 }) => e4 ? `Object.defineProperty(${t2},${n2}Symbol.toStringTag,${n2}${wo(s2)})` : t2, So = Object.keys(co);
function wo(e4) {
...
class Po extends ys {
  declareDeclarator(e4, t2) {
    this.isUsingDeclaration = t2, this.id.declare(e4, this.init || Qn);
  }
  deoptimizePath(e4) {
  ...
  render(e4, t2) {
    ...
    if (c2) o2.render(e4, t2);
    else {
      const t3 = Ri(e4.original, "=", o2.end);
      e4.remove(l2, Li(e4.original, t3 + 1));
    }
    if (a2) {
      if (o2 instanceof mi && a2 instanceof Pr && !a2.id) {
        o2.variable.getName(i2) !== o2.name && e4.appendLeft(a2.start + 5, ` ${o2.name}`);
      }
      a2.render(e4, t2, c2 ? Ae : { renderedSurroundingElement: ye });
    } else o2 instanceof mi && vo(o2.variable, n2) && e4.appendLeft(r2, `${s2}=${s2}void 0`);
  }
  applyDeoptimizations() {
    this.deoptimized = true;
    const { id: e4, init: t2 } = this;
    if (t2 && e4 instanceof mi && t2 instanceof Pr && !t2.id) {
      const { name: n2, variable: s2 } = e4;
      for (const e5 of t2.scope.accessedOutsideVariables.values()) e5 !== s2 && e5.forbidName(n2);
    }
  }
}
class Io extends ys {
  ...
  getDeterministicImportedNames() {
    ...
    if (e4 instanceof ji) return we;
    if (e4 instanceof sr) {
      const t2 = e4.parent;
      if (t2 instanceof ji) return we;
      if (t2 instanceof Po) {
        const e5 = t2.id;
        return e5 instanceof Ji ? Co(e5) : void 0;
      }
      if (t2 instanceof dr) {
        const e5 = t2.property;
        if (!t2.computed && e5 instanceof mi) return [e5.name];
      }
    } else if (e4 instanceof dr) {
      const t2 = e4.parent, n2 = e4.property;
      if (!(t2 instanceof fr && n2 instanceof mi)) return;
      const s2 = n2.name;
      if (t2.parent instanceof ji && ["catch", "finally"].includes(s2)) return we;
      if ("then" !== s2) return;
      if (0 === t2.arguments.length) return we;
      const i2 = t2.arguments[0];
      if (1 !== t2.arguments.length || !(i2 instanceof qi || i2 instanceof Ur)) return;
      if (0 === i2.params.length) return we;
      const r2 = i2.params[0];
      return 1 === i2.params.length && r2 instanceof Ji ? Co(r2) : void 0;
    }
  }
  hasEffects() {
    return true;
  ...
  render(e4, t2) {
    ...
    if (this.inlineNamespace) {
      ...
      if (this.mechanism && (e4.overwrite(this.start, Ri(e4.original, "(", this.start + 6) + 1, this.mechanism.left), e4.overwrite(this.end - 1, this.end, this.mechanism.right)), this.resolutionString) {
  ...
  getDynamicImportMechanismAndHelper(e4, t2, { compact: n2, dynamicImportInCjs: s2, format: i2, generatedCode: { arrowFunctions: r2 }, interop: o2 }, { _: a2, getDirectReturnFunction: l2, getDirectReturnIifeLeft: c2 }, u2) {
    ...
    if (d2) return { helper: null, mechanism: d2 };
    const h2 = !this.resolution || "string" == typeof this.resolution;
    switch (i2) {
      case "cjs": {
        if (s2 && (!e4 || "string" == typeof e4 || e4 instanceof En)) return { helper: null, mechanism: null };
        const n3 = ko(e4, t2, o2);
        let i3 = "require(", a3 = ")";
      ...
      case "amd": {
        const s3 = n2 ? "c" : "resolve", i3 = n2 ? "e" : "reject", u3 = ko(e4, t2, o2), [d3, p2] = l2(["m"], { functionReturn: false, lineBreakIndent: null, name: null }), f2 = u3 ? `${d3}${s3}(/*#__PURE__*/${u3}(m))${p2}` : s3, [m2, g2] = l2([s3, i3], { functionReturn: false, lineBreakIndent: null, name: null });
...
function ko(e4, t2, n2) {
  return "external" === t2 ? oo[n2(e4 instanceof En ? e4.id : null)] : "default" === t2 ? to : null;
}
const No = { amd: ["require"], cjs: ["require"], system: ["module"] };
function Co(e4) {
  const t2 = [];
  for (const n2 of e4.properties) {
    if ("RestElement" === n2.type || n2.computed || "Identifier" !== n2.key.type) return;
    t2.push(n2.key.name);
  }
  return t2;
}
...
class Mo extends ys {
  ...
  render(e4, t2) {
    this.label.included ? this.label.render(e4, t2) : e4.remove(this.start, Li(e4.original, Ri(e4.original, ":", this.label.end) + 1)), this.body.render(e4, t2);
  }
}
class Ro extends ys {
  constructor() {
  ...
  deoptimizeCache() {
    if (this.usedBranch) {
      ...
      this.expressionsToBeDeoptimized = we;
      for (const e5 of n2) e5.deoptimizeCache();
  ...
  getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2) {
    const i2 = this.getUsedBranch();
    return i2 ? (this.expressionsToBeDeoptimized.push(s2), i2.getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2)) : [new Ir([this.left.getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2)[0], this.right.getReturnExpressionWhenCalledAtPath(e4, t2, n2, s2)[0]]), false];
  }
  hasEffects(e4) {
  ...
  render(e4, t2, { isCalleeOfRenderedParent: n2, preventASI: s2, renderedParentType: i2, renderedSurroundingElement: r2 } = Ae) {
    if (this.left.included && this.right.included) this.left.render(e4, t2, { preventASI: s2, renderedSurroundingElement: r2 }), this.right.render(e4, t2);
    else {
      const o2 = Ri(e4.original, this.operator, this.left.end);
      if (this.right.included) {
        const t3 = Li(e4.original, o2 + 2);
        e4.remove(this.start, t3), s2 && Vi(e4, t3, this.right.start), this.left.removeAnnotations(e4);
      } else e4.remove(o2, this.end);
...
class Bo extends ys {
  ...
  getReferencedFileName(e4) {
    ...
    return t2 === Lo && (n2 == null ? void 0 : n2.startsWith(_o)) ? e4.getFileName(n2.slice(16)) : null;
  }
  hasEffects() {
    return false;
  }
  ...
  include() {
    if (!this.included && (this.included = true, this.meta.name === Lo)) {
      this.scope.context.addImportMeta(this);
      const e4 = this.parent, t2 = this.metaProperty = e4 instanceof dr && "string" == typeof e4.propertyKey ? e4.propertyKey : null;
      (t2 == null ? void 0 : t2.startsWith(_o)) && (this.referenceId = t2.slice(16));
    }
  }
  render(e4, t2) {
    var _a3;
    ...
    const f2 = c2;
    if (u2) {
      const t3 = s2.getFileName(u2), i3 = P(C(k(f2), t3)), r3 = s2.hookFirstSync("resolveFileUrl", [{ chunkId: f2, fileName: t3, format: n2, moduleId: p2, referenceId: u2, relativePath: i3 }]) || Ho[n2](i3);
    ...
    m2 || (m2 = (_a3 = Ko[n2]) == null ? void 0 : _a3.call(Ko, a2, { chunkId: f2, snippets: i2 }), t2.accessedDocumentCurrentScript || (t2.accessedDocumentCurrentScript = To.includes(n2) && "undefined" !== m2)), "string" == typeof m2 && (l2 instanceof dr ? e4.overwrite(l2.start, l2.end, m2, { contentOnly: true }) : e4.overwrite(d2, h2, m2, { contentOnly: true }));
  }
  setResolution(e4, t2, n2) {
    var _a3;
    this.preliminaryChunkId = n2;
    const s2 = (((_a3 = this.metaProperty) == null ? void 0 : _a3.startsWith(_o)) ? Vo : zo)[e4];
    s2.length > 0 && this.scope.addAccessedGlobals(s2, t2);
  }
}
const To = ["cjs", "iife", "umd"], zo = { amd: ["document", "module", "URL"], cjs: ["document", "require", "URL", so], es: [], iife: ["document", "URL", so], system: ["module"], umd: ["document", "require", "URL", so] }, Vo = { amd: ["document", "require", "URL"], cjs: ["document", "require", "URL"], es: [], iife: ["document", "URL"], system: ["module", "URL"], umd: ["document", "require", "URL"] }, Fo = (e4, t2 = "URL") => `new ${t2}(${e4}).href`, jo = (e4, t2 = false) => Fo(`'${_(e4)}', ${t2 ? "typeof document === 'undefined' ? location.href : " : ""}document.currentScript && document.currentScript.src || document.baseURI`), Uo = (e4) => (t2, { chunkId: n2 }) => {
  const s2 = e4(n2);
  return null === t2 ? `({ url: ${s2} })` : "url" === t2 ? s2 : "undefined";
}, Go = (e4) => `require('u' + 'rl').pathToFileURL(${e4}).href`, Wo = (e4) => Go(`__dirname + '/${_(e4)}'`), qo = (e4, t2 = false) => `${t2 ? "typeof document === 'undefined' ? location.href : " : ""}(${so} && ${so}.src || new URL('${_(e4)}', document.baseURI).href)`, Ho = { amd: (e4) => ("." !== e4[0] && (e4 = "./" + e4), Fo(`require.toUrl('${_(e4)}'), document.baseURI`)), cjs: (e4) => `(typeof document === 'undefined' ? ${Wo(e4)} : ${jo(e4)})`, es: (e4) => Fo(`'${_(e4)}', import.meta.url`), iife: (e4) => jo(e4), system: (e4) => Fo(`'${_(e4)}', module.meta.url`), umd: (e4) => `(typeof document === 'undefined' && typeof location === 'undefined' ? ${Wo(e4)} : ${jo(e4, true)})` }, Ko = { amd: Uo(() => Fo("module.uri, document.baseURI")), cjs: Uo((e4) => `(typeof document === 'undefined' ? ${Go("__filename")} : ${qo(e4)})`), iife: Uo((e4) => qo(e4)), system: (e4, { snippets: { getPropertyAccess: t2 } }) => null === e4 ? "module.meta" : `module.meta${t2(e4)}`, umd: Uo((e4) => `(typeof document === 'undefined' && typeof location === 'undefined' ? ${Go("__filename")} : ${qo(e4, true)})`) };
class Yo extends ys {
  hasEffects(e4) {
    this.deoptimized || this.applyDeoptimizations();
    for (const t2 of this.arguments) if (t2.hasEffects(e4)) return true;
  ...
  render(e4, t2) {
    this.callee.render(e4, t2), ar(e4, t2, this);
  }
  applyDeoptimizations() {
    this.deoptimized = true, this.callee.deoptimizeArgumentsOnInteractionAtPath(this.interaction, Y, te), this.scope.context.requestTreeshakingPass();
  }
  ...
  getObjectEntity() {
    ...
    for (const n2 of this.properties) {
      ...
      if (n2.computed) {
        ...
        s2 = String(e5);
      } else if (s2 = n2.key instanceof mi ? n2.key.name : String(n2.key.value), "__proto__" === s2 && "init" === n2.kind) {
        e4 = n2.value instanceof lr && null === n2.value.value ? null : n2.value;
        continue;
      }
      t2.push({ key: s2, kind: n2.kind, property: n2 });
    }
    return this.objectEntity = new vs(t2, e4);
  }
}
class Xo extends ys {
  initialise() {
...
class ea extends ys {
  ...
  hasEffects(e4) {
    for (const t2 of this.body) if (t2.hasEffects(e4)) {
  ...
  initialise() {
    if (super.initialise(), this.invalidAnnotations) for (const { start: e4, end: t2, type: n2 } of this.invalidAnnotations) this.scope.context.magicString.remove(e4, t2), "pure" !== n2 && "noSideEffects" !== n2 || this.scope.context.log(Le, nn(this.scope.context.code.slice(e4, t2), this.scope.context.module.id, n2), e4);
  }
  render(e4, t2) {
    let n2 = this.start;
    if (e4.original.startsWith("#!") && (n2 = Math.min(e4.original.indexOf("\n") + 1, this.end), e4.remove(0, n2)), this.body.length > 0) {
      for (; "/" === e4.original[n2] && /[*/]/.test(e4.original[n2 + 1]); ) {
        const t3 = Bi(e4.original.slice(n2, this.body[0].start));
        if (-1 === t3[0]) break;
        n2 += t3[1];
      }
      Ti(this.body, e4, n2, this.end, t2);
    } else super.render(e4, t2);
  }
  applyDeoptimizations() {
  }
...
class ia extends ys {
  ...
  hasEffects(e4) {
    for (const t2 of this.expressions) if (t2.hasEffects(e4)) return true;
    return false;
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
  ...
  include(e4, t2) {
    ...
    for (const s2 of this.expressions) (t2 || s2 === n2 && !(this.parent instanceof ji) || s2.shouldBeIncluded(e4)) && s2.include(e4, t2);
  }
  removeAnnotations(e4) {
  ...
  render(e4, t2, { renderedParentType: n2, isCalleeOfRenderedParent: s2, preventASI: i2 } = Ae) {
    ...
    for (const { node: l2, separator: c2, start: u2, end: d2 } of zi(this.expressions, e4, this.start, this.end)) if (l2.included) if (r2++, o2 = c2, 1 === r2 && i2 && Vi(e4, u2, l2.start), 1 === r2) {
...
class oa extends ys {
  hasEffects(e4) {
    var _a3;
    if ((_a3 = this.test) == null ? void 0 : _a3.hasEffects(e4)) return true;
  ...
  include(e4, t2) {
    ...
    for (const n2 of this.consequent) (t2 || n2.shouldBeIncluded(e4)) && n2.include(e4, t2);
  }
  render(e4, t2, n2) {
    if (this.consequent.length > 0) {
      this.test && this.test.render(e4, t2);
      const s2 = this.test ? this.test.end : Ri(e4.original, "default", this.start) + 7, i2 = Ri(e4.original, ":", s2) + 1;
      Ti(this.consequent, e4, i2, n2.end, t2);
    } else super.render(e4, t2);
  }
}
...
class aa extends ys {
  createScope(e4) {
    this.parentScope = e4, this.scope = new Fi(e4);
  }
  hasEffects(e4) {
    if (this.discriminant.hasEffects(e4)) return true;
    const { brokenFlow: t2, hasBreak: n2, ignore: s2 } = e4, { breaks: i2 } = s2;
    s2.breaks = true, e4.hasBreak = false;
    let r2 = true;
    for (const n3 of this.cases) {
      if (n3.hasEffects(e4)) return true;
  ...
  initialise() {
    super.initialise();
    for (let e4 = 0; e4 < this.cases.length; e4++) if (null === this.cases[e4].test) return void (this.defaultCase = e4);
  ...
  render(e4, t2) {
    this.discriminant.render(e4, t2), this.cases.length > 0 && Ti(this.cases, e4, this.cases[0].start, this.end - 1, t2);
  }
}
class la extends pr {
  bind() {
  ...
  hasEffects(e4) {
    this.deoptimized || this.applyDeoptimizations();
    for (const t2 of this.quasi.expressions) if (t2.hasEffects(e4)) return true;
  ...
  initialise() {
    super.initialise(), this.args = [le, ...this.quasi.expressions], this.interaction = { args: [this.tag instanceof dr && !this.tag.variable ? this.tag.object : null, ...this.args], type: 2, withNew: false };
  }
  render(e4, t2) {
...
class ua extends ys {
  ...
  getReturnExpressionWhenCalledAtPath(e4) {
    return 1 !== e4.length ? ce : fs(hs, e4[0]);
  }
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    return 0 === t2.type ? e4.length > 1 : 2 !== t2.type || 1 !== e4.length || ps(hs, e4[0], t2, n2);
  }
...
class ha extends pi {
  constructor(e4, t2, n2) {
    ...
    (s2 instanceof Lr || s2 instanceof vr) && s2.id ? (this.hasId = true, this.originalId = s2.id) : s2 instanceof mi && (this.originalId = s2);
  }
  addReference(e4) {
...
class pa extends Ii {
  constructor(e4, t2) {
    super(e4, t2), this.variables.set("this", new pi("this", null, Qn, t2, "other"));
  }
  addDeclaration(e4, t2, n2, s2) {
  ...
  deconflict(e4, t2, n2) {
    for (const s2 of this.children) s2.deconflict(e4, t2, n2);
  }
  findLexicalBoundary() {
    return this;
  }
  findVariable(e4) {
    ...
    return n2 instanceof hi && this.accessedOutsideVariables.set(e4, n2), n2;
  }
}
class fa extends ys {
  bind() {
...
class ga extends ys {
  ...
  include(e4, t2) {
    ...
    if (this.directlyIncluded && n2) {
      if (this.includedLabelsAfterBlock) for (const e5 of this.includedLabelsAfterBlock) i2.add(e5);
    } else this.included = true, this.directlyIncluded = true, this.block.include(e4, n2 ? gs : t2), i2.size > 0 && (this.includedLabelsAfterBlock = [...i2]), e4.brokenFlow = s2;
...
class ba extends ys {
  ...
  hasEffects(e4) {
    return this.deoptimized || this.applyDeoptimizations(), !("typeof" === this.operator && this.argument instanceof mi) && (this.argument.hasEffects(e4) || "delete" === this.operator && this.argument.hasEffectsOnInteractionAtPath(Y, he, e4));
  }
  hasEffectsOnInteractionAtPath(e4, { type: t2 }) {
...
class Ea extends ys {
  ...
  render(e4, t2) {
    ...
    if (this.argument.render(e4, t2), "system" === s2) {
      ...
      if (r2) if (this.prefix) 1 === r2.length ? Ki(s3, this.start, this.end, e4, t2) : Yi(s3, this.start, this.end, this.parent.type !== ye, e4, t2);
      else {
        const n3 = this.operator[0];
        !function(e5, t3, n4, s4, i3, r3, o2) {
          const { _: a2 } = r3.snippets;
          i3.prependRight(t3, `${Hi([e5], r3, o2)},${a2}`), s4 && (i3.prependRight(t3, "("), i3.appendLeft(n4, ")"));
        }(s3, this.start, this.end, this.parent.type !== ye, e4, t2, `${i2}${n3}${i2}1`);
      }
    }
  }
  applyDeoptimizations() {
    if (this.deoptimized = true, this.argument.deoptimizePath(Y), this.argument instanceof mi) {
...
class xa extends ys {
  deoptimizePath() {
    for (const e4 of this.declarations) e4.deoptimizePath(Y);
  }
  ...
  include(e4, t2, { asSingleStatement: n2 } = Ae) {
    ...
    for (const s2 of this.declarations) {
      ...
      n2 && i2.include(e4, t2), r2 && i2.included && !r2.included && (i2 instanceof Ji || i2 instanceof Us) && r2.include(e4, t2);
    }
  }
  initialise() {
    super.initialise(), this.isUsingDeclaration = "await using" === this.kind || "using" === this.kind;
    for (const e4 of this.declarations) e4.declareDeclarator(this.kind, this.isUsingDeclaration);
  }
  removeAnnotations(e4) {
  ...
  renderReplacedDeclarations(e4, t2) {
    ...
    const d2 = [], h2 = function(e5, t3, n3) {
      ...
      let s3 = null;
      if ("system" === t3.format) {
        for (const { node: i3 } of e5) i3.id instanceof mi && i3.init && 0 === n3.length && 1 === ((_a3 = t3.exportNamesByVariable.get(i3.id.variable)) == null ? void 0 : _a3.length) ? (s3 = i3.id.variable, n3.push(s3)) : i3.id.addExportedVariables(n3, t3.exportNamesByVariable);
        n3.length > 1 ? s3 = null : s3 && (n3.length = 0);
      }
      return s3;
    }(n2, t2, d2);
    for (const { node: d3, start: p2, separator: f2, contentEnd: m2, end: g2 } of n2) if (d3.included) {
      if (d3.render(e4, t2), o2 = "", !d3.id.included || d3.id instanceof mi && vo(d3.id.variable, t2.exportNamesByVariable)) c2 && (u2 += ";"), l2 = false;
      else {
        if (h2 && h2 === d3.id.variable) {
          const n3 = Ri(e4.original, "=", d3.id.end);
          Ki(h2, Li(e4.original, n3 + 1), null === f2 ? m2 : f2, e4, t2);
        }
    ...
    } else e4.remove(p2, g2), d3.removeAnnotations(e4);
...
const Sa = ["PanicError", "ParseError", "ArrayExpression", "ArrayPattern", "ArrowFunctionExpression", "AssignmentExpression", "AssignmentPattern", "AwaitExpression", "BinaryExpression", "BlockStatement", "BreakStatement", "CallExpression", "CatchClause", "ChainExpression", "ClassBody", "ClassDeclaration", "ClassExpression", "ConditionalExpression", "ContinueStatement", "DebuggerStatement", "ExpressionStatement", "DoWhileStatement", "EmptyStatement", "ExportAllDeclaration", "ExportDefaultDeclaration", "ExportNamedDeclaration", "ExportSpecifier", "ExpressionStatement", "ForInStatement", "ForOfStatement", "ForStatement", "FunctionDeclaration", "FunctionExpression", "Identifier", "IfStatement", "ImportAttribute", "ImportDeclaration", "ImportDefaultSpecifier", "ImportExpression", "ImportNamespaceSpecifier", "ImportSpecifier", "LabeledStatement", "Literal", "Literal", "Literal", "Literal", "Literal", "Literal", "LogicalExpression", "MemberExpression", "MetaProperty", "MethodDefinition", "NewExpression", "ObjectExpression", "ObjectPattern", "PrivateIdentifier", "Program", "Property", "PropertyDefinition", "RestElement", "ReturnStatement", "SequenceExpression", "SpreadElement", "StaticBlock", "Super", "SwitchCase", "SwitchStatement", "TaggedTemplateExpression", "TemplateElement", "TemplateLiteral", "ThisExpression", "ThrowStatement", "TryStatement", "UnaryExpression", "UpdateExpression", "VariableDeclaration", "VariableDeclarator", "WhileStatement", "YieldExpression"], wa = [Xo, Zo, js, Us, qi, Xi, Zi, sr, rr, Ui, or, fr, mr, gr, br, vr, Pr, kr, Nr, Cr, ji, Mr, Rr, _r, Br, Tr, zr, ji, Vr, Fr, jr, Lr, Ur, mi, qr, Hr, Kr, Yr, Io, Oo, Do, Mo, lr, lr, lr, lr, lr, lr, Ro, dr, Bo, xr, Yo, Jo, Ji, Qo, ea, ta, na, Gi, sa, ia, bs, $r, ra, oa, aa, la, ca, ua, fa, ma, ga, ba, Ea, xa, Po, $a, Aa], va = [function(e4, t2, n2) {
  e4.message = n2.convertString(n2[t2]);
}, function(e4, t2, n2) {
  e4.message = n2.convertString(n2[t2]);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.elements = Ia(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.elements = Ia(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.async = !(1 & ~i2), e4.expression = !(2 & ~i2), e4.generator = !(4 & ~i2);
  const r2 = e4.annotations = qn(n2[t2 + 1], n2);
  e4.annotationNoSideEffects = r2.some((e5) => "noSideEffects" === e5.type);
  const o2 = e4.params = Ia(e4, s2, n2[t2 + 2], n2);
  s2.addParameterVariables(o2.map((e5) => e5.declare("parameter", le)), o2[o2.length - 1] instanceof Gi), e4.body = Pa(e4, s2.bodyScope, n2[t2 + 3], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.operator = Un[n2[t2]], e4.left = Pa(e4, s2, n2[t2 + 1], n2), e4.right = Pa(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.left = Pa(e4, s2, n2[t2], n2), e4.right = Pa(e4, s2, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.argument = Pa(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.operator = Un[n2[t2]], e4.left = Pa(e4, s2, n2[t2 + 1], n2), e4.right = Pa(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.body = Ia(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.label = 0 === i2 ? null : Pa(e4, s2, i2, n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.optional = !(1 & ~i2), e4.annotations = qn(n2[t2 + 1], n2), e4.callee = Pa(e4, s2, n2[t2 + 2], n2), e4.arguments = Ia(e4, s2, n2[t2 + 3], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2], r2 = e4.param = 0 === i2 ? null : Pa(e4, s2, i2, n2);
  r2 == null ? void 0 : r2.declare("parameter", le), e4.body = Pa(e4, s2.bodyScope, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.expression = Pa(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2], r2 = e4.body = [];
  if (i2) {
    const t3 = n2[i2];
    for (let o2 = 0; o2 < t3; o2++) {
      const t4 = n2[i2 + 1 + o2];
      r2.push(Pa(e4, 1 & n2[t4 + 3] ? s2 : s2.instanceScope, t4, n2));
    }
  }
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.id = 0 === i2 ? null : Pa(e4, s2.parent, i2, n2);
  const r2 = n2[t2 + 1];
  e4.superClass = 0 === r2 ? null : Pa(e4, s2, r2, n2), e4.body = Pa(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.id = 0 === i2 ? null : Pa(e4, s2, i2, n2);
  const r2 = n2[t2 + 1];
  e4.superClass = 0 === r2 ? null : Pa(e4, s2, r2, n2), e4.body = Pa(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.test = Pa(e4, s2, n2[t2], n2), e4.consequent = Pa(e4, s2, n2[t2 + 1], n2), e4.alternate = Pa(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.label = 0 === i2 ? null : Pa(e4, s2, i2, n2);
}, function() {
  ...
  const { scope: s2 } = e4;
  e4.directive = n2.convertString(n2[t2]), e4.expression = Pa(e4, s2, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.body = Pa(e4, s2, n2[t2], n2), e4.test = Pa(e4, s2, n2[t2 + 1], n2);
}, function() {
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.exported = 0 === i2 ? null : Pa(e4, s2, i2, n2), e4.source = Pa(e4, s2, n2[t2 + 1], n2), e4.attributes = Ia(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.declaration = Pa(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.specifiers = Ia(e4, s2, n2[t2], n2);
  const i2 = n2[t2 + 1];
  e4.source = 0 === i2 ? null : Pa(e4, s2, i2, n2), e4.attributes = Ia(e4, s2, n2[t2 + 2], n2);
  const r2 = n2[t2 + 3];
  e4.declaration = 0 === r2 ? null : Pa(e4, s2, r2, n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.local = Pa(e4, s2, n2[t2], n2);
  const i2 = n2[t2 + 1];
  e4.exported = 0 === i2 ? e4.local : Pa(e4, s2, i2, n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.expression = Pa(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.left = Pa(e4, s2, n2[t2], n2), e4.right = Pa(e4, s2, n2[t2 + 1], n2), e4.body = Pa(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.await = !(1 & ~i2), e4.left = Pa(e4, s2, n2[t2 + 1], n2), e4.right = Pa(e4, s2, n2[t2 + 2], n2), e4.body = Pa(e4, s2, n2[t2 + 3], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.init = 0 === i2 ? null : Pa(e4, s2, i2, n2);
  const r2 = n2[t2 + 1];
  e4.test = 0 === r2 ? null : Pa(e4, s2, r2, n2);
  const o2 = n2[t2 + 2];
  e4.update = 0 === o2 ? null : Pa(e4, s2, o2, n2), e4.body = Pa(e4, s2, n2[t2 + 3], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.async = !(1 & ~i2), e4.generator = !(2 & ~i2);
  const r2 = e4.annotations = qn(n2[t2 + 1], n2);
  e4.annotationNoSideEffects = r2.some((e5) => "noSideEffects" === e5.type);
  const o2 = n2[t2 + 2];
  e4.id = 0 === o2 ? null : Pa(e4, s2.parent, o2, n2);
  const a2 = e4.params = Ia(e4, s2, n2[t2 + 3], n2);
  s2.addParameterVariables(a2.map((e5) => e5.declare("parameter", le)), a2[a2.length - 1] instanceof Gi), e4.body = Pa(e4, s2.bodyScope, n2[t2 + 4], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.async = !(1 & ~i2), e4.generator = !(2 & ~i2);
  const r2 = e4.annotations = qn(n2[t2 + 1], n2);
  e4.annotationNoSideEffects = r2.some((e5) => "noSideEffects" === e5.type);
  const o2 = n2[t2 + 2];
  e4.id = 0 === o2 ? null : Pa(e4, e4.idScope, o2, n2);
  const a2 = e4.params = Ia(e4, s2, n2[t2 + 3], n2);
  s2.addParameterVariables(a2.map((e5) => e5.declare("parameter", le)), a2[a2.length - 1] instanceof Gi), e4.body = Pa(e4, s2.bodyScope, n2[t2 + 4], n2);
}, function(e4, t2, n2) {
  e4.name = n2.convertString(n2[t2]);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.test = Pa(e4, s2, n2[t2], n2), e4.consequent = Pa(e4, e4.consequentScope = new Gr(s2), n2[t2 + 1], n2);
  const i2 = n2[t2 + 2];
  e4.alternate = 0 === i2 ? null : Pa(e4, e4.alternateScope = new Gr(s2), i2, n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.key = Pa(e4, s2, n2[t2], n2), e4.value = Pa(e4, s2, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.specifiers = Ia(e4, s2, n2[t2], n2), e4.source = Pa(e4, s2, n2[t2 + 1], n2), e4.attributes = Ia(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.local = Pa(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.source = Pa(e4, s2, n2[t2], n2), e4.sourceAstNode = Yn(n2[t2], n2);
  const i2 = n2[t2 + 1];
  e4.options = 0 === i2 ? null : Pa(e4, s2, i2, n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.local = Pa(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.local = Pa(e4, s2, n2[t2 + 1], n2), e4.imported = 0 === i2 ? e4.local : Pa(e4, s2, i2, n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.label = Pa(e4, s2, n2[t2], n2), e4.body = Pa(e4, s2, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const s2 = e4.bigint = n2.convertString(n2[t2]);
  e4.raw = n2.convertString(n2[t2 + 1]), e4.value = BigInt(s2);
}, function(e4, t2, n2) {
  ...
  const s2 = n2[t2];
  e4.raw = 0 === s2 ? void 0 : n2.convertString(s2), e4.value = new DataView(n2.buffer).getFloat64(t2 + 1 << 2, true);
}, function(e4, t2, n2) {
  const s2 = n2.convertString(n2[t2]), i2 = n2.convertString(n2[t2 + 1]);
  e4.raw = `/${i2}/${s2}`, e4.regex = { flags: s2, pattern: i2 }, e4.value = new RegExp(i2, s2);
}, function(e4, t2, n2) {
  e4.value = n2.convertString(n2[t2]);
  const s2 = n2[t2 + 1];
  e4.raw = 0 === s2 ? void 0 : n2.convertString(s2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.operator = Un[n2[t2]], e4.left = Pa(e4, s2, n2[t2 + 1], n2), e4.right = Pa(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.computed = !(1 & ~i2), e4.optional = !(2 & ~i2), e4.object = Pa(e4, s2, n2[t2 + 1], n2), e4.property = Pa(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.meta = Pa(e4, s2, n2[t2], n2), e4.property = Pa(e4, s2, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.static = !(1 & ~i2), e4.computed = !(2 & ~i2), e4.key = Pa(e4, s2, n2[t2 + 1], n2), e4.value = Pa(e4, s2, n2[t2 + 2], n2), e4.kind = Un[n2[t2 + 3]];
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.annotations = qn(n2[t2], n2), e4.callee = Pa(e4, s2, n2[t2 + 1], n2), e4.arguments = Ia(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.properties = Ia(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.properties = Ia(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  ...
  const { scope: s2 } = e4;
  e4.body = Ia(e4, s2, n2[t2], n2), e4.invalidAnnotations = qn(n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.method = !(1 & ~i2), e4.shorthand = !(2 & ~i2), e4.computed = !(4 & ~i2);
  const r2 = n2[t2 + 1];
  e4.value = Pa(e4, s2, n2[t2 + 2], n2), e4.kind = Un[n2[t2 + 3]], e4.key = 0 === r2 ? e4.value : Pa(e4, s2, r2, n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.static = !(1 & ~i2), e4.computed = !(2 & ~i2), e4.key = Pa(e4, s2, n2[t2 + 1], n2);
  const r2 = n2[t2 + 2];
  e4.value = 0 === r2 ? null : Pa(e4, s2, r2, n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.argument = Pa(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.argument = 0 === i2 ? null : Pa(e4, s2, i2, n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.expressions = Ia(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.argument = Pa(e4, s2, n2[t2], n2);
}, function(e4, t2, n2) {
  ...
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.test = 0 === i2 ? null : Pa(e4, s2, i2, n2), e4.consequent = Ia(e4, s2, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.discriminant = Pa(e4, e4.parentScope, n2[t2], n2), e4.cases = Ia(e4, s2, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.tag = Pa(e4, s2, n2[t2], n2), e4.quasi = Pa(e4, s2, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const s2 = n2[t2];
  e4.tail = !(1 & ~s2);
  const i2 = n2[t2 + 1], r2 = 0 === i2 ? void 0 : n2.convertString(i2), o2 = n2.convertString(n2[t2 + 2]);
  e4.value = { cooked: r2, raw: o2 };
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.quasis = Ia(e4, s2, n2[t2], n2), e4.expressions = Ia(e4, s2, n2[t2 + 1], n2);
}, function() {
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.argument = Pa(e4, s2, n2[t2], n2);
  ...
  const { scope: s2 } = e4;
  e4.block = Pa(e4, s2, n2[t2], n2);
  const i2 = n2[t2 + 1];
  e4.handler = 0 === i2 ? null : Pa(e4, s2, i2, n2);
  const r2 = n2[t2 + 2];
  e4.finalizer = 0 === r2 ? null : Pa(e4, s2, r2, n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.operator = Un[n2[t2]], e4.argument = Pa(e4, s2, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.prefix = !(1 & ~i2), e4.operator = Un[n2[t2 + 1]], e4.argument = Pa(e4, s2, n2[t2 + 2], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.kind = Un[n2[t2]], e4.declarations = Ia(e4, s2, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.id = Pa(e4, s2, n2[t2], n2);
  const i2 = n2[t2 + 1];
  e4.init = 0 === i2 ? null : Pa(e4, s2, i2, n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4;
  e4.test = Pa(e4, s2, n2[t2], n2), e4.body = Pa(e4, s2, n2[t2 + 1], n2);
}, function(e4, t2, n2) {
  const { scope: s2 } = e4, i2 = n2[t2];
  e4.delegate = !(1 & ~i2);
  const r2 = n2[t2 + 1];
  e4.argument = 0 === r2 ? null : Pa(e4, s2, r2, n2);
}];
function Pa(e4, t2, n2, s2) {
  const i2 = s2[n2], r2 = wa[i2];
  if (!r2) throw console.trace(), new Error(`Unknown node type: ${i2}`);
  const o2 = new r2(e4, t2);
  return o2.type = Sa[i2], o2.start = s2[n2 + 1], o2.end = s2[n2 + 2], va[i2](o2, n2 + 3, s2), o2.initialise(), o2;
}
function Ia(e4, t2, n2, s2) {
  if (0 === n2) return we;
  const i2 = s2[n2++], r2 = [];
  for (let o2 = 0; o2 < i2; o2++) {
    const i3 = s2[n2++];
    r2.push(i3 ? Pa(e4, t2, i3, s2) : null);
  }
  return r2;
}
const ka = { ArrayExpression: js, ArrayPattern: Us, ArrowFunctionExpression: qi, AssignmentExpression: Xi, AssignmentPattern: Zi, AwaitExpression: sr, BinaryExpression: rr, BlockStatement: Ui, BreakStatement: or, CallExpression: fr, CatchClause: mr, ChainExpression: gr, ClassBody: br, ClassDeclaration: vr, ClassExpression: Pr, ConditionalExpression: kr, ContinueStatement: Nr, DebuggerStatement: Cr, DoWhileStatement: Mr, EmptyStatement: Rr, ExportAllDeclaration: _r, ExportDefaultDeclaration: Br, ExportNamedDeclaration: Tr, ExportSpecifier: zr, ExpressionStatement: ji, ForInStatement: Vr, ForOfStatement: Fr, ForStatement: jr, FunctionDeclaration: Lr, FunctionExpression: Ur, Identifier: mi, IfStatement: qr, ImportAttribute: Hr, ImportDeclaration: Kr, ImportDefaultSpecifier: Yr, ImportExpression: Io, ImportNamespaceSpecifier: Oo, ImportSpecifier: Do, LabeledStatement: Mo, Literal: lr, LogicalExpression: Ro, MemberExpression: dr, MetaProperty: Bo, MethodDefinition: xr, NewExpression: Yo, ObjectExpression: Jo, ObjectPattern: Ji, PanicError: Xo, ParseError: Zo, PrivateIdentifier: Qo, Program: ea, Property: ta, PropertyDefinition: na, RestElement: Gi, ReturnStatement: sa, SequenceExpression: ia, SpreadElement: bs, StaticBlock: $r, Super: ra, SwitchCase: oa, SwitchStatement: aa, TaggedTemplateExpression: la, TemplateElement: ca, TemplateLiteral: ua, ThisExpression: fa, ThrowStatement: ma, TryStatement: ga, UnaryExpression: ba, UnknownNode: class extends ys {
  hasEffects() {
    return true;
  }
  include(e4) {
...
}, UpdateExpression: Ea, VariableDeclaration: xa, VariableDeclarator: Po, WhileStatement: $a, YieldExpression: Aa }, Na = "_missingExportShim";
class Ca extends xe {
  constructor(e4) {
...
class Oa extends xe {
  ...
  getMemberVariables() {
    ...
    for (const n2 of t2) if ("*" !== n2[0] && n2 !== this.module.info.syntheticNamedExports) {
  ...
  hasEffectsOnInteractionAtPath(e4, t2, n2) {
    ...
    if (1 === e4.length && 2 !== s2) return 1 === s2;
    const i2 = e4[0];
    if ("string" != typeof i2) return true;
  ...
  prepare(e4) {
    this.mergedNamespaces.length > 0 && this.module.scope.addAccessedGlobals([no], e4);
  }
  renderBlock(e4) {
    ...
    if (this.mergedNamespaces.length > 0) {
      ...
      f2 = `/*#__PURE__*/${no}(${f2},${o2}[${e5.join(`,${o2}`)}])`;
    } else r2 && (f2 = `/*#__PURE__*/Object.defineProperty(${f2},${o2}Symbol.toStringTag,${o2}${wo(l2)})`), s2 && (f2 = `/*#__PURE__*/Object.freeze(${f2})`);
    return f2 = `${a2} ${this.getName(c2)}${o2}=${o2}${f2};`, "system" === n2 && t2.has(this) && (f2 += `${u2}${Hi([this], e4)};`), f2;
  }
  renderFirst() {
...
function _a2(e4, t2) {
  ...
  if (t2) for (const e5 of t2) e5.missing || _a2(e5);
}
function La(e4) {
  if (!e4) return null;
  if ("string" == typeof e4 && (e4 = JSON.parse(e4)), !e4.mappings) return { mappings: [], names: [], sources: [], version: 3 };
...
const Ta = (e4, { allowReturnOutsideFunction: t2 = false } = {}) => function(e5) {
  const t3 = Yn(0, e5);
  switch (t3.type) {
    case "PanicError":
      return lt(ct(un(t3.message)));
    case "ParseError":
      return lt(ct(un(t3.message, t3.start)));
    default:
      return t3;
  }
}(Xn(Bn(e4, t2))), za = /* @__PURE__ */ new Set(["assert", "with"]);
function Va(e4) {
  var _a3;
  const { scope: { context: t2 }, options: n2, start: s2 } = e4;
  if (!(n2 instanceof Jo)) return n2 && t2.module.log(Le, rn(t2.module.id), s2), Se;
  const i2 = (_a3 = n2.properties.find((e5) => za.has(Fa(e5)))) == null ? void 0 : _a3.value;
  if (!i2) return Se;
  if (!(i2 instanceof Jo)) return t2.module.log(Le, (r2 = t2.module.id, { code: Pt, message: `Rollup could not statically analyze the options argument of a dynamic import in "${B(r2)}". Dynamic import options need to be an object with a nested attributes object.` }), s2), Se;
  var r2;
...
function Ya() {
  const e4 = {};
  for (const [t2, { memory: n2, time: s2, totalMemory: i2 }] of Wa) e4[t2] = [s2, n2, i2];
  return e4;
}
let Ja = qs, Xa = qs;
...
function Qa(e4, t2) {
  if (e4._hasTimer) return e4;
  e4._hasTimer = true;
  for (const n2 of Za) if (n2 in e4) {
    let s2 = `plugin ${t2}`;
...
function el(e4) {
  ...
  for (const e5 of t2) for (const s2 of [...e5.dependencies, ...e5.implicitlyLoadedBefore]) s2 instanceof En || s2.isExecuted || !s2.info.moduleSideEffects && !e5.implicitlyLoadedBefore.has(s2) || n2.has(s2.id) || (s2.isExecuted = true, n2.add(s2.id), t2.push(s2));
}
const tl = { identifier: null, localName: Na };
function nl(e4, t2, n2, s2, i2 = /* @__PURE__ */ new Map()) {
  const r2 = i2.get(t2);
  if (r2) {
    if (r2.has(e4)) return s2 ? [null] : lt((o2 = t2, a2 = e4.id, { code: Et, exporter: a2, message: `"${o2}" cannot be exported from "${B(a2)}" as it is a reexport that references itself.` }));
    r2.add(e4);
  } else i2.set(t2, /* @__PURE__ */ new Set([e4]));
  var o2, a2;
...
function sl(e4, t2) {
  ...
  for (; ; ) {
    const e5 = s2.module;
    if (s2 = s2 instanceof ha ? s2.getDirectOriginalVariable() : s2 instanceof Da ? s2.syntheticNamespace : null, !s2 || i2.has(s2)) break;
    i2.add(s2), n2.add(e5);
    const t3 = e5.sideEffectDependenciesByVariable.get(s2);
    if (t3) for (const e6 of t3) n2.add(e6);
  }
  return n2;
}
class il {
  constructor(e4, t2, n2, s2, i2, r2, o2, a2) {
    ...
    this.info = { ast: null, attributes: a2, code: null, get dynamicallyImportedIdResolutions() {
      ...
      const e5 = { ".": [...h2.keys()] };
      for (const [t3, { source: n3 }] of g2) (e5[n3] ?? (e5[n3] = [])).push(t3);
      for (const t3 of d2) (e5[t3] ?? (e5[t3] = [])).push("*");
      return e5;
    }, get exports() {
  ...
  estimateSize() {
    let e4 = 0;
    for (const t2 of this.ast.body) t2.included && (e4 += t2.end - t2.start);
    return e4;
  }
  getAllExportNames() {
    ...
    for (const e4 of this.exportAllModules) if (e4 instanceof En) this.allExportNames.add(`*${e4.id}`);
    else for (const t2 of e4.getAllExportNames()) "default" !== t2 && this.allExportNames.add(t2);
  ...
  getDependenciesToBeIncluded() {
    ...
    if (this.info.isEntry || this.includedDynamicImporters.length > 0 || this.namespace.included || this.implicitlyLoadedAfter.size > 0) for (const e5 of [...this.getReexports(), ...this.getExports()]) {
    ...
    for (let s2 of n2) {
      ...
      if (n3) for (const e5 of n3) t2.add(e5);
      s2 instanceof Da ? s2 = s2.getBaseVariable() : s2 instanceof ha && (s2 = s2.getOriginalVariable()), e4.add(s2.module);
    }
    if (this.options.treeshake && "no-treeshake" !== this.info.moduleSideEffects) this.addRelevantSideEffectDependencies(this.relevantDependencies, e4, t2);
    else for (const e5 of this.dependencies) this.relevantDependencies.add(e5);
    for (const t3 of e4) this.relevantDependencies.add(t3);
  ...
  getExportNamesByVariable() {
    ...
    for (const t2 of this.getAllExportNames()) {
      ...
      if (n2 instanceof ha && (n2 = n2.getOriginalVariable()), !n2 || !(n2.included || n2 instanceof $e)) continue;
      const s2 = e4.get(n2);
  ...
  getReexports() {
    ...
    for (const t2 of this.exportAllModules) if (t2 instanceof En) e4.add(`*${t2.id}`);
    else for (const n2 of [...t2.getReexports(), ...t2.getExports()]) "default" !== n2 && e4.add(n2);
  ...
  getVariableForExportName(e4, { importerForSideEffects: t2, isExportAllSearch: n2, onlyExplicit: s2, searchedNamesAndModules: i2 } = Se) {
    if ("*" === e4[0]) {
      if (1 === e4.length) return [this.namespace];
    ...
    if (o2) {
      if (o2 === tl) return [this.exportShimVariable];
    ...
    if ("default" !== e4) {
      ...
      if (this.namespaceReexportsByName.set(e4, n3), n3[0]) return n3;
    }
  ...
  includeAllExports(e4) {
    ...
    for (const s2 of this.exports.keys()) if (e4 || s2 !== this.info.syntheticNamedExports) {
      const e5 = this.getVariableForExportName(s2)[0];
      if (!e5) return lt((t2 = s2, n2 = this.id, { binding: t2, code: Ot, exporter: n2, message: `Exported variable "${t2}" is not defined in "${B(n2)}".`, url: Ke(Ye) }));
  ...
  async setSource({ ast: e4, code: t2, customTransformCache: n2, originalCode: s2, originalSourcemap: i2, resolvedIds: r2, sourcemapChain: o2, transformDependencies: a2, transformFiles: l2, ...c2 }) {
    ...
    if (e4) this.ast = new ka[e4.type](d2, this.scope).parseNode(e4), this.info.ast = e4;
    else {
      Xa("generate ast", 3);
      const e5 = await async function(e6, t3, n3) {
        return Bn(e6, t3);
      }(t2, false);
      Ja("generate ast", 3), this.ast = (h2 = e5, p2 = d2, f2 = this.scope, Pa(p2, f2, 0, Xn(h2))), Object.defineProperty(this.info, "ast", { get: () => {
        if (this.graph.astLru.has(u2)) return this.graph.astLru.get(u2);
        {
  ...
  traceVariable(e4, { importerForSideEffects: t2, isExportAllSearch: n2, searchedNamesAndModules: s2 } = Se) {
    ...
    if (r2) {
      const e5 = r2.module;
      if (e5 instanceof il && "*" === r2.name) return e5.namespace;
  ...
  addExport(e4) {
    if (e4 instanceof Br) this.assertUniqueExportName("default", e4.start), this.exports.set("default", { identifier: e4.variable.getAssignedVariableName(), localName: "default" });
    else if (e4 instanceof _r) {
      const t2 = e4.source.value;
      if (this.addSource(t2, e4), e4.exported) {
        const n2 = e4.exported instanceof lr ? e4.exported.value : e4.exported.name;
      ...
      } else this.exportAllSources.add(t2);
    } else if (e4.source instanceof lr) {
      const t2 = e4.source.value;
      this.addSource(t2, e4);
      for (const { exported: n2, local: s2, start: i2 } of e4.specifiers) {
        const e5 = n2 instanceof lr ? n2.value : n2.name;
        this.assertUniqueExportName(e5, i2), this.reexportDescriptions.set(e5, { localName: s2 instanceof lr ? s2.value : s2.name, module: null, source: t2, start: i2 });
      }
    } else if (e4.declaration) {
      const t2 = e4.declaration;
      if (t2 instanceof xa) for (const e5 of t2.declarations) for (const t3 of $n(e5.id)) this.assertUniqueExportName(t3, e5.id.start), this.exports.set(t3, { identifier: null, localName: t3 });
      else {
        const e5 = t2.id.name;
        this.assertUniqueExportName(e5, t2.id.start), this.exports.set(e5, { identifier: null, localName: e5 });
      }
    } else for (const { local: t2, exported: n2 } of e4.specifiers) {
      const e5 = t2.name, s2 = n2 instanceof mi ? n2.name : n2.value;
  ...
  addImport(e4) {
    ...
    for (const n2 of e4.specifiers) {
      ...
      const s2 = n2 instanceof Yr ? "default" : n2 instanceof Oo ? "*" : n2.imported instanceof mi ? n2.imported.name : n2.imported.value;
        ...
        ({ column: i2, line: r2 } = function(e5, t3) {
          ...
          e: for (; n3.length > 0; ) {
            ...
            if (e6) {
              ...
              for (const e7 of n4) if (e7[0] >= t3.column || e7 === s3) {
  ...
  addRelevantSideEffectDependencies(e4, t2, n2) {
    const s2 = /* @__PURE__ */ new Set(), i2 = (r2) => {
      for (const o2 of r2) s2.has(o2) || (s2.add(o2), t2.has(o2) ? e4.add(o2) : (o2.info.moduleSideEffects || n2.has(o2)) && (o2 instanceof En || o2.hasEffects() ? e4.add(o2) : i2(o2.dependencies)));
    };
  ...
  getVariableFromNamespaceReexports(e4, t2, n2) {
    ...
    for (const o3 of this.exportAllModules) {
      if (o3.info.syntheticNamedExports === e4) continue;
  ...
  includeAndGetAdditionalMergedNamespaces() {
    ...
    for (const n2 of [this, ...this.exportAllModules]) if (n2 instanceof En) {
...
function rl(e4, t2, n2) {
  if (e4.module instanceof il && e4.module !== n2) {
    ...
    if (s2.size > 0) {
      const i2 = n2.cycles;
      for (const r2 of i2) if (s2.has(r2)) {
...
function cl(e4, t2, n2, s2, i2, r2, o2, a2, l2 = "return ") {
  ...
  if (!n2) return `${p2}${p2}${l2}${function(e5, t3, n3, s3, i3) {
    if (e5.length > 0) return e5[0].local;
    for (const { defaultVariableName: e6, importPath: r3, isChunk: o3, name: a3, namedExportsMode: l3, namespaceVariableName: c3, reexports: u3 } of t3) if (u3) return ul(a3, u3[0].imported, l3, o3, e6, c3, n3, r3, s3, i3);
  }(e4, t2, s2, o2, h2)};`;
  let m2 = "";
  for (const { defaultVariableName: e5, importPath: i3, isChunk: a3, name: l3, namedExportsMode: d3, namespaceVariableName: f3, reexports: g2 } of t2) if (g2 && n2) {
    for (const t3 of g2) if ("*" !== t3.reexported) {
      ...
      } else "__proto__" === t3.reexported ? m2 += `Object.defineProperty(exports,${c2}"__proto__",${c2}{${p2}${r2}enumerable:${c2}true,${p2}${r2}value:${c2}${n3}${p2}});` : m2 += `exports${h2(t3.reexported)}${c2}=${c2}${n3};`;
    }
  }
  ...
  for (const { name: e5, reexports: s3 } of t2) if (s3 && n2) {
    for (const t3 of s3) if ("*" === t3.reexported) {
...
function ul(e4, t2, n2, s2, i2, r2, o2, a2, l2, c2) {
  if ("default" === t2) {
    if (!s2) {
      const t3 = o2(a2), n3 = io[t3] ? i2 : e4;
      return ro(t3, l2) ? `${n3}${c2("default")}` : n3;
    }
    return n2 ? `${e4}${c2("default")}` : e4;
  }
  return "*" === t2 ? (s2 ? !n2 : oo[o2(a2)]) ? r2 : e4 : `${e4}${c2(t2)}`;
}
function dl(e4) {
...
function hl(e4, t2, n2, { _: s2, getObject: i2 }) {
  if (e4) {
    if (t2) return n2 ? `Object.defineProperties(exports,${s2}${i2([["__esModule", dl(i2)], [null, `[Symbol.toStringTag]:${s2}${wo(i2)}`]], { lineBreakIndent: null })});` : `Object.defineProperty(exports,${s2}'__esModule',${s2}${dl(i2)});`;
    if (n2) return `Object.defineProperty(exports,${s2}Symbol.toStringTag,${s2}${wo(i2)});`;
  }
  return "";
}
...
function fl(e4, t2, n2, s2, i2, r2, o2, a2) {
  ...
  for (const { defaultVariableName: n3, imports: s3, importPath: i3, isChunk: r3, name: o3, namedExportsMode: a3, namespaceVariableName: l3, reexports: c3 } of e4) if (r3) {
    for (const { imported: e5, reexported: t3 } of [...s3 || [], ...c3 || []]) if ("*" === e5 && "*" !== t3) {
      a3 || p2(l3, to, o3);
      break;
    }
  } else {
    ...
    for (const { imported: t3, reexported: i4 } of [...s3 || [], ...c3 || []]) {
      let s4, c4;
      "default" === t3 ? r4 || (r4 = true, n3 !== l3 && (c4 = n3, s4 = io[e5])) : "*" !== t3 || "*" === i4 || a4 || (a4 = true, s4 = oo[e5], c4 = l3), s4 && p2(c4, s4, o3);
    }
  }
  return `${lo(d2, r2, o2, a2, n2, s2, i2)}${h2.length > 0 ? `${h2.join(u2)}${u2}${u2}` : ""}`;
}
function ml(e4, t2) {
...
function xl(e4) {
  ...
  for (; t2--; ) {
    ...
    if (n2 || s2) return e4.slice(0, t2 + 1);
  }
  return [];
}
const $l = ({ dependencies: e4, exports: t2 }) => {
  ...
  n2.add("default");
  for (const { reexports: t3 } of e4) if (t3) for (const e5 of t3) "*" !== e5.reexported && n2.add(e5.reexported);
  return n2;
...
var Nl = { amd: function(e4, { accessedGlobals: t2, dependencies: n2, exports: s2, hasDefaultExport: i2, hasExports: r2, id: o2, indent: a2, intro: l2, isEntryFacade: c2, isModuleFacade: u2, namedExportsMode: d2, log: h2, outro: p2, snippets: f2 }, { amd: m2, esModule: g2, externalLiveBindings: y2, freeze: b2, generatedCode: { symbols: E2 }, interop: x2, reexportProtoFromExternal: $2, strict: A2 }) {
  ...
  const v2 = function(e5, { _: t3, cnst: n3, n: s3 }, i3) {
    let r3 = "", o3 = false;
    for (const { importPath: a3, name: l3, reexports: c3, imports: u3 } of e5) c3 || u3 ? (r3 += i3 && o3 ? "," : `${r3 ? `;${s3}` : ""}${n3} `, o3 = true, r3 += `${l3}${t3}=${t3}require('${a3}')`) : (r3 && (r3 += i3 && !o3 ? "," : `;${s3}`), o3 = false, r3 += `require('${a3}')`);
    if (r3) return `${r3};${s3}${s3}`;
    return "";
    ...
    for (const { importPath: i3, reexports: r3, imports: o3, name: a3, attributes: l3 } of e5) {
      ...
      if (r3 || o3) {
        if (o3) {
          let t4 = null, i4 = null;
          const r4 = [];
          for (const e7 of o3) "default" === e7.imported ? t4 = e7 : "*" === e7.imported ? i4 = e7 : r4.push(e7);
        ...
        if (r3) {
          let t4 = null;
          const i4 = [], l4 = [];
          for (const e7 of r3) "*" === e7.reexported ? t4 = e7 : "*" === e7.imported ? i4.push(e7) : l4.push(e7);
          if (t4 && s3.push(`export${n3}*${n3}from${n3}${e6}`), i4.length > 0) {
            ...
            for (const e7 of i4) s3.push(`export${n3}{${n3}${a3 === e7.reexported ? a3 : `${a3} as ${Re(e7.reexported)}`} };`);
          }
      ...
      } else s3.push(`import${n3}${e6}`);
    }
    return s3;
  }(r2, d2, a2);
  p2.length > 0 && (s2 += p2.join(h2) + h2 + h2), (s2 += lo(null, t2, n2, a2, l2, c2, u2)) && e4.prepend(s2);
  const f2 = function(e5, { _: t3, cnst: n3 }) {
    const s3 = [], i3 = [];
    for (const r3 of e5) r3.expression && s3.push(`${n3} ${r3.local}${t3}=${t3}${r3.expression};`), i3.push(r3.exported === r3.local ? r3.local : `${r3.local} as ${Re(r3.exported)}`);
  ...
  if (x2 && k2 && (Ne(N2 = x2) || ke.test(N2))) return lt(function(e5) {
  ...
  const { _: g2, getFunctionIntro: y2, getNonArrowFunctionIntro: b2, n: E2, s: x2 } = a2, { importBindings: $2, setters: A2, starExcludes: S2 } = function(e5, t3, n3, { _: s3, cnst: i3, getObject: r3, getPropertyAccess: o3, n: a3 }) {
    ...
    for (const { imports: d3, reexports: h3 } of e5) {
      const p3 = [];
      if (d3) for (const e6 of d3) l3.push(e6.local), "*" === e6.imported ? p3.push(`${e6.local}${s3}=${s3}module;`) : p3.push(`${e6.local}${s3}=${s3}module${o3(e6.imported)};`);
      if (h3) {
        const a4 = [];
        let l4 = false;
        for (const { imported: e6, reexported: t4 } of h3) "*" === t4 ? l4 = true : a4.push([t4, "*" === e6 ? "module" : `module${o3(e6)}`]);
        if (a4.length > 1 || l4) if (l4) {
  ...
  e4.prepend(o2 + lo(null, t2, r2, a2, u2, d2, p2) + wl(s2, r2, a2)).append(`${l2}${E2}${E2}` + Pl(s2, r2, a2) + Il(s2, r2, a2)).indent(`${r2}${r2}${r2}`).append(I2).prepend(P2);
  ...
  if (r2 && !x2) return lt({ code: Rt, message: 'You must supply "output.name" for UMD bundles that have exports so that the exports are accessible in environments without a module loader.', url: Ke(ot) });
  yl(u2, n2);
  ...
  if (S2) {
    ...
    let t3;
    if (!c2 && r2) t3 = `${I2} ${e5}${P2}=${P2}${El(x2, R2, A2, `${M2}(${T2.join(`,${P2}`)})`, h2)};`;
    else {
  ...
  } else W2 = `${M2}(${T2.join(`,${P2}`)})`, !c2 && r2 && (W2 = El(x2, R2, A2, W2, h2));
...
function Rl(e4, t2, n2, s2, i2, r2, o2, a2, l2, c2, u2, d2, h2, p2) {
  const f2 = [...e4].reverse();
  for (const e5 of f2) e5.scope.addUsedOutsideNames(s2, i2, d2, h2);
  !function(e5, t3, n3) {
    for (const s3 of t3) {
      for (const t4 of s3.scope.variables.values()) t4.included && !(t4.renderBaseName || t4 instanceof ha && t4.getOriginalVariable() !== t4) && t4.setRenderNames(null, vi(t4.name, e5, t4.forbiddenNames));
      if (n3.has(s3)) {
        const t4 = s3.namespace;
        t4.setRenderNames(null, vi(t4.name, e5, t4.forbiddenNames));
      }
    }
  }(s2, f2, p2), Ml[i2](s2, n2, t2, r2, o2, a2, l2, c2, u2);
  for (const e5 of f2) e5.scope.deconflict(i2, d2, h2);
}
function _l(e4, t2, n2, s2, i2, r2, o2, a2, l2) {
  for (const t3 of n2.dependencies) (i2 || t3 instanceof F) && (t3.variableName = vi(t3.suggestedVariableName, e4, null));
  for (const n3 of t2) {
    const t3 = n3.module, s3 = n3.name;
    n3.isNamespace && (i2 || t3 instanceof En) ? n3.setRenderNames(null, (t3 instanceof En ? a2.get(t3) : o2.get(t3)).variableName) : t3 instanceof En && "default" === s3 ? n3.setRenderNames(null, vi([...t3.exportedVariables].some(([e5, t4]) => "*" === t4 && e5.included) ? t3.suggestedVariableName + "__default" : t3.suggestedVariableName, e4, n3.forbiddenNames)) : n3.setRenderNames(null, vi(Ce(s3), e4, n3.forbiddenNames));
  }
  for (const t3 of l2) t3.setRenderNames(null, vi(t3.name, e4, t3.forbiddenNames));
}
function Ll(e4, t2, { deconflictedDefault: n2, deconflictedNamespace: s2, dependencies: i2 }, r2, o2, a2, l2, c2) {
  for (const t3 of i2) t3.variableName = vi(t3.suggestedVariableName, e4, null);
  for (const t3 of s2) t3.namespaceVariableName = vi(`${t3.suggestedVariableName}__namespace`, e4, null);
  for (const t3 of n2) t3.defaultVariableName = s2.has(t3) && ao(r2(t3.id), a2) ? t3.namespaceVariableName : vi(`${t3.suggestedVariableName}__default`, e4, null);
  for (const e5 of t2) {
    const t3 = e5.module;
    if (t3 instanceof En) {
      const n3 = c2.get(t3), s3 = e5.name;
      if ("default" === s3) {
        const s4 = r2(t3.id), i3 = io[s4] ? n3.defaultVariableName : n3.variableName;
        ro(s4, a2) ? e5.setRenderNames(i3, "default") : e5.setRenderNames(null, i3);
      } else "*" === s3 ? e5.setRenderNames(null, oo[r2(t3.id)] ? n3.namespaceVariableName : n3.variableName) : e5.setRenderNames(n3.variableName, null);
    } else {
      const n3 = l2.get(t3);
...
function Bl(e4, { exports: t2, name: n2, format: s2 }, i2, r2) {
  ...
  if ("default" === t2) {
    if (1 !== o2.length || "default" !== o2[0]) return lt(sn("default", o2, i2));
  } else if ("none" === t2 && o2.length > 0) return lt(sn("none", o2, i2));
...
function Kl(e4, t2, n2) {
  return T(e4) ? lt(yn(`Invalid pattern "${e4}" for "${t2}", patterns can be neither absolute nor relative paths. If you want your files to be stored in a subdirectory, write its name without a leading slash like this: subdirectory/pattern.`)) : e4.replace(/\[(\w+)(:\d+)?]/g, (e5, s2, i2) => {
    if (!n2.hasOwnProperty(s2) || i2 && "hash" !== s2) return lt(yn(`"[${s2}${i2 || ""}]" is not a valid placeholder in the "${t2}" pattern.`));
...
function Yl(e4, { [ql]: t2 }) {
  if (!t2.has(e4.toLowerCase())) return e4;
  const n2 = N(e4);
...
class Zl {
  constructor(e4, t2, n2, s2, i2, r2, o2, a2, l2, c2, u2, d2, h2, p2, f2) {
    ...
    for (const t3 of e4) {
      ...
      for (const e5 of t3.includedDynamicImporters) m2.has(e5) || (this.dynamicEntryModules.push(t3), t3.info.syntheticNamedExports && (c2.add(t3), this.exports.add(t3.namespace)));
  ...
  static generateFacade(e4, t2, n2, s2, i2, r2, o2, a2, l2, c2, u2, d2, h2, p2, f2) {
    ...
    for (const e5 of c2.getDependenciesToBeIncluded()) m2.dependencies.add(e5 instanceof il ? r2.get(e5) : o2.get(e5));
  ...
  canModuleBeFacade(e4, t2) {
    ...
    for (const e5 of this.exports) if (!n2.has(e5)) return false;
    for (const s2 of t2) if (!(s2.module === e4 || n2.has(s2) || s2 instanceof Da && n2.has(s2.getBaseVariable()))) return false;
    return true;
  }
  ...
  generateExports() {
    ...
    if (null !== this.facadeModule && (false !== this.facadeModule.preserveSignature || this.strictFacade)) {
      ...
      for (const [n2, s2] of t2) {
        ...
        for (const e5 of s2) this.exportsByName.set(e5, n2);
        e4.delete(n2);
      }
    }
    this.outputOptions.minifyInternalExports ? function(e5, t2, n2) {
      ...
      for (const i2 of e5) {
        let [e6] = i2.name;
        if (t2.has(e6)) do {
          e6 = wi(++s2), 49 === e6.charCodeAt(0) && (s2 += 9 * 64 ** (e6.length - 1), e6 = wi(s2));
  ...
  generateFacades() {
    ...
    for (const e5 of t2) if (e5.preserveSignature) for (const t3 of e5.getExportNamesByVariable().keys()) this.chunkByModule.get(t3.module) === this && n2.add(t3);
    for (const s2 of t2) {
      ...
      for (const n3 of t3) e4.push(Zl.generateFacade(this.inputOptions, this.outputOptions, this.unsetOptions, this.pluginDriver, this.modulesById, this.chunkByModule, this.externalChunkByModule, this.facadeChunkByModule, this.includedNamespaces, s2, n3, this.getPlaceholder, this.bundle, this.inputBase, this.snippets));
    }
    for (const e5 of this.dynamicEntryModules) e5.info.syntheticNamedExports || (!this.facadeModule && this.canModuleBeFacade(e5, n2) ? (this.facadeModule = e5, this.facadeChunkByModule.set(e5, this), this.strictFacade = true, this.dynamicName = Ql(e5)) : this.facadeModule === e5 && !this.strictFacade && this.canModuleBeFacade(e5, n2) ? this.strictFacade = true : ((_a3 = this.facadeChunkByModule.get(e5)) == null ? void 0 : _a3.strictFacade) || (this.includedNamespaces.add(e5), this.exports.add(e5.namespace)));
  ...
  link() {
    this.dependencies = function(e4, t2, n2, s2) {
      ...
      for (const e5 of i2) for (const t3 of e5) o2.add(t3);
      return o2;
    }(this, this.orderedModules, this.chunkByModule, this.externalChunkByModule);
    for (const e4 of this.orderedModules) this.addImplicitlyLoadedBeforeFromModule(e4), this.setUpChunkImportsAndExportsForModule(e4);
  }
  async render() {
    ...
    if (l2 && !c2 && null !== n2) for (const t3 of e4) t3 instanceof Zl && this.inlineChunkDependencies(t3);
    ...
    if (!$2) {
      for (const { exported: e5 } of E2) if ("default" === e5) {
        $2 = true;
        break;
      }
    }
...
`);
  ...
  addNecessaryImportsForFacades() {
    for (const [e4, t2] of this.includedReexportsByModule) if (this.includedNamespaces.has(e4)) for (const e5 of t2) this.imports.add(e5);
  }
...
Either change the import in "${B(o2)}" to point directly to the exporting module or ${a2 ? 'do not use "output.preserveModules"' : 'reconfigure "output.manualChunks"'} to ensure these modules end up in the same chunk.`, reexporter: r2 })), t2 = c2;
  ...
  getChunkExportDeclarations(e4) {
    ...
    for (const n2 of this.getExportNames()) {
      ...
      if (s2 instanceof pi) {
        for (const e5 of s2.declarations) if (e5.parent instanceof Lr || e5 instanceof Br && e5.declaration instanceof Lr) {
          r2 = true;
          break;
        }
      } else s2 instanceof Da && (i2 = o2, "es" === e4 && (o2 = s2.renderName));
  ...
  getDependenciesToBeDeconflicted(e4, t2, n2) {
    ...
    for (const t3 of [...this.exportNamesByVariable.keys(), ...this.imports]) if (e4 || t3.isNamespace) {
      const o2 = t3.module;
      if (o2 instanceof En) {
        const a2 = this.externalChunkByModule.get(o2);
        s2.add(a2), e4 && ("default" === t3.name ? io[n2(o2.id)] && i2.add(a2) : t3.isNamespace && oo[n2(o2.id)] && (this.imports.has(t3) || !((_a3 = this.exportNamesByVariable.get(t3)) == null ? void 0 : _a3.every((e5) => e5.startsWith("*")))) && r2.add(a2));
      } else {
        const n3 = this.chunkByModule.get(o2);
        n3 !== this && (s2.add(n3), e4 && "default" === n3.exportMode && t3.isNamespace && r2.add(n3));
      }
    }
    if (t2) for (const e5 of this.dependencies) s2.add(e5);
  ...
  getImportSpecifiers() {
    ...
    for (const n2 of this.imports) {
      ...
      let i2, r2;
      if (s2 instanceof En) {
        if (i2 = this.externalChunkByModule.get(s2), r2 = n2.name, "default" !== r2 && "*" !== r2 && "defaultOnly" === e4(s2.id)) return lt(mn(s2.id, r2, false));
      } else i2 = this.chunkByModule.get(s2), r2 = i2.getVariableExportName(n2);
  ...
  getIncludedDynamicImports() {
    ...
    const e4 = [];
    for (const t2 of this.orderedModules) for (const { node: n2, resolution: s2 } of t2.dynamicImports) n2.included && e4.push(s2 instanceof il ? { chunk: this.chunkByModule.get(s2), externalChunk: null, facadeChunk: this.facadeChunkByModule.get(s2), node: n2, resolution: s2 } : s2 instanceof En ? { chunk: null, externalChunk: this.externalChunkByModule.get(s2), facadeChunk: null, node: n2, resolution: s2 } : { chunk: null, externalChunk: null, facadeChunk: null, node: n2, resolution: s2 });
  ...
  getReexportSpecifiers() {
    ...
    for (let s2 of this.getExportNames()) {
      ...
      if ("*" === s2[0]) {
        ...
        if (n3 instanceof Da) continue;
        const a2 = n3.module;
        if (a2 instanceof il) {
          if (i2 = this.chunkByModule.get(a2), i2 === this) continue;
          r2 = i2.getVariableExportName(n3), o2 = n3.isReassigned;
        } else {
          if (i2 = this.externalChunkByModule.get(a2), r2 = n3.name, "default" !== r2 && "*" !== r2 && "defaultOnly" === t2(a2.id)) return lt(mn(a2.id, r2, true));
          o2 = e4 && ("default" !== r2 || ro(t2(a2.id), true));
        }
      }
  ...
  getReferencedFiles() {
    ...
    for (const t2 of this.orderedModules) for (const n2 of t2.importMetas) {
  ...
  inlineChunkDependencies(e4) {
    for (const t2 of e4.dependencies) this.dependencies.has(t2) || (this.dependencies.add(t2), t2 instanceof Zl && this.inlineChunkDependencies(t2));
  }
  renderModules(e4) {
    var _a3;
    ...
    const $2 = new b({ separator: `${x2}${x2}` }), A2 = function(e5, t3) {
      if (true !== t3.indent) return t3.indent;
      for (const t4 of e5) {
    ...
    for (const e5 of a2) {
      ...
      if (e5.isIncluded() || i2.has(e5)) {
        ...
        !I2.accessedDocumentCurrentScript && To.includes(p2) && ((_a3 = this.accessedGlobalsByScope.get(e5.scope)) == null ? void 0 : _a3.delete(so)), I2.accessedDocumentCurrentScript = false, { source: n3 } = r4, k2 || (k2 = r4.usesTopLevelAwait), s3 = n3.length(), s3 && (h2 && n3.lastLine().includes("//") && n3.append("\n"), P2.set(e5, n3), $2.addSource(n3), S2.push(e5));
        ...
        if (a3) for (const e6 of a3) v2.add(e6);
      }
  ...
  setDynamicImportResolutions(e4) {
    ...
    for (const r2 of this.getIncludedDynamicImports()) if (r2.chunk) {
  ...
  setIdentifierRenderResolutions() {
    ...
    for (const e5 of this.orderedModules) if (e5.needsExportShim) {
  ...
  setImportMetaResolutions(e4) {
    ...
    for (const r2 of s2) {
      for (const n3 of r2.importMetas) n3.setResolution(i2, t2, e4);
  ...
  setUpChunkImportsAndExportsForModule(e4) {
    ...
    if (!this.outputOptions.preserveModules && this.includedNamespaces.has(e4)) {
      ...
      for (const e5 of Object.values(n2)) e5.included && t2.add(e5);
    }
    for (let n2 of t2) {
    ...
    for (const { node: t3, resolution: n2 } of e4.dynamicImports) t3.included && n2 instanceof il && this.chunkByModule.get(n2) === this && !this.includedNamespaces.has(n2) && (this.includedNamespaces.add(n2), this.ensureReexportsAreAvailableForModule(n2));
  }
}
function Ql(e4) {
...
function tc(e4, t2) {
  ...
  for (const [s2, i2] of e4) {
    ...
    if (i2.imports) for (const { imported: t3 } of i2.imports) e5.add(t3);
    if (i2.reexports) for (const { imported: t3 } of i2.reexports) e5.add(t3);
    n2[t2(s2)] = [...e5];
  }
  return n2;
}
...
function rc(e4, t2, n2, s2) {
  const { chunkDefinitions: i2, modulesInManualChunks: r2 } = function(e5) {
    ...
    for (const [t4, i3] of e5) oc(t4, s3[i3] || (s3[i3] = []), n3);
    for (const [e6, n4] of Object.entries(s3)) t3.push({ alias: e6, modules: n4 });
    ...
    for (const e6 of i3) {
      ...
      for (const e7 of a4) {
        j(n3, e7, U).add(r3);
        for (const t4 of e7.getDependenciesToBeIncluded()) t4 instanceof En || a4.add(t4);
        for (const { resolution: n4 } of e7.dynamicImports) n4 instanceof il && n4.includedDynamicImporters.length > 0 && !i3.has(n4) && (t3.add(n4), i3.add(n4), o4.add(n4));
        for (const n4 of e7.implicitlyLoadedBefore) i3.has(n4) || (t3.add(n4), i3.add(n4));
      }
      r3++;
    }
    const o3 = [...i3], { dynamicEntries: a3, dynamicImportsByEntry: l3 } = function(e6, t4, n4) {
      const s4 = /* @__PURE__ */ new Map(), i4 = /* @__PURE__ */ new Set();
      for (const [n5, r5] of e6.entries()) s4.set(r5, n5), t4.has(r5) && i4.add(n5);
      const r4 = [];
      for (const e7 of n4) {
        const t5 = /* @__PURE__ */ new Set();
        for (const n5 of e7) t5.add(s4.get(n5));
        r4.push(t5);
      }
    ...
    for (const [s4, r3] of t3) {
      ...
      const o3 = i3[s4];
      let a3 = o3;
      for (const t4 of r3) a3 &= e5[t4] | i3[t4];
      if (a3 !== o3) {
        i3[s4] = a3;
        for (const e6 of n3[s4]) j(t3, e6, U).add(s4);
      }
    }
    return i3;
  }(d2, l2, c2, o2);
  !function(e5, t3) {
    ...
    for (const { dependentEntries: s3 } of e5) {
      for (const e6 of s3) (t3[e6] & n3) === n3 && s3.delete(e6);
      n3 <<= 1n;
    }
  }(u2, h2);
  const { chunks: p2, sideEffectAtoms: f2, sizeByAtom: m2 } = function(e5, t3, n3, s3) {
    ...
    for (const { dependentEntries: c4, modules: u3 } of e5) {
      let e6 = 0n, d3 = -1n;
      for (const s4 of c4) e6 |= 1n << BigInt(s4), d3 &= t3[s4] | n3[s4];
      ...
      let p3 = 0, f3 = true;
      for (const e7 of u3) r3.set(e7, h3), e7.isIncluded() && (f3 && (f3 = !e7.hasEffects()), p3 += s3 > 1 ? e7.estimateSize() : 1);
    ...
    return a3 |= function(e6, t4, n4) {
      ...
      for (const r4 of e6) {
        ...
        for (const a4 of o4) for (const o5 of a4.getDependenciesToBeIncluded()) if (o5 instanceof En) {
  ...
  return i2.push(...function(e5, t3, n3, s3, i3) {
    ...
    const r3 = function(e6, t4) {
      const n4 = [], s4 = [];
      for (const i4 of e6) (i4.size < t4 ? n4 : s4).push(i4);
      if (0 === n4.length) return null;
      return n4.sort(lc), s4.sort(lc), { big: new Set(s4), small: new Set(n4) };
    }(e5, t3);
    if (!r3) return Xa("optimize chunks", 3), e5;
    return t3 > 1 && i3("info", cn(e5.length, r3.small.size, "Initially")), function(e6, t4, n4, s4) {
      ...
      for (const r4 of i4) {
        ...
        if (o3) {
          ...
          for (const e7 of r4.dependentEntries) h3.add(e7);
          for (const e7 of r4.dependencies) u3.add(e7), e7.dependentChunks.delete(r4), e7.dependentChunks.add(o3);
          for (const e7 of r4.dependentChunks) d3.add(e7), e7.dependencies.delete(r4), e7.dependencies.add(o3);
...
function oc(e4, t2, n2) {
  ...
  for (const e5 of s2) {
    ...
    for (const t3 of e5.dependencies) t3 instanceof En || n2.has(t3) || s2.add(t3);
  }
}
function ac(e4, t2, n2) {
  ...
  for (const i2 of t2) {
    ...
    for (const n3 of ic([r2.includedDynamicImporters, r2.implicitlyLoadedAfter])) for (const s3 of e4.get(n3)) t3.add(s3);
  }
  return s2;
}
...
function dc(e4, t2, n2, s2, i2) {
  ...
  return function(e5, t3, n3) {
    let s3 = 0, i3 = 0, r3 = 1n;
    const { length: o3 } = n3;
    for (; i3 < o3; i3++) if ((e5 & r3) === r3 && (s3 += n3[i3]), r3 <<= 1n, s3 >= t3) return 1 / 0;
    return s3;
  }(o2 & ~r2, n2, i2);
}
...
function fc(e4, t2, n2) {
  ...
  let r2 = t2;
  for (e4.cycles.add(s2); r2 !== e4; ) r2.cycles.add(s2), i2.push(r2.id), r2 = n2.get(r2);
    ...
    for (const o2 of this.mappings) {
      ...
      for (const r3 of o2) {
        ...
        if (l2) {
          ...
          let p2 = t2.get(h2);
          if (void 0 === p2) p2 = e4.length, e4.push(h2), t2.set(h2, p2), n2[p2] = d2;
          else if (null == n2[p2]) n2[p2] = d2;
          else if (null != d2 && n2[p2] !== d2) return lt(fn(h2));
...
async function vc(e4, t2, n2, s2, i2) {
  Ja("render chunks", 2), function(e5) {
    for (const t3 of e5) t3.facadeModule && t3.facadeModule.isUserDefinedEntryPoint && t3.getPreliminaryFileName();
  }(e4);
  ...
  const o2 = Ac[s2.hashCharacters], a2 = function(e5) {
    ...
    for (const { preliminaryFileName: { hashPlaceholder: t4 } } of e5) t4 && u3.add(t4);
...
async function Pc(e4, t2, n2, s2, i2, r2, o2) {
  ...
  if (d2 || "\n" === u2[u2.length - 1] || (u2 += "\n"), f2) {
    ...
    l2 = function(e5, t3, n3, s4, i3, r3) {
      ...
      h3 = i3 ? null : h3;
      for (const e6 of n3) _a2(e6.originalSourcemap, e6.sourcemapChain);
...
class kc {
  ...
  async generate(e4) {
    ...
    try {
      ...
      const e5 = /* @__PURE__ */ (() => {
        let e6 = 0;
        return (t4, n3) => {
          if (n3 > 22) return lt(yn(`Hashes cannot be longer than 22 characters, received ${n3}. Check the "${t4}" option.`));
          const s2 = `${Vl}${wi(++e6).padStart(n3 - 5, "0")}${Fl}`;
      ...
      t3.length > 1 && function(e6, t4) {
        if ("umd" === e6.format || "iife" === e6.format) return lt(on("output.format", tt, "UMD and IIFE output formats are not supported for code-splitting builds", e6.format));
        if ("string" == typeof e6.file) return lt(on("output.file", Ze, 'when building multiple chunks, the "output.dir" option must be used, not "output.file". To inline dynamic imports, set the "inlineDynamicImports" option'));
        if (e6.sourcemapFile) return lt(on("output.sourcemapFile", at, '"output.sourcemapFile" is only supported for single-file builds'));
      ...
      for (const e6 of t3) e6.generateExports();
    ...
    return ((e5) => {
      ...
      for (const e6 of n3) "asset" === e6.type && e6.needsCodeReference && t3.add(e6.fileName);
      for (const e6 of n3) if ("chunk" === e6.type) for (const n4 of e6.referencedFiles) t3.has(n4) && t3.delete(n4);
      for (const n4 of t3) delete e5[n4];
  ...
  async generateChunks(e4, t2) {
    const { experimentalMinChunkSize: n2, inlineDynamicImports: s2, manualChunks: i2, preserveModules: r2 } = this.outputOptions, o2 = "object" == typeof i2 ? await this.addManualChunks(i2) : this.assignManualChunks(i2), a2 = function({ compact: e5, generatedCode: { arrowFunctions: t3, constBindings: n3, objectShorthand: s3, reservedNamesAsProps: i3 } }) {
      ...
      const t3 = [];
      for (const n3 of e5.values()) n3 instanceof il && (n3.isIncluded() || n3.info.isEntry || n3.includedDynamicImporters.length > 0) && t3.push(n3);
      return t3;
    }(this.graph.modulesById), c2 = function(e5) {
      if (0 === e5.length) return "/";
      if (1 === e5.length) return k(e5[0]);
      ...
      const n3 = [];
      for (const s3 of e5) (s3.info.isEntry || t3) && w(s3.id) && n3.push(s3.id);
      return n3;
    }(l2, r2)), u2 = function(e5, t3, n3) {
      const s3 = /* @__PURE__ */ new Map();
      for (const i3 of e5.values()) i3 instanceof En && s3.set(i3, new F(i3, t3, n3));
      return s3;
    ...
    for (const e5 of d2) e5.link();
    const p2 = [];
    for (const e5 of d2) p2.push(...e5.generateFacades());
    return [...d2, ...p2];
  }
}
function Nc(e4, t2, n2) {
  const s2 = n2.get(t2);
  if ("string" == typeof s2 && s2 !== e4) return lt((i2 = t2.id, r2 = e4, o2 = s2, { code: wt, message: `Cannot assign "${B(i2)}" to the "${r2}" chunk as it is already in the "${o2}" chunk.` }));
  var i2, r2, o2;
  n2.set(t2, e4);
}
...
async function Rc(e4, t2, n2, s2, i2, r2, o2, a2, l2) {
  const c2 = await function(e5, t3, n3, s3, i3, r3, o3, a3) {
    ...
    if (i3) {
      l3 = /* @__PURE__ */ new Set();
      for (const n4 of i3) e5 === n4.source && t3 === n4.importer && l3.add(n4.plugin);
...
async function Yc(e4, t2, n2, s2) {
  ...
  try {
    b2 = await n2.hookReduceArg0("transform", [m2, i2], function(e5, n3, i3) {
      let o3, a2;
      if ("string" == typeof n3) o3 = n3;
      else {
        if (!n3 || "object" != typeof n3) return e5;
        if (t2.updateOptions(n3), null == n3.code) return (n3.map || n3.ast) && s2(Le, function(e6) {
...
class Xc {
    ...
    return this.extendLoadModulesPromise(this.loadEntryModule(e4.id, false, e4.importer, null).then(async (s2) => {
      if (Qc(s2, e4, false, n2), !s2.info.isEntry) {
        ...
        if (!s2.info.isEntry) {
          ...
          for (const e5 of n3) s2.implicitlyLoadedAfter.add(e5);
          for (const e5 of s2.implicitlyLoadedAfter) e5.implicitlyLoadedBefore.add(s2);
        }
      }
      return s2;
    }));
  ...
  async addModuleSource(e4, t2, n2) {
    ...
    if (!o2 || o2.customTransformCache || o2.originalCode !== i2.code || await this.pluginDriver.hookFirst("shouldTransformCachedModule", [{ ast: o2.ast, code: o2.code, id: o2.id, meta: o2.meta, moduleSideEffects: o2.moduleSideEffects, resolvedSources: o2.resolvedIds, syntheticNamedExports: o2.syntheticNamedExports }])) n2.updateOptions(i2), await n2.setSource(await Yc(i2, n2, this.pluginDriver, this.options.onLog));
    else {
      if (o2.transformFiles) for (const e5 of o2.transformFiles) this.pluginDriver.emitFile(e5);
  ...
  async fetchDynamicDependencies(e4, t2) {
    ...
    for (const t3 of n2) t3 && (e4.dynamicDependencies.add(t3), t3.dynamicImporters.push(e4.id));
  }
  async fetchModule({ attributes: e4, id: t2, meta: n2, moduleSideEffects: s2, syntheticNamedExports: i2 }, r2, o2, a2) {
    ...
    if (l2 instanceof il) return r2 && ja(e4, l2.info.attributes) && this.options.onLog(Le, en(l2.info.attributes, e4, t2, r2)), await this.handleExistingModule(l2, o2, a2), l2;
    if (l2 instanceof En) return lt({ code: "EXTERNAL_MODULES_CANNOT_BE_TRANSFORMED_TO_MODULES", message: `${l2.id} is resolved as a module now, but it was an external module before. Please check whether there are conflicts in your Rollup options "external" and "manualChunks", manualChunks cannot include external modules.` });
  ...
  fetchResolvedDependency(e4, t2, n2) {
    if (n2.external) {
      ...
      if (l2) {
        if (!(l2 instanceof En)) return lt(function(e5, t3) {
      ...
      } else l2 = new En(this.options, r2, o2, a2, "absolute" !== i2 && w(r2), s2), this.modulesById.set(r2, l2);
      return Promise.resolve(l2);
    }
    return this.fetchModule(n2, t2, false, false);
  }
  async fetchStaticDependencies(e4, t2) {
    for (const n2 of await Promise.all(t2.map((t3) => t3.then(([t4, n3]) => this.fetchResolvedDependency(t4, e4.id, n3))))) e4.dependencies.add(n2), n2.importers.push(e4.id);
    if (!this.options.treeshake || "no-treeshake" === e4.info.moduleSideEffects) for (const t3 of e4.dependencies) t3 instanceof il && (t3.importedFromNotTreeshaken = true);
  }
  ...
  async handleExistingModule(e4, t2, n2) {
    ...
    if (n2) return n2 === Jc ? tu(s2) : s2;
    if (t2) {
      e4.info.isEntry = true, this.implicitEntryModules.delete(e4);
      for (const t3 of e4.implicitlyLoadedAfter) t3.implicitlyLoadedBefore.delete(e4);
  ...
  async loadEntryModule(e4, t2, n2, s2, i2 = false) {
    ...
    if (null == r2) return lt(null === s2 ? function(e5) {
  ...
  async resolveDynamicImport(e4, t2, n2, s2) {
    ...
    if ("string" != typeof t2) return "string" == typeof i2 ? i2 : i2 ? this.getResolvedIdWithDefaults(i2, s2) : null;
    if (null == i2) {
...
function Qc(e4, { fileName: t2, name: n2 }, s2, i2) {
  var _a3;
  if (null !== t2) e4.chunkFileNames.add(t2);
  else if (null !== n2) {
    let t3 = 0;
    for (; ((_a3 = e4.chunkNames[t3]) == null ? void 0 : _a3.priority) < i2; ) t3++;
...
class nu extends Pi {
  ...
  findVariable(e4) {
    ...
    return t2 || (t2 = new hi(e4), this.variables.set(e4, t2)), t2;
  }
}
...
class cu {
  constructor(e4, t2, n2) {
    this.graph = e4, this.options = t2, this.facadeChunkByModule = null, this.nextIdBase = 1, this.output = null, this.outputFileEmitters = [], this.emitFile = (e5) => function(e6) {
      ...
      for (const [e5, t3] of this.filesByReferenceId) if ("asset" === t3.type && "string" != typeof t3.fileName) return lt({ code: "ASSET_SOURCE_MISSING", message: `Plugin error creating asset "${t3.name || e5}" - no asset source set.` });
    }, this.getFileName = (e5) => {
      ...
      if (!n3) return lt({ code: "ASSET_NOT_FOUND", message: `Plugin error - Unable to set the source for unknown asset "${e5}".` });
      if ("asset" !== n3.type) return lt(yn(`Asset sources can only be set for emitted assets but "${e5}" is an emitted chunk.`));
      if (void 0 !== n3.source) return lt({ code: "ASSET_SOURCE_ALREADY_SET", message: `Unable to set the source for asset "${n3.name || e5}", source already set.` });
      const s2 = ou(t3, n3, e5);
      if (this.output) this.finalizeAdditionalAsset(n3, s2, this.output);
      else {
        n3.source = s2;
        for (const e6 of this.outputFileEmitters) e6.finalizeAdditionalAsset(n3, s2, e6.output);
      }
      ...
      for (const e6 of this.filesByReferenceId.values()) e6.fileName && iu(e6.fileName, s2, this.options.onLog);
      const i2 = /* @__PURE__ */ new Map();
      for (const e6 of this.filesByReferenceId.values()) if ("asset" === e6.type && void 0 !== e6.source) if (e6.fileName) this.finalizeAdditionalAsset(e6, e6.source, s2);
      else {
        j(i2, n3(e6.source), () => []).push(e6);
      }
      else "prebuilt-chunk" === e6.type && (this.output.bundle[e6.fileName] = this.createPrebuiltChunk(e6));
      for (const [e6, t4] of i2) this.finalizeAssetsWithSameSource(t4, e6, s2);
  ...
  assignReferenceId(e4, t2) {
    ...
    for (const { filesByReferenceId: t3 } of this.outputFileEmitters) t3.set(n2, e4);
    return n2;
  }
  ...
  emitAsset(e4) {
    ...
    if (this.output) this.emitAssetWithReferenceId(n2, this.output);
    else for (const e5 of this.outputFileEmitters) e5.emitAssetWithReferenceId(n2, e5.output);
    return s2;
  }
  ...
  emitChunk(e4) {
    if (this.graph.phase > Ma.LOAD_AND_PARSE) return lt({ code: "INVALID_ROLLUP_PHASE", message: "Cannot emit chunks after module loading has finished." });
    if ("string" != typeof e4.id) return lt(yn(`Emitted chunks need to have a valid string id, received "${e4.id}"`));
  ...
  emitPrebuiltChunk(e4) {
    if ("string" != typeof e4.code) return lt(yn(`Emitted prebuilt chunks need to have a valid string code, received "${e4.code}".`));
    if ("string" != typeof e4.fileName || T(e4.fileName)) return lt(yn(`The "fileName" property of emitted prebuilt chunks must be strings that are neither absolute nor relative paths, received "${e4.fileName}".`));
...
function du(t2, n2, s2, i2, r2, o2) {
  ...
  let c2, u2 = true;
  if ("string" != typeof t2.cacheKey && (t2.name.startsWith(_c) || t2.name.startsWith(Lc) || o2.has(t2.name) ? u2 = false : o2.add(t2.name)), n2) if (u2) {
    const e4 = t2.cacheKey || t2.name;
    h2 = n2[e4] || (n2[e4] = /* @__PURE__ */ Object.create(null)), c2 = { delete: (e5) => delete h2[e5], get(e5) {
      const t3 = h2[e5];
      if (t3) return t3[0] = 0, t3[1];
    }, has(e5) {
      const t3 = h2[e5];
  ...
  } else d2 = t2.name, c2 = { delete: () => Tc(d2), get: () => Tc(d2), has: () => Tc(d2), set: () => Tc(d2) };
  else c2 = Bc;
  var d2, h2;
...
class pu {
  constructor(e4, t2, n2, s2, i2) {
    ...
    if (this.pluginContexts = new Map(this.plugins.map((n3) => [n3, du(n3, s2, e4, t2, this.fileEmitter, r2)])), i2) for (const e5 of n2) for (const n3 of hu) n3 in e5 && t2.onLog(Le, (o2 = e5.name, { code: "INPUT_HOOK_IN_OUTPUT_PLUGIN", message: `The "${n3}" hook used by the output plugin ${o2} is a build time hook and will not be run for that plugin. Either this plugin cannot be used as an output plugin, or it should have an option to configure it as an output plugin.` }));
    var o2;
  }
  ...
  async hookFirstAndGetPlugin(e4, t2, n2, s2) {
    for (const i2 of this.getSortedPlugins(e4)) {
      if (s2 == null ? void 0 : s2.has(i2)) continue;
      const r2 = await this.runHook(e4, t2, i2, n2);
      if (null != r2) return [r2, i2];
    }
    return null;
  }
  ...
  async hookParallel(e4, t2, n2) {
    const s2 = [];
    for (const i2 of this.getSortedPlugins(e4)) i2[e4].sequential ? (await Promise.all(s2), s2.length = 0, await this.runHook(e4, t2, i2, n2)) : s2.push(this.runHook(e4, t2, i2, n2));
    await Promise.all(s2);
  }
  hookReduceArg0(e4, [t2, ...n2], s2, i2) {
    let r2 = Promise.resolve(t2);
    for (const t3 of this.getSortedPlugins(e4)) r2 = r2.then((r3) => this.runHook(e4, [r3, ...n2], t3, i2).then((e5) => s2.call(this.pluginContexts.get(t3), r3, e5, t3)));
    return r2;
  }
  ...
  async hookReduceValue(e4, t2, n2, s2) {
    const i2 = [], r2 = [];
    for (const t3 of this.getSortedPlugins(e4, gu)) t3[e4].sequential ? (i2.push(...await Promise.all(r2)), r2.length = 0, i2.push(await this.runHook(e4, n2, t3))) : r2.push(this.runHook(e4, n2, t3));
  ...
  hookSeq(e4, t2, n2) {
    ...
    for (const i2 of this.getSortedPlugins(e4)) s2 = s2.then(() => this.runHook(e4, t2, i2, n2));
    return s2.then(yu);
  }
...
function gu(e4, t2, n2) {
  if ("string" != typeof e4 && "function" != typeof e4) return lt(function(e5, t3) {
...
class Eu {
  constructor(e4, t2) {
    ...
    if (this.options = e4, this.astLru = function(e5) {
      ...
      if ((_a3 = e4.cache) == null ? void 0 : _a3.modules) for (const t3 of e4.cache.modules) this.cachedModules.set(t3.id, t3);
      ...
      for (const e5 in this.pluginCache) {
        ...
        for (const e6 of Object.values(t3)) e6[0]++;
      }
    }
    if (t2) {
      this.watchMode = true;
    ...
    this.pluginDriver = new pu(this, e4, e4.plugins, this.pluginCache), this.moduleLoader = new Xc(this, this.modulesById, this.options, this.pluginDriver), this.fileOperationQueue = new bu(e4.maxParallelFileOps), this.pureFunctions = (({ treeshake: e5 }) => {
      ...
      for (const n2 of e5 ? e5.manualPureFunctions : []) {
        let e6 = t3;
        for (const t4 of n2.split(".")) e6 = e6[t4] || (e6[t4] = /* @__PURE__ */ Object.create(null));
        e6[Ws] = true;
      }
      return t3;
    })(e4);
  ...
  getCache() {
    for (const e4 in this.pluginCache) {
      ...
      let n2 = true;
      for (const [e5, s2] of Object.entries(t2)) s2[0] >= this.options.experimentalCacheExpiry ? delete t2[e5] : n2 = false;
  ...
  async generateModuleGraph() {
    var e4;
    if ({ entryModules: this.entryModules, implicitEntryModules: this.implicitEntryModules } = await this.moduleLoader.addEntryModules((e4 = this.options.input, Array.isArray(e4) ? e4.map((e5) => ({ fileName: null, id: e5, implicitlyLoadedAfter: [], importer: void 0, name: null })) : Object.entries(e4).map(([e5, t2]) => ({ fileName: null, id: t2, implicitlyLoadedAfter: [], importer: void 0, name: e5 }))), true), 0 === this.entryModules.length) throw new Error("You must supply options.input to rollup");
    for (const e5 of this.modulesById.values()) e5.cacheInfoGetters(), e5 instanceof il ? this.modules.push(e5) : this.externalModules.push(e5);
  }
  includeStatements() {
    const e4 = [...this.entryModules, ...this.implicitEntryModules];
    for (const t2 of e4) el(t2);
    if (this.options.treeshake) {
      let t2 = 1;
      do {
        Ja(`treeshaking pass ${t2}`, 3), this.needsTreeshakingPass = false;
        for (const e5 of this.modules) e5.isExecuted && ("no-treeshake" === e5.info.moduleSideEffects ? e5.includeAllInBundle() : e5.include());
        if (1 === t2) for (const t3 of e4) false !== t3.preserveSignature && (t3.includeAllExports(false), this.needsTreeshakingPass = true);
        Xa("treeshaking pass " + t2++, 3);
      } while (this.needsTreeshakingPass);
    } else for (const e5 of this.modules) e5.includeAllInBundle();
    for (const e5 of this.externalModules) e5.warnUnusedImports();
    for (const e5 of this.implicitEntryModules) for (const t2 of e5.implicitlyLoadedAfter) t2.info.isEntry || t2.isIncluded() || lt(ln(t2));
  }
  sortModules() {
    const { orderedModules: e4, cyclePaths: t2 } = function(e5) {
      ...
      const n2 = [], s2 = /* @__PURE__ */ new Set(), i2 = /* @__PURE__ */ new Set(), r2 = /* @__PURE__ */ new Map(), o2 = [], a2 = (e6) => {
        if (e6 instanceof il) {
          for (const t4 of e6.dependencies) r2.has(t4) ? s2.has(t4) || n2.push(fc(t4, e6, r2)) : (r2.set(t4, e6), a2(t4));
          for (const t4 of e6.implicitlyLoadedBefore) i2.add(t4);
          for (const { resolution: t4 } of e6.dynamicImports) t4 instanceof il && i2.add(t4);
          o2.push(e6);
        }
        e6.execIndex = t3++, s2.add(e6);
      };
      for (const t4 of e5) r2.has(t4) || (r2.set(t4, null), a2(t4));
      for (const e6 of i2) r2.has(e6) || (r2.set(e6, null), a2(e6));
      return { cyclePaths: n2, orderedModules: o2 };
    }(this.entryModules);
    for (const e5 of t2) this.options.onLog(Le, Xt(e5));
    this.modules = e4;
    for (const e5 of this.modules) e5.bindReferences();
    this.warnForMissingExports();
  }
  warnForMissingExports() {
    for (const e4 of this.modules) for (const t2 of e4.importDescriptions.values()) "*" === t2.name || t2.module.getVariableForExportName(t2.name)[0] || e4.log(Le, an(t2.name, e4.id, t2.module.id), t2.start);
  }
}
function xu(e4, t2) {
  return t2();
...
function $u(t2, n2, s2, i2) {
  ...
  const r2 = ze[i2], o2 = (i3, a2, l2 = ve) => {
    ...
    if (!(ze[i3] < r2)) {
      for (const n3 of t2) {
        if (l2.has(n3)) continue;
        const { onLog: t3 } = n3, c2 = (e4) => ze[e4] < r2 ? qs : (t4) => o2(e4, jc(t4), new Set(l2).add(n3));
        if (false === ("handler" in t3 ? t3.handler : t3).call({ debug: c2(Te), error: (e4) => lt(jc(e4)), info: c2(Be), meta: { rollupVersion: e, watchMode: s2 }, warn: c2(Le) }, i3, a2)) return;
      }
      n2(i3, a2);
    }
  };
...
const Au = (e4) => {
  ...
  if ("function" == typeof e4) return (t3, ...n2) => !t3.startsWith("\0") && e4(t3, ...n2) || false;
  if (e4) {
    const n2 = /* @__PURE__ */ new Set(), s2 = [];
    for (const i2 of (t2 = e4, Array.isArray(t2) ? t2.filter(Boolean) : t2 ? [t2] : [])) i2 instanceof RegExp ? s2.push(i2) : n2.add(i2);
  ...
  if ("function" == typeof n2) return (e5) => n2(e5) ?? t2;
  if (n2) {
    const e5 = /* @__PURE__ */ Object.create(null);
    for (const [t3, s2] of Object.entries(n2)) e5[O(t3)] = s2;
    return (n3) => e5[n3] ?? t2;
  }
  return () => t2;
}, Iu = (e4) => {
  if (false === e4.treeshake) return false;
  ...
  if ("no-external" === e4) return (e5, t2) => !t2;
  if ("function" == typeof e4) return (t2, n2) => !!t2.startsWith("\0") || false !== e4(t2, n2);
  if (Array.isArray(e4)) {
    const t2 = new Set(e4);
    return (e5) => t2.has(e5);
  }
...
const Du = (e4, t2, n2) => {
  const { file: s2 } = e4;
  if ("string" == typeof s2) {
    if (t2) return lt(on("output.file", Ze, 'you must set "output.dir" instead of "output.file" when using the "output.preserveModules" option'));
    if (!Array.isArray(n2.input)) return lt(on("output.file", Ze, 'you must set "output.dir" instead of "output.file" when providing named inputs'));
  }
  return s2;
}, Mu = (e4) => {
  const t2 = e4.format;
  ...
  if (s2) {
    if (t2) return lt(on("output.inlineDynamicImports", st, 'this option is not supported for "output.preserveModules"'));
    if (false === n2.preserveEntrySignatures) return lt(on("preserveEntrySignatures", "configuration-options/#preserveentrysignatures", 'setting this option to false is not supported for "output.preserveModules"'));
  }
  return s2;
}, Lu = (e4) => {
  const { preserveModulesRoot: t2 } = e4;
  if (null != t2) return O(t2);
}, Bu = (e4) => {
...
function Fu(e4, t2) {
  const n2 = e4.exports;
  if (null == n2) t2.add("exports");
  else if (!["default", "named", "none", "auto"].includes(n2)) return lt({ code: vt, message: `"output.exports" must be "default", "named", "none", "auto", or left unspecified (defaults to "auto"), received "${n2}".`, url: Ke(Qe) });
  return n2 || "auto";
}
const ju = (e4, t2) => (null != e4.externalImportAssertions && bn('The "output.externalImportAssertions" option is deprecated. Use the "output.externalImportAttributes" option instead.', "configuration-options/#output-externalimportattributes", 0, t2), e4.externalImportAttributes ?? e4.externalImportAssertions ?? true), Uu = (e4) => {
  ...
  if (s2) {
    if (t2) return lt(on("output.manualChunks", rt, 'this option is not supported for "output.inlineDynamicImports"'));
    if (n2) return lt(on("output.manualChunks", rt, 'this option is not supported for "output.preserveModules"'));
  }
  return s2 || {};
...
function Zu(t2) {
  return async function(t3, n2) {
    const { options: s2, unsetOptions: i2 } = await async function(t4, n3) {
      if (!t4) throw new Error("You must supply an options object to rollup");
    ...
    (function(e4) {
      e4.perf ? (Wa = /* @__PURE__ */ new Map(), Ja = Ha, Xa = Ka, e4.plugins = e4.plugins.map(Qa)) : (Ja = qs, Xa = qs);
...
async function ed(e4, t2, n2, s2, i2) {
  ...
  return xu(0, async () => {
    ...
    if (e4) {
      if (Ja("WRITE", 1), !r2.dir && !r2.file) return lt({ code: Lt, message: 'You must specify "output.file" or "output.dir" for the build.', url: Ke(Ze) });
